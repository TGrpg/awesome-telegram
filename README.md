<div align="center">

<a href="https://tgbox.cc/en/"><img src="assets/logo.svg" width="96" height="96" alt="TGbox"></a>

# Awesome Telegram [![Awesome](https://awesome.re/badge.svg)](https://awesome.re)

A curated, auto-updated list of 992 public Telegram channels, groups and bots — with live member counts, recent posts and categories on [TGbox](https://tgbox.cc/en).

[🌐 Website](https://tgbox.cc/en) · [📮 Submit yours](https://t.me/tgboxccbot?start=submit) · [🤝 Exchange links](https://t.me/tgboxccbot?start=links) · **English** · [简体中文](README.zh-CN.md) · [繁體中文](README.zh-Hant.md)

[![Entries](https://img.shields.io/badge/Channels%20%2B%20Groups%20%2B%20Bots-992-2AABEE?logo=telegram&logoColor=white)](https://tgbox.cc/en/)
[![Updated](https://img.shields.io/badge/updated-2026--09--26-brightgreen)](https://github.com/TGrpg/awesome-telegram/actions/workflows/update.yml)
[![License: CC BY 4.0](https://img.shields.io/badge/license-CC%20BY%204.0-lightgrey.svg)](LICENSE)

</div>

> Every name below opens its page on **TGbox**, a free, open-source Telegram directory: subscriber counts and growth, recent posts, creation date, activity and similar channels, in English, Simplified and Traditional Chinese. The list is regenerated every day from the site's [open data](https://tgbox.cc/data/entries.json), so dead or banned entries drop out on their own.

**382** channels · **244** groups · **366** bots · updated 2026-09-26

## Contents

- [🔥 Most popular](#popular)
- [🆕 Recently added](#newest)
- [Channels](#channel) (382)
  - [News](#channel-news) (64)
  - [Software](#channel-software) (59)
  - [Crypto & Web3](#channel-crypto) (42)
  - [Development](#channel-tech) (31)
  - [Resources](#channel-resources) (26)
  - [Blogs](#channel-blog) (24)
  - [VPN & Proxy](#channel-vpn) (20)
  - [Deals](#channel-deals) (14)
  - [AI](#channel-ai) (13)
  - [Fun](#channel-fun) (13)
  - [Wallpapers & Images](#channel-wallpaper) (13)
  - [Books & Comics](#channel-books) (10)
  - [Games](#channel-games) (10)
  - [Video & Music](#channel-video) (8)
  - [Life & Living](#channel-life) (7)
  - [Cloud Drives](#channel-cloud-drive) (6)
  - [Jobs & Careers](#channel-jobs) (6)
  - [Learning](#channel-learning) (6)
  - [Giveaways & Freebies](#channel-giveaway) (4)
  - [Directories](#channel-nav) (4)
  - [Stickers](#channel-stickers) (2)
- [Groups](#group) (244)
  - [Development](#group-tech) (38)
  - [Crypto & Web3](#group-crypto) (29)
  - [Software](#group-software) (27)
  - [VPN & Proxy](#group-vpn) (24)
  - [Interests](#group-interest) (22)
  - [Chat](#group-chat) (19)
  - [iOS](#group-ios) (17)
  - [AI](#group-ai) (16)
  - [Local & Regional](#group-local) (14)
  - [Learning & Exams](#group-learning) (11)
  - [Hosting & VPS](#group-vps) (11)
  - [Games](#group-games) (5)
  - [Video & Music](#group-video) (5)
  - [Jobs & Careers](#group-jobs) (4)
  - [Other](#group-other) (2)
- [Bots](#bot) (366)
  - [Utilities](#bot-tools) (77)
  - [Group Admin](#bot-group-admin) (50)
  - [Media Download](#bot-media) (40)
  - [Search](#bot-search) (39)
  - [Productivity](#bot-productivity) (27)
  - [Channel Tools](#bot-channel-tools) (17)
  - [Files & Storage](#bot-files) (16)
  - [Stickers & Emoji](#bot-stickers) (16)
  - [RSS & Feeds](#bot-rss) (14)
  - [Crypto & Wallets](#bot-crypto) (12)
  - [Games & Fun](#bot-games) (12)
  - [Giveaways](#bot-giveaway) (10)
  - [Analytics](#bot-analytics) (9)
  - [Translation](#bot-translate) (9)
  - [AI Assistants](#bot-ai) (7)
  - [Trading Bots](#bot-trading) (7)
  - [Messaging](#bot-messaging) (4)
- [How this list works](#how)
- [Add or report an entry](#contribute)
- [License](#license)

<a id="popular"></a>

## 🔥 Most popular

| Name | Category | Size | About |
| --- | --- | ---: | --- |
| [Spam Info Bot](https://tgbox.cc/en/detail/spambot/) ✔️ | bot · Utilities | 35.6M | The official Spam Info Bot by Telegram. Helps users with… |
| [Foursquare](https://tgbox.cc/en/detail/foursquare/) | bot · Search | 21.4M | This bot can help you find restaurants and other places… |
| [Telegram Tips](https://tgbox.cc/en/detail/telegramtips/) ✔️ | channel · Software | 11M | Telegram stands for freedom and privacy and has many easy… |
| [Pavel Durov](https://tgbox.cc/en/detail/durov/) ✔️ | channel · Blogs | 10.6M | Founder of Telegram. |
| [Telegram News](https://tgbox.cc/en/detail/telegram/) ✔️ | channel · News | 9.5M | The official Telegram on Telegram. Much recursion. Very… |
| [Notcoin Community](https://tgbox.cc/en/detail/notcoin/) ✔️ | channel · Crypto & Web3 | 8.6M | notcoin.org |
| [BotFather](https://tgbox.cc/en/detail/botfather/) ✔️ | bot · Utilities | 8.2M | BotFather is the one bot to rule them all. Use it to create… |
| [Premium Bot](https://tgbox.cc/en/detail/premiumbot/) ✔️ | bot · Utilities | 6.7M | This bot lets you subscribe to Telegram Premium or buy… |
| [Telegram Premium](https://tgbox.cc/en/detail/premium/) ✔️ | channel · Software | 6.6M | Telegram Premium – a subscription that unlocks dozens of… |
| [极搜🔍资源搜索@JISOU](https://tgbox.cc/en/detail/jisou/) | bot · Search | 5.7M | Telegram is the essential search engine for JISOU to help… |

<a id="newest"></a>

## 🆕 Recently added

| Name | Category | Listed | About |
| --- | --- | ---: | --- |
| [欧易OKX官方中文群](https://tgbox.cc/en/detail/okxgroup_cn/) ✔️ | group · Chat | 2026-09-20 | OKX公告频道：https://t.me/OKXAnnouncements\_CN Discord中文社区：… |
| [APP喵 - 破解软件资源](https://tgbox.cc/en/detail/appmew/) | channel · Resources | 2026-09-20 | APP喵资源仓库。发现分享好用 APP 软件，优质 资源，游戏，网站，极客，教程，各种 网络… |
| [ReVanced](https://tgbox.cc/en/detail/app_revanced/) | channel · Software | 2026-09-20 | Continuing the legacy of Vanced at https://revanced.app… |
| [Aero Apps \| Channel](https://tgbox.cc/en/detail/aerolla/) | channel · Software | 2026-09-20 | Aero Apps Official Telegram Channel. Web: www.whatsaero.com… |
| [中国数字时代](https://tgbox.cc/en/detail/cdtchinesefeed/) | channel · News | 2026-09-20 | 官方名片：http://cdt.media 官方推特账号：https://twitter.com/CDTChinese… |
| [链接收藏](https://tgbox.cc/en/detail/ldlist/) | channel · Resources | 2026-09-20 | 链接收藏 网址导航 评论区规则，包含并不仅限 1、封禁【博彩/赌毒/宗教/政治/键政/黑产/灰产/违法色情/暴力】… |
| [xManager - Updates](https://tgbox.cc/en/detail/xmanagerupdates/) | channel · Software | 2026-09-20 | Official Channel for xManager by xC3FFF0E Website:… |
| [Bybit 华语交流群](https://tgbox.cc/en/detail/bybitzh/) ✔️ | group · Chat | 2026-09-20 | 官方公告频道👉 https://t.me/bybit\_announcement\_ZH… |
| [海外媒体中国新闻](https://tgbox.cc/en/detail/chinanews001/) | channel · News | 2026-09-20 | 海外媒体关于中国的报道 需翻墙打开网页的，可用网页代理网站https://telegramcn.herokuapp.co… |
| [NGOCN](https://tgbox.cc/en/detail/ngocn01/) | channel · News | 2026-09-20 | 独立发声。欢迎关注我们：https://linktr.ee/ngocn |

<a id="channel"></a>

## Channels

<a id="channel-news"></a>

### News

| Name | Subscribers | About |
| --- | ---: | --- |
| [Telegram News](https://tgbox.cc/en/detail/telegram/) ✔️ | 9.5M | The official Telegram on Telegram. Much recursion. Very Telegram. Wow. <sub>`Official`</sub> |
| [在花🎗️科技圈](https://tgbox.cc/en/detail/zaihuanews/) | 283.1K | 分享 科技、AI、数码、互联网及前沿资讯，分享见闻与实用内容。 📌 群规 • 文明交流，理性讨论 • 禁止刷屏、广告推广、恶意引流 • 禁止黑产、色情、赌博等违规内容 •… <sub>`Daily Digest`</sub> |
| [在花🎗️科技圈](https://tgbox.cc/en/detail/testflightcn/) | 283.1K | Share technology, AI, digital, Internet and cutting-edge information to share insights… <sub>`Daily Digest` `Gadgets`</sub> |
| [在花🎗️科技圈](https://tgbox.cc/en/detail/zaihuapd/) | 283K | Share technology, AI, digital, Internet and cutting-edge information to share insights… <sub>`Daily Digest`</sub> |
| [竹新社](https://tgbox.cc/en/detail/tnews365/) | 164.2K | 7×24 compiles instant news reports from domestic and foreign media on a regular basis.… <sub>`Daily Digest`</sub> |
| [风向旗参考快讯](https://tgbox.cc/en/detail/xhqcankao/) | 163.3K | This channel is designed to expand the information horizon and does not represent the… <sub>`Gadgets` `Hosting & VPS`</sub> |
| [Noticias de Telegram](https://tgbox.cc/en/detail/telegrames/) ✔️ | 147.5K | Canal oficial de noticias de Telegram en español. <sub>`Official`</sub> |
| [Telegram Brasil](https://tgbox.cc/en/detail/telegrambr/) ✔️ | 114.7K | Canal oficial do Telegram, no Telegram, sobre o Telegram (Wow) com detalhes das… <sub>`Official`</sub> |
| [你不知道的内幕消息🅥](https://tgbox.cc/en/detail/inside1024/) | 107.8K | Some of the insider news you don't know, the latest news, big company negativity,… <sub>`Daily Digest`</sub> |
| [电报时报](https://tgbox.cc/en/detail/times001/) | 84.9K | Channel linked linked linked linked linked linked linked linked linked linked linked… <sub>`Daily Digest`</sub> |
| [قناة تيليجرام](https://tgbox.cc/en/detail/telegramarabia/) ✔️ | 78.6K | مرحبًا بك في قناة أخبار تيليجرام الرسمية. هنا سنقوم بنشر آخر التحديثات بالإضافة لنصائح… <sub>`Official`</sub> |
| [每日消费电子观察](https://tgbox.cc/en/detail/ce_observe/) | 74.8K | If you want to be anonymous, mistaken or live abroad, welcome PM @MizuhashiZ to add a… <sub>`Gadgets`</sub> |
| [LoopDNS资讯播报](https://tgbox.cc/en/detail/dnspodt/) | 72K | LoopDNS comprehensive information -- simple, timely, fast, accurate channel: @DNSPODT… <sub>`Daily Digest`</sub> |
| [Yummy 😋](https://tgbox.cc/en/detail/godlynews1/) | 69.2K | This channel regularly pushes new tech digital news, welcome attention! ©️ The content… <sub>`Gadgets`</sub> |
| [【华尔街见闻】- 财经时讯 \| AI 实时互动](https://tgbox.cc/en/detail/financenewsdaily/) | 57.7K | China’s leading financial information platform, 7×24 coverage of global stock markets,… <sub>`Finance`</sub> |

<details>
<summary>Show 49 more</summary>

| Name | Subscribers | About |
| --- | ---: | --- |
| [Solidot](https://tgbox.cc/en/detail/solidot/) | 47.6K | solidot.org is an unofficial RSS push channel. Remove it immediately if there is any… <sub>`Daily Digest` `Science`</sub> |
| [蓝点网订阅频道](https://tgbox.cc/en/detail/landiansub/) | 45.1K | <sub>`Daily Digest`</sub> |
| [中国数字时代](https://tgbox.cc/en/detail/cdtchinesefeed/) | 44.2K | 官方名片：http://cdt.media 官方推特账号：https://twitter.com/CDTChinese 404文库频道：https://t.me/cdt404not… |
| [NOW 新聞](https://tgbox.cc/en/detail/nowtv_news/) | 42.7K | The program automatically retrieves and pushes Now TV Hong Kong News and International… |
| [看鉴中国 OutsightChina](https://tgbox.cc/en/detail/outsightchina/) | 41.3K | A healthy society shouldn’t have just one voice. Visit China, focusing on a daily news… |
| [财经慢报](https://tgbox.cc/en/detail/financial_express/) | 37K | Affiliated group @Financial\_Express\_Chat Financial Slow News is the largest pan-financial… <sub>`Finance`</sub> |
| [ Apple Nuts](https://tgbox.cc/en/detail/applenuts/) | 37K | Let’s Think Different. @OnlineAppleUserGroup <sub>`iOS`</sub> |
| [zaobao.sg 早报](https://tgbox.cc/en/detail/zaobaosg/) ✔️ | 36.5K | Talk to you with wisdom and wisdom. Download the Joint Morning News App:… <sub>`Daily Digest`</sub> |
| [科技&趣闻&杂记](https://tgbox.cc/en/detail/kejiqu/) | 36.4K | Share valuable and interesting information!You can also post, including but not limited… <sub>`Daily Digest`</sub> |
| [纽约时报 全文 实时推送](https://tgbox.cc/en/detail/niuyueshibao_rss/) | 34.1K | \[RSS full subscription real-time update\] News @zhihuribao\_rss New York Times… <sub>`Daily Digest`</sub> |
| [cnBeta.COM中文业界资讯站 - 总频道(精选新闻)](https://tgbox.cc/en/detail/cnbeta_com/) | 27.5K | cnBeta.COM is a Chinese website that provides IT-related news, technical articles and… <sub>`Daily Digest`</sub> |
| [BBC中文 全文 实时推送](https://tgbox.cc/en/detail/bbczhongwen_rss/) | 26.9K | \[RSS full subscription real-time update\] News @zhihuribao\_rss New York Times… <sub>`Daily Digest`</sub> |
| [金十数据 闪电资讯](https://tgbox.cc/en/detail/jin10light/) | 25.1K | Data 24H Extremely fast push! top paid articles in Chinese Times and Economy Circle! pay… <sub>`Finance`</sub> |
| [Time经济观察](https://tgbox.cc/en/detail/timehorizonx/) | 20.9K | Macroeconomics/Public Policy/Financial Markets Public: TimeHorizonX Knowledge Planet:… <sub>`Finance`</sub> |
| [科技圈的日常](https://tgbox.cc/en/detail/misakatech/) | 18.2K | Big and small things in science and technology: @UntitledGroup |
| [荔枝木](https://tgbox.cc/en/detail/lychee_wood/) | 17.7K | The world is complex. We try to coexist with the complexity, to a better tomorrow.… |
| [知乎日报 全文 实时推送](https://tgbox.cc/en/detail/zhihuribao_rss/) | 16.7K | \[RSS full subscription real-time update\] News @zhihuribao\_rss New York Times… |
| [People's Daily, China](https://tgbox.cc/en/detail/pdchinanews/) ✔️ | 15.8K | The largest newspaper in China <sub>`Official`</sub> |
| [Stop Child Abuse](https://tgbox.cc/en/detail/stopca/) ✔️ | 15.5K | This channel publishes daily updates on banned CA-related content. Report via the in-app… <sub>`Official` `Security`</sub> |
| [ISIS Watch](https://tgbox.cc/en/detail/isiswatch/) ✔️ | 15.4K | This channel publishes daily updates on banned terrorist content. Report content via the… <sub>`Official` `Security`</sub> |
| [美国之音 全文 实时推送](https://tgbox.cc/en/detail/meiguozhiyin_rss/) | 13.5K | \[RSS全文订阅 实时更新\] 知乎日报 @zhihuribao\_rss 纽约时报 @niuyueshibao\_rss BBC中文 @bbczhongwen\_rss 路透中文… <sub>`Daily Digest`</sub> |
| [Notizie da Telegram](https://tgbox.cc/en/detail/telegramit/) ✔️ | 12K | Il canale ufficiale di Telegram in italiano. <sub>`Official`</sub> |
| [经济信息联播](https://tgbox.cc/en/detail/eco_cn/) | 11.7K | Independent, personal financial, financial information morning news, summarizing U.S.… <sub>`Finance`</sub> |
| [路透中文 全文 实时推送](https://tgbox.cc/en/detail/lutouzhongwen_rss/) | 11.4K | Early reading group of friends @ideahub\_ml <sub>`Daily Digest` `Finance`</sub> |
| [华尔街日报 股票财经新闻](https://tgbox.cc/en/detail/hejrb233/) | 10.3K | The Wall Street Journal was founded in 1889 and became the largest paid financial… <sub>`Finance`</sub> |
| [LetITFly News](https://tgbox.cc/en/detail/letitflyw/) | 9.6K | @LetITFly's channels include, but are not limited to, Android, Windows, Web, consumer… <sub>`Android` `Windows`</sub> |
| [校长读报](https://tgbox.cc/en/detail/xiaozhangdubao/) | 9.5K | Read more, thank you, thank you. |
| [财经快讯](https://tgbox.cc/en/detail/fnnew/) | 8.6K | The latest worldwide financial information 24 hours uninterrupted live broadcast, A-share… <sub>`Finance`</sub> |
| [Readhub](https://tgbox.cc/en/detail/readhub_cn/) | 8.5K | Buy Ads: https://telega.io/c/readhub\_cn → More: @solidot @wanqu\_channel @idaily\_magazine… <sub>`Daily Digest`</sub> |
| [日本 共同网 朝日新闻 日经中文网](https://tgbox.cc/en/detail/jp_rss/) | 7.8K | \[RSS全文订阅 实时更新\] 知乎日报 @zhihuribao\_rss 纽约时报 @niuyueshibao\_rss BBC中文 @bbczhongwen\_rss 路透中文… <sub>`Daily Digest`</sub> |
| [德国 德国之声 中文全文](https://tgbox.cc/en/detail/dw_rss/) | 7.8K | \[RSS full subscription real-time update\] News @zhihuribao\_rss New York Times… |
| [海外媒体中国新闻](https://tgbox.cc/en/detail/chinanews001/) | 7.4K | 海外媒体关于中国的报道 需翻墙打开网页的，可用网页代理网站https://telegramcn.herokuapp.com/ |
| [台湾 中央社 香港 苹果日报](https://tgbox.cc/en/detail/ttww_rss/) | 7.1K | \[RSS full subscription real-time update\] News @zhihuribao\_rss New York Times… |
| [FT中文网 全文 实时推送](https://tgbox.cc/en/detail/ftzhongwen_rss/) | 6.7K | \[RSS full subscription real-time update\] News @zhihuribao\_rss New York Times… <sub>`Finance`</sub> |
| [Telegram Info 中文](https://tgbox.cc/en/detail/tginfocn/) | 5.7K | This is the Chinese mirror image of @tginfo. Discuss chat: @tginfocnchat Beta channel:… |
| [NGOCN](https://tgbox.cc/en/detail/ngocn01/) | 5.6K | 独立发声。欢迎关注我们：https://linktr.ee/ngocn |
| [Telegram Türkçe](https://tgbox.cc/en/detail/telegramtr/) ✔️ | 5.3K | Resmi Telegram haber kanalı. Telegram ile ilgili en son güncellemeler ve haberler için… <sub>`Official`</sub> |
| [V2EX 最热主题](https://tgbox.cc/en/detail/v2ex_hot/) | 5.2K | v2ex最热主题 RSS推送 www.V2EX.com V2EX is a community of start-ups, designers, developers and… |
| [報導者 The Reporter](https://tgbox.cc/en/detail/tw_reporter_org/) | 5K | Taiwan's first online media established by the Public Welfare Foundation, with a deep,… |
| [中文維基新聞廣播頻道](https://tgbox.cc/en/detail/wikinews_zh_broadcast/) | 4.9K | Welcome to share news or join our citizen reporter line. Chinese Wikipedia was founded in… |
| [⚡️ 7x24投资快讯](https://tgbox.cc/en/detail/golden_wind_news/) | 4.8K | 24 Hour Investment Related News <sub>`Finance`</sub> |
| [韩国 中央日报 朝鲜日报](https://tgbox.cc/en/detail/korea_rss/) | 4.4K | \[RSS全文订阅 实时更新\] 知乎日报 @zhihuribao\_rss 纽约时报 @niuyueshibao\_rss BBC中文 @bbczhongwen\_rss 路透中文… <sub>`Daily Digest`</sub> |
| [法国 国际广播电台 中文全文](https://tgbox.cc/en/detail/rfi_rss/) | 4.2K | \[RSS full subscription real-time update\] News @zhihuribao\_rss New York Times… |
| [辟谣爱好者频道](https://tgbox.cc/en/detail/fightdisinformation/) | 4.1K | 关于近期不实信息，待证实信息，有争议信息的调研分析汇总。希望保持公正，对信息新闻进行验证。如有错误欢迎指正。 姐妹频道：https://t.me/twitter\_translate… |
| [每天60秒读懂世界-已复活🎉](https://tgbox.cc/en/detail/dailynewszh/) | 3.5K | @DailyNewsZH <sub>`Daily Digest`</sub> |
| [科技第一线](https://tgbox.cc/en/detail/kejidiyixian/) | 3.2K | 第一时间分享全网最热门科技新闻 电报导航：dianbaodaohang.com <sub>`Gadgets`</sub> |
| [悦读「优质少量RSS聚合」](https://tgbox.cc/en/detail/dailyrss/) | 1.7K | 文章、图片、视频、音乐、播客RSS订阅聚合频道 部分支持「🎉Tg即时预览/内嵌播放」 订阅内容主要类型为IT、科技类文章、减压向图片音频等 💬子频道： 物种日历… |
| [金十数据](https://tgbox.cc/en/detail/jin10shandian/) | 1.4K | 金十数据, 一手时政&财经&币圈 数据24H极速推送！中文时政财经圈顶级付费文章！关注频道，洞悉世界！ <sub>`Finance`</sub> |
| [BlockBeats律动](https://tgbox.cc/en/detail/blockbeats_news/) | 41 | BlockBeats-专业的区块链研究机构与资讯平台。 <sub>`Finance`</sub> |

</details>

[All news on TGbox →](https://tgbox.cc/en/channel/news/)

<a id="channel-software"></a>

### Software

| Name | Subscribers | About |
| --- | ---: | --- |
| [Telegram Tips](https://tgbox.cc/en/detail/telegramtips/) ✔️ | 11M | Telegram stands for freedom and privacy and has many easy to use features. <sub>`Free` `Official`</sub> |
| [Telegram Premium](https://tgbox.cc/en/detail/premium/) ✔️ | 6.6M | Telegram Premium – a subscription that unlocks dozens of exclusive features. <sub>`Official`</sub> |
| [نصائح تيليجرام](https://tgbox.cc/en/detail/telegramtipsar/) ✔️ | 1.6M | يرمز تيليجرام إلى الحريّة والخصوصيّة ويحوي العديد من المزايا سهلة الاستخدام. <sub>`Free` `Official`</sub> |
| [Tips de Telegram](https://tgbox.cc/en/detail/telegramtipses/) ✔️ | 1.5M | Telegram es sinónimo de libertad y privacidad y tiene muchas funciones fáciles de usar. <sub>`Free` `Official`</sub> |
| [Astuces Telegram](https://tgbox.cc/en/detail/telegramtipsfr/) ✔️ | 702.7K | Telegram est synonyme de liberté, de respect de la vie privée et offre de nombreuses… <sub>`Free` `Official`</sub> |
| [Dicas do Telegram](https://tgbox.cc/en/detail/telegramtipsbr/) ✔️ | 700K | Telegram é sinônimo de liberdade e privacidade, e tem várias funcionalidades fáceis de… <sub>`Free` `Official`</sub> |
| [قابلیت‌های تلگرام](https://tgbox.cc/en/detail/telegramtipsfa/) ✔️ | 589.3K | تلگرام متکی بر آزادی و حریم خصوصی بوده و ویژگی‌های کاربر پسند و متنوع بسیاری دارد. <sub>`Free` `Official`</sub> |
| [Tips Telegram](https://tgbox.cc/en/detail/telegramtipsid/) ✔️ | 451K | Telegram adalah sebuah ide tentang kebebasan dan privasi, memiliki banyak fitur yang… <sub>`Free` `Official`</sub> |
| [AyuGram Releases](https://tgbox.cc/en/detail/ayugramreleases/) | 355.8K | Official AyuGram builds channel for all platforms. @ayugram, @ayugramchat, @ayugramfaq <sub>`Open Source`</sub> |
| [Telegram APKs for Android](https://tgbox.cc/en/detail/tandroidapk/) ✔️ | 257.6K | Official channel for Telegram Android APKs. You can also download them here:… <sub>`Android` `Free` `Official`</sub> |
| [Telegram İpuçları](https://tgbox.cc/en/detail/telegramtipstr/) ✔️ | 228.5K | Telegram; özgürlük ve gizlilik anlamına gelir ve kullanımı kolay birçok özelliğe sahiptir. <sub>`Free` `Official`</sub> |
| [Suggerimenti di Telegram](https://tgbox.cc/en/detail/telegramtipsit/) ✔️ | 183.7K | Telegram è sinonimo di libertà e privacy e ha molte funzionalità facili da usare. <sub>`Free` `Official`</sub> |
| [ReVanced](https://tgbox.cc/en/detail/app_revanced/) | 116K | Continuing the legacy of Vanced at https://revanced.app ReVanced topics:… <sub>`Android` `Free` `Open Source`</sub> |
| [텔레그램 팁모음](https://tgbox.cc/en/detail/telegramtipsko/) ✔️ | 106.2K | 텔레그램은 자유와 보안을 추구하며, 쉬운 기능들을 제공하고 있습니다. <sub>`Free` `Official`</sub> |
| [Widget🏂软件工具资源分享](https://tgbox.cc/en/detail/widgetchannel/) | 99.9K | Daily push of useful and interesting content, including but not limited to: Software… <sub>`Free`</sub> |

<details>
<summary>Show 44 more</summary>

| Name | Subscribers | About |
| --- | ---: | --- |
| [exteraGram](https://tgbox.cc/en/detail/exteragram/) | 99.8K | Website: exteraGram.app APKs: @exteraReleases Beta: @exteraGramCI FAQ: @exteraFAQ Forum:… <sub>`Android`</sub> |
| [Telegram Designers](https://tgbox.cc/en/detail/designers/) ✔️ | 71.5K | Feature suggestions for Telegram from designers all over the world. If you want to… <sub>`Design` `Official`</sub> |
| [Newlearnerの自留地](https://tgbox.cc/en/detail/newlearnerchannel/) | 67.5K | Blog Groups & Postgroups & Communications Groups: @NewlearnerGroup Email… <sub>`Free`</sub> |
| [小声逼逼](https://tgbox.cc/en/detail/me888888888888/) | 50.4K | Treasure mining Appletv/iOS/Mac 🎁 Regular sweepstakes Exclusive news Emby video audio… <sub>`iOS`</sub> |
| [折腾啥](https://tgbox.cc/en/detail/zhetengsha/) | 49.5K | Power Users/Automators toss / discuss / share various open source tools / scripts /… <sub>`Open Source`</sub> |
| [Loon News](https://tgbox.cc/en/detail/loonnews/) | 47K | Loon News <sub>`iOS`</sub> |
| [Porady Telegrama](https://tgbox.cc/en/detail/telegramtipspl/) ✔️ | 45.2K | Telegram jest synonimem wolności i prywatności i ma wiele łatwych w użyciu funkcji. <sub>`Free` `Official`</sub> |
| [Aero Apps \| Channel](https://tgbox.cc/en/detail/aerolla/) | 44.9K | Aero Apps Official Telegram Channel. Web: www.whatsaero.com www.aeroinsta.com… <sub>`Android` `Free`</sub> |
| [Nekogram APKs](https://tgbox.cc/en/detail/nekogramapks/) | 41.5K | The official APKs channel for Nekogram. News: @NekoUpdates Discussion: @NekoChat Чат:… <sub>`Android` `Open Source`</sub> |
| [Cherrygram 🍒](https://tgbox.cc/en/detail/cherrygram/) | 41.4K | The official channel of the Cherrygram. Support and discussion: @CherrygramSupport APKs:… <sub>`Android` `Open Source`</sub> |
| [APPDO 数字生活指南](https://tgbox.cc/en/detail/appdodo/) | 40.4K | 💡 Internet/Digital/App/Hair/Camera/Digital Guide Business Contact:@appdo\_bot (only,… |
| [少数派sspai](https://tgbox.cc/en/detail/sspai/) | 38.2K | Official channel of the minority (sspai.com).Weibo: @ minority spai, public number:sspaime <sub>`iOS`</sub> |
| [Nekogram](https://tgbox.cc/en/detail/nekoupdates/) | 36.1K | The official channel of the Nekogram. Discussion: @NekoChat Чат: @NekoChatRU 讨论:… <sub>`Android` `Official` `Open Source`</sub> |
| [Appinn Feed｜小众软件](https://tgbox.cc/en/detail/appinnfeed/) | 33.5K | Here are content updates for popular software and discovery channels, as well as a… <sub>`Free`</sub> |
| [Panduan Telegram](https://tgbox.cc/en/detail/telegramtipsms/) ✔️ | 31.7K | Telegram memperjuangkan kebebasan dan privasi, dan mempunyai banyak ciri-ciri mudah… <sub>`Free` `Official`</sub> |
| [xManager - Updates](https://tgbox.cc/en/detail/xmanagerupdates/) | 26.3K | Official Channel for xManager by xC3FFF0E Website: https://xmanagerapp.com Reddit:… <sub>`Android` `Free` `Open Source`</sub> |
| [Ai一线 \| 软件 资讯 新闻](https://tgbox.cc/en/detail/followbun/) | 26.2K |  |
| [主频道](https://tgbox.cc/en/detail/swnbnb96/) | 23.5K | 本系列作品仅供学习研究，使用者的任何行为与应用作者无关，请在24小时内删除本应用 \[下载即代表同意本协议\] ↔️ 若有侵权违法行为，请联系我删除内容… <sub>`Android`</sub> |
| [Swiftgram](https://tgbox.cc/en/detail/swiftgram/) | 17.7K | Supercharged Telegram for iOS Download - https://apps.apple.com/app/id6471879502 Discuss… <sub>`iOS` `Open Source`</sub> |
| [Cool Scripts](https://tgbox.cc/en/detail/cool_scripts/) | 16.3K | QuanX, Loon, Surge, JsBox, Pythonista, Scriptable, Shortcuts and other script sharing <sub>`iOS`</sub> |
| [APPDO的互联网记忆](https://tgbox.cc/en/detail/appdopic/) | 16.2K | This channel is published by multiple editors and does not represent any personal views;… <sub>`Freebies` `iOS`</sub> |
| [Flippy\_Openwrt](https://tgbox.cc/en/detail/openwrt_flippy/) | 14.9K | Flippy's Openwrt/Armbian software Group link: https://t.me/joinchat/WTWXSt\_bMW0xHQZE If… <sub>`Linux` `Open Source`</sub> |
| [iBeta 尝鲜派｜公告栏](https://tgbox.cc/en/detail/ibeta_channel/) | 13.6K | iBeta official channel: iBeta.me Group: t.me/ibetame <sub>`iOS`</sub> |
| [极客分享 2.0](https://tgbox.cc/en/detail/xgeekshare/) | 13.5K | Extreme Sharing 2.0 Channel Focus on sharing AI, open source projects, productivity… <sub>`Open Source`</sub> |
| [KoolCenter Merlin Firmware](https://tgbox.cc/en/detail/ks_merlin/) | 13.1K | 信息发布 <sub>`Official`</sub> |
| [Leeco-雪花社 ❄️](https://tgbox.cc/en/detail/xuehuashe/) | 12.3K | Make more snowflakes \~ Share practical quick instructions, some scripts <sub>`iOS`</sub> |
| [Hell Cell 功能教学](https://tgbox.cc/en/detail/hellcellzc123/) | 10.8K | YouTube Video Teaching Channel Business Contact @Tartarus2014 Through a video… <sub>`Free`</sub> |
| [TestFlight News](https://tgbox.cc/en/detail/testflights/) | 10.7K | Collect the TestFlight application address for sharing iOS with relevant information and… <sub>`iOS`</sub> |
| [ALL About RSS](https://tgbox.cc/en/detail/aboutrss/) | 9.5K | About \#RSS technology \#application \# surroundings \# introduction \# method \# tutorial \#… |
| [Forkgram](https://tgbox.cc/en/detail/forkgram/) | 8.9K | <sub>`Open Source`</sub> |
| [Telegram Web A](https://tgbox.cc/en/detail/webachannel/) ✔️ | 8.2K | Story of the official Telegram Web app web.telegram.org/a. <sub>`Official`</sub> |
| [互联网研习社（破解软件/游戏 \| 实测）](https://tgbox.cc/en/detail/ruanjianfenxiang77/) | 8K | 分享互联网实用技巧、实用软件、破解软件、有趣网站、实用工具，所有软件游戏都经过实测。 Telegram知识库：https://tgnav.github.io/tgwiki/comm… <sub>`Android` `Windows`</sub> |
| [Office Tool Plus 📰](https://tgbox.cc/en/detail/otp_channel/) | 7.1K | Office Tool Plus 的官方频道。 Official channel of Office Tool Plus. 频道默认使用静默模式发布消息，只有紧急事情才会发布通知。… <sub>`Free` `Windows`</sub> |
| [Cherrygram APKs](https://tgbox.cc/en/detail/cherrygramapks/) | 6.3K | Official APKs channel for Cherrygram. News: @Cherrygram Support and discussion:… <sub>`Android` `Open Source`</sub> |
| [OctoGram \| News](https://tgbox.cc/en/detail/octogramapp/) | 6.1K | 🕯 OctoGram is an unofficial messaging app that uses Telegram's API. 💭 Group:… <sub>`Android`</sub> |
| [简悦 - SimpRead 📢](https://tgbox.cc/en/detail/simpread/) | 5.7K | Easy-to-read channel, easy-to-read - lets you instantly enter the immersive reading… |
| [Fedora News](https://tgbox.cc/en/detail/fedoranews/) | 5.5K | This channel is for news relating to the Fedora Project (getfedora.org). <sub>`Linux` `Official`</sub> |
| [WWWeng🐝の分享](https://tgbox.cc/en/detail/wwwengshare/) | 5.1K | Mainly to share some shortcuts or other utilities must see:https://t.me/wwwengshare/192 <sub>`iOS`</sub> |
| [Cherrygram Beta](https://tgbox.cc/en/detail/cherrygrambetaapks/) | 4.3K | Beta APKs channel for Cherrygram. APKs in this channel contain pre-release features, but… <sub>`Android` `Open Source`</sub> |
| [Nekogram Test APKs](https://tgbox.cc/en/detail/nekotestapks/) | 3.5K | Official TEST APKs channel for Nekogram. APKs in this channel contain pre-release… <sub>`Android` `Open Source`</sub> |
| [AppPie](https://tgbox.cc/en/detail/apppie/) | 3.5K | Welcome to AppPie!We are eager to explore the latest developments in the Apple world and… <sub>`iOS`</sub> |
| [theBlock](https://tgbox.cc/en/detail/theblockclub/) | 2.6K | Love from Notion fans, @iceyaya, @niinjoy & @craigary |
| [AdGuard消息](https://tgbox.cc/en/detail/adguardcn/) | 2.4K | You can get all the news about ad blocking and cybersecurity here. AdGuard official… <sub>`Security`</sub> |
| [materialgram](https://tgbox.cc/en/detail/materialgram/) | 1.9K | Telegram Desktop fork with material icons and some improvements… <sub>`Open Source`</sub> |

</details>

[All software on TGbox →](https://tgbox.cc/en/channel/software/)

<a id="channel-crypto"></a>

### Crypto & Web3

| Name | Subscribers | About |
| --- | ---: | --- |
| [Notcoin Community](https://tgbox.cc/en/detail/notcoin/) ✔️ | 8.6M | notcoin.org <sub>`TON`</sub> |
| [Binance Announcements](https://tgbox.cc/en/detail/binance_announcements/) ✔️ | 3.4M | Binance Official English Group @binanceexchange Binance Official Chinese Group… <sub>`Exchanges`</sub> |
| [Cointelegraph](https://tgbox.cc/en/detail/cointelegraph/) | 334.4K | The Future of Money Unbiased crypto, markets & Web3 news in real-time. Trusted since… <sub>`Finance`</sub> |
| [Whale Alert](https://tgbox.cc/en/detail/whale_alert_io/) | 300.7K | The most advanced \#blockchain tracker and analytics system reporting large and… <sub>`On-chain Data`</sub> |
| [CoinMarketCap Announcements](https://tgbox.cc/en/detail/coinmarketcapannouncements/) ✔️ | 246K | Official CMC Telegram Channel: Updates, News, and Events! Build with CMC (API, AI,… |
| [Wu Blockchain News](https://tgbox.cc/en/detail/wublockchainenglish/) | 232.9K | WuBlockchain’s only official Telegram account Colin Wu, Reporter Important Crypto News… <sub>`Finance`</sub> |
| [TON Status](https://tgbox.cc/en/detail/tonstatus/) ✔️ | 158.4K | Technical notifications and up-to-date requests for action for TON validators, developers… <sub>`TON`</sub> |
| [方程式新闻 BWEnews](https://tgbox.cc/en/detail/bwenews/) | 72.7K | The fastest&alpha only crypto news media, we strive to make high quality information… |
| [欧易OKX公告](https://tgbox.cc/en/detail/okxannouncements_cn/) ✔️ | 45K | 关注欧易OKX中文公告频道，及时把握平台动态！注意某些产品服务或推广活动不适用于部分地区。 认准本频道为唯一指定官方公告频道：… <sub>`Exchanges` `Official`</sub> |
| [Glassnode](https://tgbox.cc/en/detail/glassnode/) | 44.2K | Institutional Data and Market Intelligence for Digital Assets. https://studio.glassnode.co… <sub>`On-chain Data` `Quant Trading`</sub> |
| [BlockBeats](https://tgbox.cc/en/detail/theblockbeats/) | 37.5K | BlockBeats – a professional blockchain research agency and information platform.… <sub>`Finance`</sub> |
| [币安公告](https://tgbox.cc/en/detail/binance_cn/) | 34.1K | Pay attention to Binance's Chinese announcement channel, you can catch up with the… <sub>`Exchanges`</sub> |
| [大宇投资频道](https://tgbox.cc/en/detail/btcdayu2/) | 30.1K | Updated daily must-read articles and personal investment views, discussion groups are… |
| [The Block News Feed](https://tgbox.cc/en/detail/the_block_crypto/) | 25K | The Block's official Telegram feed for news articles. <sub>`Finance`</sub> |
| [PANews Web3丨加密货币丨区块链新闻频道](https://tgbox.cc/en/detail/channelpanews/) | 23.9K | Your Web3 Information Officer, provide crypto first-line emails and in-depth reports.… |

<details>
<summary>Show 27 more</summary>

| Name | Subscribers | About |
| --- | ---: | --- |
| [金色财经新闻频道](https://tgbox.cc/en/detail/jinse2017/) | 22.8K | Gold Finance is a one-stop blockchain industry service platform that gathers industry… <sub>`Finance`</sub> |
| [Odaily资讯速递](https://tgbox.cc/en/detail/odaily_news/) | 20.6K | HTTPS://www.odaily.news/ APP Download: HTTPS://download.odaily.com Official X:… <sub>`Finance`</sub> |
| [TechFlow 深潮 ｜ 加密货币&区块链新闻](https://tgbox.cc/en/detail/techflowdaily/) | 18.7K | Sharing valuable information. |
| [Lookonchain](https://tgbox.cc/en/detail/lookonchainchannel/) | 18.7K | Please contact @lookonchainsupport if you have any questions. We will never DM you first!… <sub>`On-chain Data`</sub> |
| [Foresight News](https://tgbox.cc/en/detail/foresightnews/) | 18K | Web 3.0 & Blockchain Age Global Information Frontline, "Meeting" the Future. APP Download… <sub>`Finance`</sub> |
| [RootData](https://tgbox.cc/en/detail/rootdatalabs/) | 17.6K | Daily update Rootdata project database status, including new projects, new fundraising,… |
| [吴说区块链 新闻与深度](https://tgbox.cc/en/detail/wublock/) | 15.6K | Wu said: Close to the truth, more reliable Blockchain news reports, deep content analysis… <sub>`Finance`</sub> |
| [币圈动态](https://tgbox.cc/en/detail/biquan321/) | 11.5K | We all own any cryptocurrency! hot spot sharing as there is an infringement message… |
| [Wizz Radar](https://tgbox.cc/en/detail/groupdigest/) | 11.5K | AI Selected Group Chat Summary (Content from multiple Chinese-English crypto communities) |
| [6551News](https://tgbox.cc/en/detail/news6551/) | 11.5K | The Degen News 币圈接地气的新闻推送https://x.com/6551News |
| [老陆的区块链笔记 LaoLu's Blockchain Notes](https://tgbox.cc/en/detail/laolunote/) | 11K | Info & Research in: blockchain projects. MESSAGES IN THIS CHANNEL DO NOT CONSTITUTE ANY… |
| [链捕手 ChainCatcher](https://tgbox.cc/en/detail/chaincatcher/) | 10K | Mastering the depth and cutting-edge blockchain brand media. <sub>`Finance`</sub> |
| [实话频道](https://tgbox.cc/en/detail/dollcall/) | 9.6K | In two years, there has never been a continuous truth export of evil debt a little bit… |
| [Crypto Market Aggregator\|币圈新闻汇总](https://tgbox.cc/en/detail/cryptomarketaggregator/) | 9K | AI automatic summary news, Beijing time 8 p.m., 16 p.m., 0 p.m., content is AI based on… |
| [PolyBeats](https://tgbox.cc/en/detail/polybeats_bot/) | 8.3K | Follow @PolyBeatsEN \#PredictionMarket \#Crypto \#Polymarket |
| [aggrnews](https://tgbox.cc/en/detail/aggrnewswire/) | 8.1K | Delivering the latest and most significant updates from your favorite news outlets.… <sub>`Finance`</sub> |
| [Ai 姨](https://tgbox.cc/en/detail/ai_9684xtpa/) | 7.8K | \#crypto is the antidote of APTX4869 DYOR \#DeFi \#NFT \#BTC \#Binance automatically tweet to… |
| [Fragment Monitor 👁‍🗨](https://tgbox.cc/en/detail/fragment_monitor/) | 6.6K | Fragment Monitor is a Fragment event notification channel that sends notifications to the… <sub>`TON`</sub> |
| [Alex 投资备忘](https://tgbox.cc/en/detail/alex_xu_investment/) | 6.4K | Share good content related to Web3 and investment, as well as thinking about content,… <sub>`Finance`</sub> |
| [币圈快讯](https://tgbox.cc/en/detail/btcnewsdaily/) | 4.9K | 币圈快讯，掌握币圈动态 https://open.spotify.com/show/324jsumOEHiYz2mmpmGvYM?si=02d2d6dfcb564ca8 |
| [PANews 编辑部Web3资讯严选](https://tgbox.cc/en/detail/panewsselected/) | 4.3K | Are you tired of the endless information bombardment? don't want to be flooded with… |
| [區塊先生 📕圖書館📘](https://tgbox.cc/en/detail/mrblock_info/) | 4.3K | Various alphas/news/market trends/tools Share the various types of information you see on… |
| [0x Meme/项目/空投](https://tgbox.cc/en/detail/newsforbitcoin/) | 3.6K | \* Regularly send Web3 Air Investments/Projects/Games/Roll/NFT \*\* Daily Push Web3 Content… <sub>`Airdrops`</sub> |
| [币圈日报](https://tgbox.cc/en/detail/bidaily/) | 3.5K | Currency Circle Daily - the leading currency circle news every day <sub>`Daily Digest`</sub> |
| [撸空投💰币圈💰Web3💰财务自由](https://tgbox.cc/en/detail/tglukongtou/) | 3.3K | Share the latest cutting-edge investments and move toward financial freedom <sub>`Airdrops`</sub> |
| [Bitget官方公告](https://tgbox.cc/en/detail/bitgetcn_announcement/) | 3.3K | Bitget overseas Chinese language community https://t.me/Bitget\_CNOfficial Bitget official… <sub>`Exchanges` `Official`</sub> |
| [CCXT Announcements](https://tgbox.cc/en/detail/ccxt_announcements/) | 839 | CryptoCurrency eXchange Trading Library Important Announcements <sub>`Open Source` `Quant Trading`</sub> |

</details>

[All crypto & web3 on TGbox →](https://tgbox.cc/en/channel/crypto/)

<a id="channel-tech"></a>

### Development

| Name | Subscribers | About |
| --- | ---: | --- |
| [Telegram Contests](https://tgbox.cc/en/detail/contest/) ✔️ | 299.1K | Here we announce Telegram coding contests in Android Java, iOS Swift, JS, C/C++.… <sub>`Official` `Programming`</sub> |
| [BotNews](https://tgbox.cc/en/detail/botnews/) ✔️ | 131.6K | The official source for news about the Telegram Bot API. https://core.telegram.org/bots <sub>`Official` `Programming`</sub> |
| [Linuxgram 🐧](https://tgbox.cc/en/detail/linuxgram/) | 72.7K | News and info from the Linux world 🐧 📨 linuxgr4m@gmail.com 📨 💸 If you want to support… <sub>`Linux`</sub> |
| [Demo Store](https://tgbox.cc/en/detail/teststore/) | 69.4K | This test store features the most exotic imaginary goods no money can buy, so you can… <sub>`Official` `Programming`</sub> |
| [VPS信号旗播报](https://tgbox.cc/en/detail/vps_xhq/) | 49.7K | VPS News Channel, VPS Signal Flag is an instant messaging small organization, focusing on… <sub>`Hosting & VPS`</sub> |
| [The Devs](https://tgbox.cc/en/detail/thedevs/) | 32.1K | Developers community on Telegram. https://thedevs.network Jobs: @thehire Have something… <sub>`Programming`</sub> |
| [互联网从业者充电站](https://tgbox.cc/en/detail/https1024/) | 30.8K | Exclusively for Internet practitioners Content is broadly related to different topics… <sub>`Programming`</sub> |
| [Hacker News](https://tgbox.cc/en/detail/hacker_news_feed/) | 29.7K | Top stories from https://news.ycombinator.com (with 100+ score) Contribute to the… <sub>`Programming`</sub> |
| [GitHub repos](https://tgbox.cc/en/detail/github_repos/) | 27.3K | Welcome to GitHub repos. Here you'll find valuable information on the latest trending… <sub>`Open Source`</sub> |
| [Front End World](https://tgbox.cc/en/detail/front_end_first/) | 25.9K | New articles about Front End development. JS, CSS, React, Vue, Angular and others!… <sub>`Programming`</sub> |
| [如有乐享](https://tgbox.cc/en/detail/ruyoblog/) | 23.9K | Main updates such as blog content, and a few other friends' channels dry. Cloud servers,… <sub>`Hosting & VPS`</sub> |
| [Bug Bounty Channel](https://tgbox.cc/en/detail/bug_bounty_channel/) | 14.8K | All bug bounties here. <sub>`Security`</sub> |
| [Android Developers](https://tgbox.cc/en/detail/androiddeveu/) | 14.7K | Welcome to the community for Android developers of all levels! Here, we share the latest… <sub>`Android` `Programming`</sub> |
| [科技爱好者周刊](https://tgbox.cc/en/detail/scitech_fans/) | 14.6K | Record weekly technical content worth sharing, released on Friday. This channel is… <sub>`Programming`</sub> |
| [RSSHub News](https://tgbox.cc/en/detail/awesomersshub/) | 12.2K | 🍰 Everything is RSSible This channel will include: - \#RSSHubCore updates - \#RSSHub route… <sub>`Open Source`</sub> |

<details>
<summary>Show 16 more</summary>

| Name | Subscribers | About |
| --- | ---: | --- |
| [🥳靠谱VPS推荐(默认带aff） - VPS仓/古博](https://tgbox.cc/en/detail/vpscang/) | 11.8K | VPS Recommendations, VPS Storage Supplements, etc. All information is provided by default… <sub>`Hosting & VPS`</sub> |
| [GitHub Trends](https://tgbox.cc/en/detail/githubtrending/) | 11.6K | See what the GitHub community is most excited about today. A bot automatically fetches… <sub>`Open Source` `Programming`</sub> |
| [Arch Linux Chinese Messages](https://tgbox.cc/en/detail/archlinuxcn/) | 11.2K | Arch Linux related information Follow Arch Linux’s Daily Track Alerts, Translate Official… <sub>`Linux` `Open Source`</sub> |
| [主机百科资讯分享](https://tgbox.cc/en/detail/zhujiwiki_info/) | 10.7K | Hostage Encyclopedia: Hostage Encyclopedia: https://zhujiwiki.com Posts/advertisements… <sub>`Hosting & VPS`</sub> |
| [信息安全技术频道🔎](https://tgbox.cc/en/detail/tg_internetsecurity/) | 10.6K | <sub>`Security`</sub> |
| [层叠 - The Cascading](https://tgbox.cc/en/detail/outvivid/) | 9K | The main topic is probably open source industry news. All content tagged with \#today is a… <sub>`Open Source`</sub> |
| [Instant View Contest News](https://tgbox.cc/en/detail/ivcontest/) ✔️ | 8.8K | The official channel for news about Telegram's Instant View Template Competition. <sub>`Official` `Programming`</sub> |
| [JIKE社区](https://tgbox.cc/en/detail/jikeinfo/) | 6.2K |  |
| [AWS Notes](https://tgbox.cc/en/detail/aws_notes/) | 5.8K | AWS Notes — Amazon Web Services Educational and Information Channel Chat:… <sub>`Programming`</sub> |
| [iGitHub](https://tgbox.cc/en/detail/igithub/) | 5.6K | Share some applicable GitHub open source projects Share more @lihaiba <sub>`Open Source`</sub> |
| [CatOps](https://tgbox.cc/en/detail/catops/) | 5.1K | DevOps and other issues by Yurii Rochniak (@grem1in) - SRE @ Preply && Maksym Vlasov… <sub>`Linux`</sub> |
| [Python潮流周刊🐍](https://tgbox.cc/en/detail/pythontrendingweekly/) | 4.5K | Filter from 250+ sources at home and abroad to select the most worth sharing articles,… <sub>`Open Source` `Programming`</sub> |
| [Sysadmin Tools 🇺🇦](https://tgbox.cc/en/detail/sysadmin_tools/) | 4.2K | Sysadmin/DevOps tools, news and other interesting things from modern IT world. Feed… <sub>`Linux`</sub> |
| [Hacker News 摘要](https://tgbox.cc/en/detail/hacker_news_zh/) | 2.3K | Automatically forward Hacker News homepage entries with a score of 150 using AI to… <sub>`Programming`</sub> |
| [开发者日报](https://tgbox.cc/en/detail/cndevdaily/) | 2K | A hardworking robot, just climbing hot, high quality, and reading strong content. The… <sub>`Daily Digest` `Programming`</sub> |
| [Hostloc 新帖图文推送](https://tgbox.cc/en/detail/hostlocpro/) | 1.7K | Basic features of this channel: Get the most famous host forums at home and abroad… <sub>`Linux`</sub> |

</details>

[All development on TGbox →](https://tgbox.cc/en/channel/tech/)

<a id="channel-resources"></a>

### Resources

| Name | Subscribers | About |
| --- | ---: | --- |
| [ahhhhfs｜A姐分享](https://tgbox.cc/en/detail/abskoop/) | 306.3K | A姐分享订阅频道 分享资源、影视、软件和福利等有趣的东西。 所有内容均发布在：https://www.ahhhhfs.com 影视资源：https://www.xbwpys.com… <sub>`Free`</sub> |
| [APP喵 - 破解软件资源](https://tgbox.cc/en/detail/appmew/) | 166.4K | APP喵资源仓库。发现分享好用 APP 软件，优质 资源，游戏，网站，极客，教程，各种 网络 黑科技，节点，免费VPN，影视 等… <sub>`Free`</sub> |
| [Notification Sounds](https://tgbox.cc/en/detail/customnotificationsounds/) | 76.4K | <sub>`Free` `Official`</sub> |
| [曹老板白嫖分享社](https://tgbox.cc/en/detail/clbfxs/) | 76K | 资源/副业/网赚/知识/课程/软件/网课/工具/精选/AI/源码/游戏/区块链/WEB3/MacOS/VR 商务/反馈/投稿 @BiaoPiaoZY\_bot… <sub>`Free`</sub> |
| [极客分享](https://tgbox.cc/en/detail/geekshare/) | 64.7K | Focus on sharing a variety of high-quality websites, tools, apps, open-source projects,… <sub>`Free` `Open Source`</sub> |
| [好软分享](https://tgbox.cc/en/detail/haoruanfenxiang/) | 64.4K | 好软分享·主频道： @haoruanfenxiang 好软分享·列表频道： @haoruanfenxianglist 好软分享·资源频道：… <sub>`Free`</sub> |
| [飞鱼资源分享\|冲浪指南\|软件工具](https://tgbox.cc/en/detail/feiyu123/) | 62.2K | Share some tools, software, browser plug-in scripts, websites, songs, Internet-related… <sub>`Free`</sub> |
| [链接收藏](https://tgbox.cc/en/detail/ldlist/) | 42.2K | 链接收藏 网址导航 评论区规则，包含并不仅限 1、封禁【博彩/赌毒/宗教/政治/键政/黑产/灰产/违法色情/暴力】 2、删除+警告【Aff/隐私】… |
| [LIHAI 分享](https://tgbox.cc/en/detail/lihaiba/) | 39.1K | A shared channel, in the channel first look at the top, from time to time unquantitative… |
| [Zapro Notice](https://tgbox.cc/en/detail/zaproshare/) | 29.1K | Welcome to the discussion group: https://t.me/zaprobest <sub>`Free`</sub> |
| [优质信息收藏夹](https://tgbox.cc/en/detail/chunse1024/) | 25.7K | Share high-quality content/information/software/plugins/websites, etc. Share fun, useful… |
| [Odyssey+](https://tgbox.cc/en/detail/odysseyplus/) | 24.9K | Eating guide:https://odysseyplus.notion.site Resource push:t.me/odysseyfeed Channel… |
| [好软分享·资源频道](https://tgbox.cc/en/detail/haoruanfenxiangresources/) | 22.8K | 好软分享·主频道： @haoruanfenxiang 好软分享·列表频道： @haoruanfenxianglist 好软分享·资源频道：… <sub>`Free`</sub> |
| [书墨资源](https://tgbox.cc/en/detail/shumozy/) | 20.8K | 书墨资源丨学习资料、各类软件、生活百科、世间风味，享你所想 频道指南 t.me/shumozy/7 频道索引 @shumoindex 安卓 @shumoandroid1 音乐… |
| [那些好看的Tg主题](https://tgbox.cc/en/detail/beautifultgtheme/) | 12.9K | 分享一些好看的Telegram主题 大部分是Android,官方 如果你需要TgX、IOS、或PC桌面版的主题，请联系我，我尽量做… <sub>`Design`</sub> |

<details>
<summary>Show 11 more</summary>

| Name | Subscribers | About |
| --- | ---: | --- |
| [拾趣](https://tgbox.cc/en/detail/peekfun/) | 12.9K | Share something interesting and useful. |
| [优质「资源」收藏夹](https://tgbox.cc/en/detail/meiriyishu/) | 12.2K | 收藏高质量资料，帮助你获取优质资源。 |
| [鲲的碎碎念](https://tgbox.cc/en/detail/atashare/) | 7.5K | Share useful, massive network resources and information. @YuanShen ATA Bots @atabots 🎉… |
| [🔍此岸情报局🔎](https://tgbox.cc/en/detail/justreformation/) | 7.5K | 资源情报资讯更新板！ 羊毛、好物、聊天、VIP、软件、Apple、淘宝、京东、福利、App、iOS、老司机、破解、小说、搞笑、资源、视频、中文、Google！源于网络，分享世界！… <sub>`Free`</sub> |
| [Odyssey+ Feed](https://tgbox.cc/en/detail/odysseyfeed/) | 4.7K | Odyssey+ 资源推送频道 |
| [好软分享·列表频道](https://tgbox.cc/en/detail/haoruanfenxianglist/) | 4.5K | 👇列表清单👇 ⭐️ https://t.me/haoruanfenxianglist/6 ⭐️ 👆列表清单👆 好软分享·主频道： @haoruanfenxiang… <sub>`Free`</sub> |
| [米哈的 Telegram 工具箱](https://tgbox.cc/en/detail/tgbotdevelopment/) | 4.2K | Telegram Tools/Bots Support/News Channel ID: -1002097234024 Chat link:… <sub>`Open Source`</sub> |
| [Telegram中文语言包](https://tgbox.cc/en/detail/go2cn/) | 3.4K | 一键替换中文语言 <sub>`Free`</sub> |
| [资源笔记](https://tgbox.cc/en/detail/tieliu/) | 1.2K | 转频道 @piracy6 <sub>`Free`</sub> |
| [TG我们来分享](https://tgbox.cc/en/detail/bdpd8/) | 448 | @bdpd8 苹果/安卓/黑科技/白嫖/节点/梯子/翻墙/vpn/代理/加速器/apk/ipa/TF/限免/教程/破解/软件/资源/网站 😘免费资源共享… <sub>`Free`</sub> |
| [素材库｜电报素材｜日常素材｜素材大师](https://tgbox.cc/en/detail/sucaidashi/) | 289 | 收集全网适用素材！🤝 常见与不常见素材！🤝 我们只收集非原创！🤝 推送可能不合时宜！🤝 您可以选择性使用！🤝 感谢有您鼎力支持！🤝 分类清晰方便查找！🤝… <sub>`Design`</sub> |

</details>

[All resources on TGbox →](https://tgbox.cc/en/channel/resources/)

<a id="channel-blog"></a>

### Blogs

| Name | Subscribers | About |
| --- | ---: | --- |
| [Pavel Durov](https://tgbox.cc/en/detail/durov/) ✔️ | 10.6M | Founder of Telegram. <sub>`Official`</sub> |
| [即刻精选](https://tgbox.cc/en/detail/jike_collection/) | 35.8K | X:x.com/jike\_collection Created by @i5tar No ads |
| [秋风のとおり道](https://tgbox.cc/en/detail/awavenue/) | 26K | A year-on-year fall of the Chase channel meet, observe, think, learn, perceive.… |
| [不靠谱的喵(\>^ω^\<) \#CatGPT](https://tgbox.cc/en/detail/mlgmxyysd_bibilailai/) | 21.6K | This channel is not a professional media and cannot fully guarantee the timeliness and… |
| [🍟 整点薯条: 频道](https://tgbox.cc/en/detail/getsomefrieschannel/) | 20.1K |  |
| [晚来天欲雪 🅥 🌈🌝🐳🏳️‍🌈 limbopro.com 🔗](https://tgbox.cc/en/detail/limboprossr/) | 18K | Follow the channel, get up, and don’t miss the link. Newcomers must watch… |
| [橘橘橘子汁 & 🍊](https://tgbox.cc/en/detail/microblock_pub/) | 15.3K | This channel does not make any guarantees for the authenticity of the message, true and… |
| [小道消息](https://tgbox.cc/en/detail/webnoteslah/) | 15.2K | Invisible street, street message https://x.com/Fenng |
| [Leonn的博客](https://tgbox.cc/en/detail/liyuans/) | 14.3K | The main content is low-cost hosting resources, which will blend the channel’s main… <sub>`Hosting & VPS`</sub> |
| [Find Blog👁发现博客](https://tgbox.cc/en/detail/findblog/) | 13.9K | Discover great blogs with creators Discover blogs, articles filtered, and blog related… |
| [404 KIDS SEE GHOSTS (生产力之王版](https://tgbox.cc/en/detail/isaiahsystem/) | 13.6K | Group: @ghosttown666 Contact: @madebyblackstack |
| [caoz的梦呓](https://tgbox.cc/en/detail/caozsay/) | 13.4K | Caoz's Nightmare Telephone Channel |
| [《一天世界》博客](https://tgbox.cc/en/detail/yitianshijie/) | 6.2K | Blog address: https://blog.yitianshijie.net |
| [为也行](https://tgbox.cc/en/detail/weiyexing/) | 5.6K | According to incomplete statistics, channel content is mainly: 40%: books, movies,… |
| [bupt.moe](https://tgbox.cc/en/detail/bupt_moe/) | 5.1K | Struggle for the development and liberation of all mankind! hrx@bupt.moe Original content… |

<details>
<summary>Show 9 more</summary>

| Name | Subscribers | About |
| --- | ---: | --- |
| [十三月评论](https://tgbox.cc/en/detail/elevemberreview/) | 4.5K | Pushing content based on the history of the republic and (wide) political economics,… |
| [&'a ::rynco::UntitledChannel](https://tgbox.cc/en/detail/rynif/) | 4.5K | This channel is: - a place to push anything interesting - a place to occasionally watch… |
| [啰哩啰嗦分享频道](https://tgbox.cc/en/detail/notonlyshare/) | 4.4K | A restrained, rational channel that focuses on Internet technology and humanities, also… |
| [一起偷偷观察 DIYgod](https://tgbox.cc/en/detail/awesomediygod/) | 4.1K | Channel strongly driven by RSSHub, channel directory: \#DIYgod's Twitter Update \#DIYgod's… |
| [Nanase的随记](https://tgbox.cc/en/detail/nanaselog/) | 4.1K | 投稿/ VPS送测：@p06y7 (机场类不做) 测评站: review.nxtrace.org From the river to the sea, 🇵🇸Palestine… <sub>`Hosting & VPS`</sub> |
| [Milkice's 生而喵喵](https://tgbox.cc/en/detail/milkice_portal/) | 3.2K | @milkice personal channel audio tour / technology / daily try not to forward news from… |
| [鸥 Billchan 鸥 🍟 大薯 \| 去码头整点薯条](https://tgbox.cc/en/detail/billchenla/) | 3.2K | Gugugu Peing https://peing.net/zh-CN/billchenchina Lyrics https://space.bilibili.com/76856… |
| [K4YT3X's Channel](https://tgbox.cc/en/detail/k3yt4x/) | 2.3K | @k4yt3x's personal channel All comments will burn themselves in three months |
| [Cat Ch.](https://tgbox.cc/en/detail/usr_bin_cat/) | 2.1K | Unreasonable Unobjective Unneutral Think about what to write, soap. This channel contains… |

</details>

[All blogs on TGbox →](https://tgbox.cc/en/channel/blog/)

<a id="channel-vpn"></a>

### VPN & Proxy

| Name | Subscribers | About |
| --- | ---: | --- |
| [科学上网观察与机场测速](https://tgbox.cc/en/detail/jichangtj/) | 134.1K | \#机场推荐 \#机场测速 \#机场评测 /观察/点评，科学上网和科技相关新闻资讯； \#机场推荐 : http://bit.ly/3fdHzbp Twitter:… |
| [Hiddify](https://tgbox.cc/en/detail/hiddify/) | 122.1K | Hiddify, Internet Freedom Solution Supporting: 🎯 xHTTP 🎯 HTTPUpgrade 🎯 Shadowsocks 2022 🎯… <sub>`Official` `Open Source`</sub> |
| [前女友们用过的机场—始于机场，不止机场](https://tgbox.cc/en/detail/gebaopicloud/) | 101.1K | 投稿： exairport2022@gmail.com 机场主投稿请按如下格式 机场名： 机场官网： 机场频道： 机场群组： 机场套餐： 机场订阅： 机场特色：（可选）… |
| [Clash Verge Rev Channel](https://tgbox.cc/en/detail/clash_verge_re/) | 80.5K | Clash Verge Rev official announcement channel <sub>`Official` `Open Source`</sub> |
| [Surge & Loon & QX 脚本收集](https://tgbox.cc/en/detail/nobyda/) | 63.4K | Group discussion: @NobyDa\_Chat script feedback: @NobyDa\_bot Collaboration Contact:… <sub>`iOS`</sub> |
| [FlClash Channel](https://tgbox.cc/en/detail/flclash/) | 47K | The official channel of the FlClash github: https://github.com/chen08209/FlClash group:… <sub>`Official` `Open Source`</sub> |
| [Shadowrocket News](https://tgbox.cc/en/detail/shadowrocketnews/) | 46.7K | Shadowrocket 只有iOS/iPadOS/tvOS 版本. 讨论群组: @ShadowrocketApp App Store下载链接:… <sub>`iOS`</sub> |
| [Quantumult X News](https://tgbox.cc/en/detail/quanxnews/) | 41.1K | 此频道用于发布 Quantumult、Quantumult X、以及相关资讯 下载: https://apps.apple.com/us/app/quantumult-x/id14… <sub>`iOS`</sub> |
| [Project X Channel](https://tgbox.cc/en/detail/projectxtls/) | 29.9K | Donation: https://github.com/XTLS/Xray-core/issues/3668 Chinese Group:… <sub>`Open Source`</sub> |
| [Stash Channel](https://tgbox.cc/en/detail/stashfeed/) | 27.1K | Stash information official release channel. Stash is the best choice for Clash rules on… <sub>`iOS` `Official`</sub> |
| [Geph announcements \| 迷雾通通知](https://tgbox.cc/en/detail/gephannounce/) | 21.9K | Chat: https://t.me/gephusers <sub>`Official`</sub> |
| [Clash for Windows 汉化公告板](https://tgbox.cc/en/detail/clashr_for_windows_channel/) | 19.7K | Discussion Group:https://t.me/+Se4RSc06w8QK1HiS Clash of Clans Project… <sub>`Windows`</sub> |
| [ShellCrash发布](https://tgbox.cc/en/detail/shellclash/) | 17.7K | Support for all types of Linux/OpenWrt environments <sub>`Linux` `Open Source`</sub> |
| [mihomo ｜ notification](https://tgbox.cc/en/detail/clashmeta/) | 16.9K | <sub>`Official` `Open Source`</sub> |
| [Clash-Party channel](https://tgbox.cc/en/detail/mihomopartychannel/) | 14.9K | <sub>`Open Source`</sub> |

<details>
<summary>Show 5 more</summary>

| Name | Subscribers | About |
| --- | ---: | --- |
| [mini](https://tgbox.cc/en/detail/orzmini/) | 14.6K | Discussion Group https://t.me/Orz\_mini Project address https://github.com/Orz-3/mini mini… <sub>`Open Source`</sub> |
| [Hysteria Releases](https://tgbox.cc/en/detail/hysteria_releases/) | 9.9K | https://hysteria.network/ The official channel for Hysteria, a powerful, lightning fast… <sub>`Official` `Open Source`</sub> |
| [机场云 测速☁️频道](https://tgbox.cc/en/detail/yunspeedtest/) | 9K | 本频道关联群组: @yunspeedtestg 申请测速/机场投稿/广告投放 请联系 @yunspeedtest\_bot |
| [Clash Verge-官方](https://tgbox.cc/en/detail/clashvergerev/) | 8.3K | 🌐 Clash Verge Rev 特点 🌐多平台支持：支持常见的桌面端，包括 Windows、macOS 以及 Linux 🌐代理功能强大：使用最新Clash… <sub>`Official` `Open Source`</sub> |
| [机场情报(新) 🅥 🌈🌝🐳🏳️‍🌈 limbopro.com 🔗](https://tgbox.cc/en/detail/airportbbq1/) | 4.8K | 电报联系🤖 @limboprobot 邮件联系📧 service.limbopro.com@gmail.com 机场推荐 https://bit.ly/3JXb1yx… |

</details>

[All vpn & proxy on TGbox →](https://tgbox.cc/en/channel/vpn/)

<a id="channel-deals"></a>

### Deals

| Name | Subscribers | About |
| --- | ---: | --- |
| [\[合租通知\] Netflix\|YouTube\|Spotify\|office365…](https://tgbox.cc/en/detail/hezu2/) | 100.4K | 好物、街拍、软件、Apple、EXE、小说、搞笑、资源、视频、中文、Google、返利、二次元、自拍、机场、鉴、羊王、光棍协会、weibo、合租、软路由 <sub>`Freebies`</sub> |
| [🎏「 彼岸情报🔎！」🎏薅羊毛情报（网站/资源/软件/限免APP）见闻社](https://tgbox.cc/en/detail/baccanosoul/) | 25.5K | Advertising / Promotion / Collaboration: @BunnyJump\_Bot All-Platform App Limited /… <sub>`Freebies`</sub> |
| [你有一个打折需要了解](https://tgbox.cc/en/detail/steamny/) | 16.2K | Known: + 24 Hours (GMT/UTC+8) (0:00 - 23:59) + Meaning Switching within the T channel… <sub>`Freebies`</sub> |
| [频道🥇福利资源 \| 薅羊毛·省钱中心🅥](https://tgbox.cc/en/detail/pojieapp/) | 13.5K | 🚀共享ID 独享ID 兑换码 @yangwangshop 🐷频道主 好棒的羊王 @haobangdada 机场推荐 私聊羊王 @haobangdada <sub>`Freebies`</sub> |
| [NewMobileLife](https://tgbox.cc/en/detail/newmobilelife/) | 12.6K | Time-limited free information, Apple news, new technology and practical tricks <sub>`iOS`</sub> |
| [搬瓦工补货推送](https://tgbox.cc/en/detail/bandwagonhostnews/) | 11.7K | BandwagonHost supplier information and important information. Welcome to BandwagonHost:… <sub>`Hosting & VPS`</sub> |
| [灵车漂移](https://tgbox.cc/en/detail/hearse_drifting/) | 10.6K | Regularly recommending DJI products (including but not limited to consumer electronics)… <sub>`Gadgets`</sub> |
| [ App Store 限免应用 & ...](https://tgbox.cc/en/detail/ooapps/) | 10.3K |  Limited-Time Free https://oodata.net <sub>`Free` `iOS`</sub> |
| [限時免費 LimitFree](https://tgbox.cc/en/detail/limitfree/) | 6.4K | 可依照以下 Hashtag 搜尋您要的系統喔! \#iOS, \#Mac, \#Win, \#Android, \#Origin, \#UPlay, \#Steam \#Rockstar… <sub>`Freebies` `iOS`</sub> |
| [免費資源網路社群 Free Group](https://tgbox.cc/en/detail/free_group/) | 5.3K | https://free.com.tw Related Channel: @LimitFree i️ Made by @SeanChannel <sub>`Freebies`</sub> |
| [LIHAI买买买&收藏夹](https://tgbox.cc/en/detail/lihai/) | 5.2K | Source: forum, news, channel lihai sharing group: @lihaiba |
| [🅰️纯粹的 App Store 应用推荐👍](https://tgbox.cc/en/detail/app_store/) | 5K | 一站式推荐苹果五大系统中精选的免费和限免应用。【限免价格以 🇺🇸 App Store 为准】 🅰️App\_Sore 交流群💬：@appstorechat 🦋Bluesky… <sub>`Free` `iOS`</sub> |
| [🎏彼岸羊毛薅到底捡漏优惠组🛒](https://tgbox.cc/en/detail/lycorisstore/) | 4.6K | 高质量优惠活动信息时报！！不错过任何好价商品！ 期待各位加入！！ 「招待不周」 本群淘宝食用方法： 复制淘口令(或复制全文)打开陶宝会自动弹出商品信息！无需登录！… <sub>`Freebies`</sub> |
| [iOS限免&降价](https://tgbox.cc/en/detail/apps_share/) | 3.6K | Recommend quality apps, free of time validity, please download the first time, open… <sub>`Free` `iOS`</sub> |

[All deals on TGbox →](https://tgbox.cc/en/channel/deals/)

<a id="channel-ai"></a>

### AI

| Name | Subscribers | About |
| --- | ---: | --- |
| [Cocoon](https://tgbox.cc/en/detail/cocoon/) ✔️ | 173.3K | <sub>`LLMs` `Official`</sub> |
| [Data Science by ODS.ai 🦜](https://tgbox.cc/en/detail/opendatascience/) | 38.7K | First Telegram Data Science channel. Covering all technical and popular staff about… <sub>`LLMs`</sub> |
| [AI探索指南](https://tgbox.cc/en/detail/aigc1024/) | 37.1K | About AIGC Artificial Intelligence, Thinking, Knowledge Expansion, Capacity Enhancement,… <sub>`ChatGPT`</sub> |
| [AI中文社区](https://tgbox.cc/en/detail/lpttech/) | 35.7K | AI year, human decade ️ Post @FreonLiquid Channel https://t.me/LptTech Discord… <sub>`LLMs`</sub> |
| [AI, Data Science, Machine Learning & IOT](https://tgbox.cc/en/detail/kdnuggets/) | 23.2K | Posts from world's largest datascientists community and latest trends learning articles… <sub>`LLMs`</sub> |
| [ChatGPT / AI新闻聚合](https://tgbox.cc/en/detail/ai_news_cn/) | 22K | HTTPS://t.me/AI\_News\_CN Mainstream AI service status page Notification HTTPS ChatGPT/AI… <sub>`ChatGPT` `LLMs`</sub> |
| [AI一线｜ShareCentre](https://tgbox.cc/en/detail/sharecentre/) | 15.8K | 每日更新最新最全AI新闻 免费节点请关注 @ShareCentrePro 💁‍♂️投放广告请联系 @AS24400 <sub>`LLMs`</sub> |
| [ChatGPT 精选](https://tgbox.cc/en/detail/awesomechatgpt/) | 15.1K | Selected ChatGPT Resources <sub>`ChatGPT`</sub> |
| [Roller \| ChatGPT Astra 中文社区](https://tgbox.cc/en/detail/rollerrolling/) | 8.6K | Interesting AI technology diaries Group: t.me/+XThkfMjY1bRiYTVh Written by @bnl5110 and… <sub>`ChatGPT`</sub> |
| [Hermes爱马仕&🦞OpenClaw小龙虾](https://tgbox.cc/en/detail/openclaw1024/) | 7K | Share the latest games, tricks, experiences, opinions, etc. with Hermes & OpenClaw.2026's… <sub>`AI Agents`</sub> |
| [动察Beating AI News](https://tgbox.cc/en/detail/onemillion_ai/) | 3.8K | AI新闻信息流 |
| [PriceAI 通知频道](https://tgbox.cc/en/detail/priceaicc2/) | 336 | This channel mainly releases: 1, platform function updates. 2, new channels, new… <sub>`API Relay`</sub> |
| [AI 工具情报局](https://tgbox.cc/en/detail/aigongjuqbj/) | 325 | Use AI for specific tasks: organizing data, processing documents, creating content and… <sub>`AI Agents`</sub> |

[All ai on TGbox →](https://tgbox.cc/en/channel/ai/)

<a id="channel-fun"></a>

### Fun

| Name | Subscribers | About |
| --- | ---: | --- |
| [每日沙雕墙](https://tgbox.cc/en/detail/woshadiao/) | 115.2K | Sorry to say, you are all here: @shadiaoooooo Post Robot: @tougaotest\_bot Unchecked text:… <sub>`Memes`</sub> |
| [树洞🌳群友问❓有点意思🗳](https://tgbox.cc/en/detail/shortcutscn/) | 89.4K | Discover all kinds of pleasures and ask questions as long as there are questions for the… |
| [树洞🌳群友问❓有点意思🗳](https://tgbox.cc/en/detail/qunyouwen/) | 89.4K | Discover all kinds of pleasures and ask questions as long as there are questions for the… |
| [简中互联网废物大赏](https://tgbox.cc/en/detail/fucku_idiot/) | 40.4K | 本频道是纯兴趣驱动的非盈利频道，暂不接受任何软硬广告刊登，亦不接受挂人开盒网暴骚扰信息！ 投稿方式：私聊 @zhcnidiot\_bot 所有投稿均做匿名处理… <sub>`Memes`</sub> |
| [平壤卫视](https://tgbox.cc/en/detail/pingrangtv/) | 29.2K | 新闻热点 趣味资讯 沙雕动态 猫猫狗狗 聊天群组: @Orz\_zayu 广告合作: @zayutalk\_bot 频道二台: @zayuTV （防失联） <sub>`Memes`</sub> |
| [如何与沙雕相处](https://tgbox.cc/en/detail/ruheyushadiaoxiangchu/) | 28.7K | Rosmontis's Daily @Rosmontis\_Daily <sub>`Memes`</sub> |
| [每天趣事](https://tgbox.cc/en/detail/meitian/) | 28.7K | The content of this channel: A variety of sand-cold jokes, joke and curiosities. Record… |
| [煎蛋无聊图](https://tgbox.cc/en/detail/jandan_pic/) | 25.3K | Automatically scratch the baking egg home recommended boredom charts and their comments… <sub>`Memes`</sub> |
| [上班划水之无聊图](https://tgbox.cc/en/detail/goworkbitch/) | 24.8K | Note: This channel discussion group is only used as a comment zone, does not accept… <sub>`Memes`</sub> |
| [每日无数猫](https://tgbox.cc/en/detail/miaowu/) | 20K | Posted to Post Robot @miaowu\_submission\_bot With a problem please tap admin @pikameow… |
| [Foolish TraceWind](https://tgbox.cc/en/detail/foolishtracewind/) | 16.6K | This channel was founded on August 12, 2018 \~ Dedicated to updating high-quality fun sand… <sub>`Memes`</sub> |
| [🐾 可爱の收藏 cute collection](https://tgbox.cc/en/detail/kawaii_collection/) | 6.7K | Dedicated to spreading happiness Share some fun, charming and cute things with NSFW sand… |
| [每日無數汪](https://tgbox.cc/en/detail/wushuwang/) | 2.1K | All is the world of Vancouver Posts Please contact @pikameow @marogatari @SaltedFishSH… |

[All fun on TGbox →](https://tgbox.cc/en/channel/fun/)

<a id="channel-wallpaper"></a>

### Wallpapers & Images

| Name | Subscribers | About |
| --- | ---: | --- |
| [Android Themes Channel](https://tgbox.cc/en/detail/androidthemes/) ✔️ | 278.7K | Android Themes for all! If you'd like to see your Android theme featured here or learn… <sub>`Android` `Design` `Official`</sub> |
| [Desktop Themes Channel](https://tgbox.cc/en/detail/themes/) ✔️ | 120.2K | Desktop Themes for all! If you'd like to see your Desktop theme featured here or learn… <sub>`Design` `Official` `Windows`</sub> |
| [PIXIV站每日 Top50搬运\|动漫\|ACG\|二次元\|每日精选图片\|動漫\|P站](https://tgbox.cc/en/detail/pixiv_top50/) | 90.5K | Shipping PIXIV daily comprehensive ranking top 50 images resources, updated from time to… <sub>`Anime`</sub> |
| [🧸心情文案吧｜素材｜壁纸｜头像](https://tgbox.cc/en/detail/sucai939/) | 63.9K | Mindfulness is the old man Pen written is the old story Daily update, your attention, my… |
| [🧸心情文案吧｜素材｜壁纸｜头像](https://tgbox.cc/en/detail/wenanba/) | 62.7K | 🌸心心念念是旧人🌸 🌸笔笔写下是旧情🌸 每日更新，你的关注，是我最大的动力！ 💎商务合作： @Gujiajia\_bot （顾佳佳） 多一个字少一个字都是骗子哦～ |
| [公诸同好 \| 二次元美图分享 🅥](https://tgbox.cc/en/detail/gongzhutonghao/) | 38.1K | High-quality animated & secondary (secondary) & illustrated work sharing discussion group… <sub>`Anime`</sub> |
| [SomeACG \| 每日少女壁纸 🍂](https://tgbox.cc/en/detail/someacg/) | 37K | Please come a wallpaper today? here is the highest quality ACG wallpaper channel all TG!… <sub>`Anime`</sub> |
| [绝对萌域](https://tgbox.cc/en/detail/moeisland/) | 21.3K | O❤️O may be repeated Try to be the latest snapshot Discovered AI maps will be tagged New… <sub>`Anime`</sub> |
| [國家地理雜誌 中文版](https://tgbox.cc/en/detail/natgeomedia/) | 14.7K | Hi explorers, explore the world with national geography! official website of National… <sub>`Photography` `Science`</sub> |
| [\[Pixiv\]wallpaper\| 高质量插画壁纸](https://tgbox.cc/en/detail/pixivshare/) | 13.9K | A channel dedicated to sharing various pixiv works\~ ------ friendship… <sub>`Anime`</sub> |
| [Cyunreiの插画收藏夹](https://tgbox.cc/en/detail/cyunreicollection/) | 7.9K | <sub>`Anime` `Design`</sub> |
| [🖼ANIME Wallpaper](https://tgbox.cc/en/detail/acgwallpaper/) | 6.1K | 壁纸 Wallpaper 每天更新一次 Update daily 其他频道 Other channels： @bravo\_hentai @nskjdnfnfnj… <sub>`Anime`</sub> |
| [疗愈小屋 二周目🍡](https://tgbox.cc/en/detail/sweetmoepic/) | 2.3K | Study work hard a day! it will be lovely healing to the sand sculpting content, pure… <sub>`Anime`</sub> |

[All wallpapers & images on TGbox →](https://tgbox.cc/en/channel/wallpaper/)

<a id="channel-books"></a>

### Books & Comics

| Name | Subscribers | About |
| --- | ---: | --- |
| [电子书/听书/期刊/课程资源](https://tgbox.cc/en/detail/sharebooks4you/) | 131.7K | Business @msgmenow\_bot Search @sharing\_books4u Navigation https://t.me/sharebooks4you/7 <sub>`E-books`</sub> |
| [Legado｜开源阅读｜频道](https://tgbox.cc/en/detail/legado_channels/) | 93K | You can post newspapers on your channel, read APKs, book sources, subject fonts, and some… <sub>`Android` `E-books` `Open Source`</sub> |
| [小声读书 🙈](https://tgbox.cc/en/detail/weekly_books/) | 38.1K | In the noisy wave of AI, we choose to go back to the simplest logic of growth: by reading… <sub>`E-books`</sub> |
| [日更100 刺猬猫菠萝包起点飞卢小说有声书txt分享](https://tgbox.cc/en/detail/xkjdz55/) | 22.5K | 资源来源于网洛，侵权找我必删 <sub>`E-books`</sub> |
| [☁️云上报刊亭📰 \[精选\] 🅥 -- 新闻，报纸，杂志，电子书，资料 PDF](https://tgbox.cc/en/detail/magazinesclubnew/) | 18.5K | News, newspapers, magazines, e-books, information PDF magazine channel @newspaperdigest <sub>`E-books`</sub> |
| [Lemaned的宝藏书屋（轻小说书库）](https://tgbox.cc/en/detail/lemaned1314/) | 16.7K | 分享各种有趣的轻小说 <sub>`Anime` `E-books`</sub> |
| [Λ-Reading ♡](https://tgbox.cc/en/detail/goreading/) | 15.2K | The main content of this channel is: 1. book and reading \# book extracts \# book… <sub>`E-books`</sub> |
| [图书杂志资源免费分享](https://tgbox.cc/en/detail/kkfxzy/) | 10.2K | 本频道致力于分享杂志资源、交流项目，每天会免费更新杂志！有数百种杂志进行更新，希望有更多的杂志爱好者可以关注我们，分享我们的频道！ <sub>`E-books` `Free`</sub> |
| [电书摊](https://tgbox.cc/en/detail/telebookstall/) | 4.8K | 愿客观主义照亮你的世界 <sub>`E-books`</sub> |
| [随机漫谈](https://tgbox.cc/en/detail/what_to_read_today/) | 3.1K | Subscribe to the monthly newsletter: https://quail.ink/rambling |

[All books & comics on TGbox →](https://tgbox.cc/en/channel/books/)

<a id="channel-games"></a>

### Games

| Name | Subscribers | About |
| --- | ---: | --- |
| [Galgame 频道](https://tgbox.cc/en/detail/galgamer_channel/) | 54.8K | Galgame information and recommendations for you → @Galgamer → Monkey Burning Friend:… <sub>`Anime`</sub> |
| [Steam快讯](https://tgbox.cc/en/detail/steamsteam/) | 15.4K |  |
| [瞎玩菌 \#SFW](https://tgbox.cc/en/detail/blindgamer/) | 10.7K | Focus on PC/hosting games, share game insights, sunbaths, and videos. Join a player group… |
| [Genshin PaiHub](https://tgbox.cc/en/detail/paihub/) | 9.5K | Genshin PaiHub 一个原神全自动化的推送图床 该频道仅限用于图床存档 友链/Link @Daily0721 @PureWaterSpirit2nd <sub>`Anime`</sub> |
| [原神 \| Genshin Impact CN](https://tgbox.cc/en/detail/yuanshen/) | 8.9K | Step into a vast magical world of adventure This channel semi-automatically posts… <sub>`Official`</sub> |
| [NS新闻转报](https://tgbox.cc/en/detail/switchnewcn/) | 7.5K | For official information see http://www.nintendo.com.hk/https://www.nintendo.com.hk/switch… |
| [VNKC Public Channel](https://tgbox.cc/en/detail/visualgalgame/) | 6.9K | Visual Novel Kenkyu Club Group: https://t.me/+vIoZMjanZfYzNTc9 Resource channel:… <sub>`Anime`</sub> |
| [追尾游戏热点](https://tgbox.cc/en/detail/vwyxrd/) | 4.2K | 能聊与玩游戏真是太好了！ 发一发喜欢得消息，游戏为主，其余分享！ 也要有时间发空，远望宇宙。 发与分享喜欢，知与游玩游戏。 聊与分享折扣，追尾游戏热点。 + 群组… |
| [Ingress 中文](https://tgbox.cc/en/detail/ingresschinese/) | 3.6K | The world is not as you see it. |
| [PlayStation 新闻转发](https://tgbox.cc/en/detail/playstationnewssss/) | 3.2K | Playstation XXXX: Play Has No Limits |

[All games on TGbox →](https://tgbox.cc/en/channel/games/)

<a id="channel-video"></a>

### Video & Music

| Name | Subscribers | About |
| --- | ---: | --- |
| [阿里、夸克、百度网盘4K影视资源](https://tgbox.cc/en/detail/aliyun_4k_movies/) | 216.5K | Welcome to the 4K video channel, channel management is for film enthusiasts, mainly… <sub>`Cloud Drive` `Movies` `TV Series`</sub> |
| [4K影视屋(分屋）-蓝光无损电影](https://tgbox.cc/en/detail/dianying4k/) | 87.2K | Advertising recommendations @pinuo\_bot <sub>`Movies`</sub> |
| [懒狗集中营-115/阿里/百度/迅雷/夸克 影视分享 阿里云盘百度网盘115网盘…](https://tgbox.cc/en/detail/vip115hot/) | 52.4K | ✨ Recommended Posting and Advertising @pinuo\_bot <sub>`Cloud Drive` `Movies`</sub> |
| [抖音AI动漫Ai短剧视频](https://tgbox.cc/en/detail/tamanila/) | 43.3K | <sub>`Short Drama`</sub> |
| [115影视资源分享频道](https://tgbox.cc/en/detail/qukanmovie/) | 30.6K | <sub>`Cloud Drive` `Movies`</sub> |
| [爷青回动画分享频道](https://tgbox.cc/en/detail/yeqingjie_gjg666/) | 22.3K | My childhood memories will forever remain in my mind a small piece of land, and what I… <sub>`Anime`</sub> |
| [高画质动漫分享 Anime share \[超级牛马版\]](https://tgbox.cc/en/detail/yxhmd/) | 20.3K | Receive high quality BDrip \# Anime \# Anime Channel \# Anime \# Second Dimension Comic… <sub>`Anime`</sub> |
| [观影视界\[GyWEB\]](https://tgbox.cc/en/detail/wfysfx03/) | 6.7K | 每天更新全球最新4K高画质，高帧率，剧集，动漫，电影，综艺..... 私聊联系→： @WFengY\_bot <sub>`Movies` `TV Series`</sub> |

[All video & music on TGbox →](https://tgbox.cc/en/channel/video/)

<a id="channel-life"></a>

### Life & Living

| Name | Subscribers | About |
| --- | ---: | --- |
| [小红书一瞥](https://tgbox.cc/en/detail/xhsyp/) | 31.5K | Note: Copyright belongs to the original author, and content with more than 50 likes will… |
| [中年人生存报告](https://tgbox.cc/en/detail/dogdairy/) | 14K | About workplace, life, entrepreneurship, 30+, unemployment, middle-aged life, couple… |
| [rvalue的生草日常](https://tgbox.cc/en/detail/rvalue_daily/) | 8.3K | (可能位于火星的)大 草 原(? @rvalue\_official 的个人channel 你以为是草图channel, 其实是日常channel哒!(… |
| [笔记本：Lin's 文字世界 : 个人见闻/B站视频/网络见闻](https://tgbox.cc/en/detail/linsbooka/) | 8.1K | 频道始于2019年6月，长期保持不太稳定的持续更新，内容主要以纯文字为主。伴有频道主的个人生活。 附属群聊：查看置顶 个人频道：https://t.me/LinYunChannel… |
| [浮生东京](https://tgbox.cc/en/detail/lifejapan/) | 7.4K | Discuss group @JapanChats Business collaboration @appdo\_bot Sometimes you talk about… |
| [每日摄影观察](https://tgbox.cc/en/detail/cnphotog_collect/) | 6.8K | A non-serious photography channel group: @cnphotog <sub>`Photography`</sub> |
| [树洞](https://tgbox.cc/en/detail/atreehole/) | 5.1K | 树洞是一个匿名分享秘密和诉说心事的地方。可以将自己的秘密匿名分享在树洞里，获得其他网友的建议与关注。 树洞入口 @TheTreeHoleBot 输入的消息将显示在这里，完全匿名。… |

[All life & living on TGbox →](https://tgbox.cc/en/channel/life/)

<a id="channel-cloud-drive"></a>

### Cloud Drives

| Name | Subscribers | About |
| --- | ---: | --- |
| [阿里云盘发布频道](https://tgbox.cc/en/detail/sharealiyun/) | 201.8K | Harmonious and loving Ali Cloud Sharing Channel, where everyone can share the resources… <sub>`Cloud Drive`</sub> |
| [Google Drive 资源 \| New @gdurl](https://tgbox.cc/en/detail/gdsharing/) | 124.5K | 很高兴你来到新的 @gdurl 频道。这里有各种 Google Drive 资源，包括大电影，小电影，电子书，无损音乐等。 Here is channel for Google… <sub>`Cloud Drive` `Movies`</sub> |
| [海外影视资源频道(夸克、阿里、百度)](https://tgbox.cc/en/detail/netdisk_movies/) | 37.2K | 此频道为阿里云盘、夸克网盘、百度网盘频道，发布热门与经典海外影视。 <sub>`Cloud Drive` `Movies`</sub> |
| [LIHAICloud](https://tgbox.cc/en/detail/lihaicloud/) | 31.6K | @lihaiba 云盘归档，资源为收集，如侵犯你的权利联系BOT @lihaibadeBot ，我们第一时间处理 <sub>`Cloud Drive`</sub> |
| [夸克网盘资源收藏夹](https://tgbox.cc/en/detail/quarkfree/) | 24K | 合作 @ygwpBot 防失联 ygwp.cc \#短剧 \#擦边短剧 \#电视剧 \#电影 \#电子书 \#微短剧 \#横屏短剧 \#网络剧 <sub>`Cloud Drive` `Short Drama`</sub> |
| [阿里发布影子频道](https://tgbox.cc/en/detail/sharealiyun00/) | 3K | <sub>`Cloud Drive`</sub> |

[All cloud drives on TGbox →](https://tgbox.cc/en/channel/cloud-drive/)

<a id="channel-jobs"></a>

### Jobs & Careers

| Name | Subscribers | About |
| --- | ---: | --- |
| [DeJob—Web3招聘求职频道📮](https://tgbox.cc/en/detail/dejob_official/) | 46.7K | DeJob Web3 & AI Professional Services Platform 62K+ Users · 1,890+ Web3 Enterprises… |
| [远程工作者](https://tgbox.cc/en/detail/remote_cn/) | 25K | --- Share articles related to domestic remote work, discussions and job opportunities… <sub>`Programming`</sub> |
| [DeJob Global—Web3 Jobs📮](https://tgbox.cc/en/detail/dejob_global/) | 16.6K | DeJob｜Web3 & AI Professional Services 62K+ Users · 1,890+ Web3 Companies 🌐 dejob.ai 📢 CN… |
| [The Hire](https://tgbox.cc/en/detail/thehire/) | 8.8K | Job Board on Telegram. Submit jobs: https://goo.gl/forms/YylidnpzpvxdHaOa2 Part of The… <sub>`Programming`</sub> |
| [Remote Jobs👨‍💻](https://tgbox.cc/en/detail/remotejobs/) | 7.3K | Hand-picked remote jobs, daily roundups and practical career tips. Find your next high… |
| [DeJob｜求职频道 Job Seekers🔍](https://tgbox.cc/en/detail/dejob_talent/) | 2.6K | DeJob Web3 × AI 62K+ Users · 1,890+ Web3 Companies Jobs · Talent · Professional Services… |

[All jobs & careers on TGbox →](https://tgbox.cc/en/channel/jobs/)

<a id="channel-learning"></a>

### Learning

| Name | Subscribers | About |
| --- | ---: | --- |
| [你不知道的冷知识](https://tgbox.cc/en/detail/knowledge1024/) | 19.5K | Cold Knowledge Sharing Interesting, Funny, Useful, Unnecessary Knowledge Introducing… <sub>`Science`</sub> |
| [看新闻 学英语 双语新闻](https://tgbox.cc/en/detail/shuangyunews_rss/) | 15.2K | \[RSS full subscription real-time update\] News @zhihuribao\_rss New York Times… |
| [来一点医学科学前沿🤯🤯🥹🥹](https://tgbox.cc/en/detail/cnsmydream/) | 13.5K | See what the top biomedical giants did when they touched the fish. Message channel… <sub>`Science`</sub> |
| [Scriptable教学](https://tgbox.cc/en/detail/scriptablejs/) | 11.1K | Regularly updating teaching videos <sub>`iOS`</sub> |
| [网络技术文章分享](https://tgbox.cc/en/detail/wzxylh/) | 6.9K | 提供各种详细的网络技术教程，包括软件，文章，pdf。聊天群：https://t.me/xytool <sub>`Programming`</sub> |
| [Best Offers - Courses, Books etc](https://tgbox.cc/en/detail/coursenuggets/) | 2.3K | Amazing Free, Top Rated and Discounted courses covering various tech and non tech stacks… <sub>`Free`</sub> |

[All learning on TGbox →](https://tgbox.cc/en/channel/learning/)

<a id="channel-giveaway"></a>

### Giveaways & Freebies

| Name | Subscribers | About |
| --- | ---: | --- |
| [Telegram 抽奖活动导航 (Cn lottery)](https://tgbox.cc/en/detail/lottery_home/) | 41.4K | Easy and fair Telegram Group Lottery Tool. Telegram Lottery Assistant @cnLottery\_bot… <sub>`Freebies`</sub> |
| [免费资源](https://tgbox.cc/en/detail/freeresource/) | 28.1K | The main content of this channel is personal interest information. such as steam etc.… <sub>`Freebies`</sub> |
| [Apps推荐&抽奖&活动](https://tgbox.cc/en/detail/appssweepstakesnews/) | 8.5K | Promotion is not limited to the Apps lottery events, welcome forwarding! if disturbed… <sub>`iOS`</sub> |
| [Telegram 抽奖](https://tgbox.cc/en/detail/lotteryindex/) | 4.1K | @tgLotteryBot's lottery navigation channel. Group discussion @LotteryChat Usage:… <sub>`Freebies`</sub> |

[All giveaways & freebies on TGbox →](https://tgbox.cc/en/channel/giveaway/)

<a id="channel-nav"></a>

### Directories

| Name | Subscribers | About |
| --- | ---: | --- |
| [Coronavirus Info](https://tgbox.cc/en/detail/corona/) ✔️ | 130.1K | A list of official channels with information on COVID-19. <sub>`Official`</sub> |
| [悟空的日常TG频道](https://tgbox.cc/en/detail/daily5kong/) | 19.2K | 博客：https://wkdaily.cpolar.cn 【建议铁粉订阅本频道】 资源导航： https://tvhelper.cpolar.cn 📧【技术咨询】… |
| [Telegram 中文频道 🅥](https://tgbox.cc/en/detail/tgcn_wiki/) | 13.8K | Here you can find everything about Telegram. |
| [TG中文群组频道导航](https://tgbox.cc/en/detail/grinx_bot/) | 592 | The TG Chinese Navigation Group lists all the high-quality groups and channels across the… <sub>`Free`</sub> |

[All directories on TGbox →](https://tgbox.cc/en/channel/nav/)

<a id="channel-stickers"></a>

### Stickers

| Name | Subscribers | About |
| --- | ---: | --- |
| [Trending Stickers 🍒](https://tgbox.cc/en/detail/trendingstickers/) | 56K | <sub>`Memes`</sub> |
| [ACG Stickers](https://tgbox.cc/en/detail/acgstickers/) | 8.3K | Sharing some cute stickers and GIFs! <sub>`Anime` `Memes`</sub> |

[All stickers on TGbox →](https://tgbox.cc/en/channel/stickers/)

<a id="group"></a>

## Groups

<a id="group-tech"></a>

### Development

| Name | Members | About |
| --- | ---: | --- |
| [BDD@Tech](https://tgbox.cc/en/detail/bigdongdonggroup/) | 40K | Banned any political content & NSFW content & attacks insulting others will be… |
| [Telegram Bot Talk](https://tgbox.cc/en/detail/bottalk/) | 26.5K | Welcome to Bot Talk! 😌 Be nice 🗣 Please use English so everyone can participate 🤖 Keep… <sub>`Official` `Programming`</sub> |
| [Python 中文交流群](https://tgbox.cc/en/detail/pythonzh/) | 22.5K | Joining means agreeing to comply with: 1. Strictly prohibited: advertising/recruitment,… <sub>`Programming`</sub> |
| [折腾啥](https://tgbox.cc/en/detail/zhetengsha_group/) | 21.1K | Power Users/Automators 折腾/讨论/分享各种开源工具/脚本/自动化工作流 📢 频道 @zhetengsha 📌 资源合集导航… <sub>`Open Source`</sub> |
| [V2EX 交流群](https://tgbox.cc/en/detail/v2expro/) | 19.4K | AI, programmers, sharing creation, questions and answers, networks, life <sub>`Programming`</sub> |
| [RouterGroup\|软路由\|硬路由\|外贸电视](https://tgbox.cc/en/detail/ruanlu/) | 17.9K | Channel: https://t.me/routergroup Top: https://t.me/ruanlu/6608 |
| [Data Science,ML & AI Nugget Chats](https://tgbox.cc/en/detail/datasciencechats/) | 13.5K | Discussion community for DS, ML, IOT, AI, DEEP LEARNING and much more from Main Channel… <sub>`Programming`</sub> |
| [RSSHub - Everything is RSSible](https://tgbox.cc/en/detail/rsshub/) | 12.5K | Channel: @awesomeRSSHub Repo: https://github.com/DIYgod/RSSHub Docs:… <sub>`Open Source`</sub> |
| [Telegram Contests](https://tgbox.cc/en/detail/contests/) ✔️ | 11.7K | This group is intended for discussions of coding and design competitions held by… <sub>`Official` `Programming`</sub> |
| [Synology/黑群晖用户群](https://tgbox.cc/en/detail/nasfan/) | 11.4K | For more tutorials please visit the forum https://www.openos.org Any post or forwarding… <sub>`Linux`</sub> |
| [TDLib chat](https://tgbox.cc/en/detail/tdlibchat/) | 10.6K | <sub>`Official` `Programming`</sub> |
| [Java 编程语言](https://tgbox.cc/en/detail/javaer/) | 8.8K | Java is an object-oriented programming language, not the JAV group (SFW)… <sub>`Programming`</sub> |
| [NewlearnerのIT社群](https://tgbox.cc/en/detail/newlearnergroup/) | 8.5K | Main Topics: Main IT topics, including but not limited to open source sharing, hardware &… <sub>`Open Source`</sub> |
| [Docker](https://tgbox.cc/en/detail/dockertutorial/) | 8.3K | Docker 学习\|交流\|分享 Docker \| Moby \| Docker Compose \| Docker Swarm \| Kubernetes (k8s)\| Go… <sub>`Linux`</sub> |
| [C++ 中文交流群](https://tgbox.cc/en/detail/cpluspluszh/) | 7.6K | Joining means agreeing to comply with: 1. Strictly prohibited: advertising/recruitment,… <sub>`Programming`</sub> |

<details>
<summary>Show 23 more</summary>

| Name | Members | About |
| --- | ---: | --- |
| [\#archlinux-cn](https://tgbox.cc/en/detail/archlinuxcn_group/) | 7.1K | The news channel @archlinuxcn needs to be checked before you join the group, and you can… <sub>`Linux` `Open Source`</sub> |
| [python 自学交流](https://tgbox.cc/en/detail/p_y_t_h_o_n/) | 6.4K | Learning, sharing and growing <sub>`Programming`</sub> |
| [Dart / Flutter](https://tgbox.cc/en/detail/dartlang_group/) | 6.2K | <sub>`Programming`</sub> |
| [Go](https://tgbox.cc/en/detail/golangcn/) | 5.3K | Please consciously comply with the speech regulations https://telegra.ph/GolangCN-001-Draf… <sub>`Programming`</sub> |
| [Ubuntu 中文](https://tgbox.cc/en/detail/ubuntuzh/) | 4.6K | Ubuntu Chinese Communications Group Language: Chinese (Simplified and Traditional)… <sub>`Linux`</sub> |
| [Kali / BlackArch Linux 中文交流](https://tgbox.cc/en/detail/hackerzh/) | 4.5K | Every day, we change the world. The group promotes meaningful speech, sharing valuable… <sub>`Linux` `Security`</sub> |
| [Coder Offtopic 中文群](https://tgbox.cc/en/detail/coder_ot/) | 4.4K | In this group, you can even discuss programming about this group please carefully read… <sub>`Programming`</sub> |
| [GNU/Linux](https://tgbox.cc/en/detail/linux_group/) | 3.6K | Rules are pinned. Read them. Ping for issues: — @watzon — @mazunki — @elfametesar Avatar… <sub>`Linux`</sub> |
| [wikipedia-zh-computer-science](https://tgbox.cc/en/detail/wikipedia_zh_cs/) | 3.4K | T.me/wikipedia\_zh\_cs This group mainly discusses the editing of computer/computer science… |
| [OpenWRT\*LEDE/Wireless Routers](https://tgbox.cc/en/detail/openwrt_routers/) | 3.1K | This is a discussion group about routers and wireless devices ...................... the… <sub>`Linux`</sub> |
| [\#archlinux-cn-offtopic](https://tgbox.cc/en/detail/archlinuxcn_offtopic/) | 2.9K | Arch Linux 相关人员的无关讨论群！技术提问左转主群 \| 新加群请先阅读 https://z.sh/arch 本群所有历史聊天记录都有落絮的公开存档，自删无法删除历史存档… <sub>`Linux`</sub> |
| [JavaScript 中文交流](https://tgbox.cc/en/detail/javascriptzh/) | 2.7K | JavaScript, commonly abbreviated as JS, is an advanced, explanatory programming language… <sub>`Programming`</sub> |
| [\[CN\] Android Dev](https://tgbox.cc/en/detail/androiddevcn/) | 2.5K | For Android development topics only. Do not chat, go to the following group. Neighborhood… <sub>`Android` `Programming`</sub> |
| [大数据杂谈](https://tgbox.cc/en/detail/bigdatazh/) | 2.2K | Big data architecture and development technology communication Do not sell data, do not… <sub>`Programming`</sub> |
| [Frontend 前端中文技术交流](https://tgbox.cc/en/detail/frontend_talk/) | 2.1K | This channel is mainly used as a front-end development classmates technology exchange… <sub>`Programming`</sub> |
| [NixOS 中文](https://tgbox.cc/en/detail/nixos_zhcn/) | 2.1K | 加群后请先阅读 https://t.me/nixos\_zhcn/343893 <sub>`Linux`</sub> |
| [grammY](https://tgbox.cc/en/detail/grammyjs/) | 1.9K | grammY. The Telegram Bot Framework. Website: https://grammy.dev/ Source:… <sub>`Open Source` `Programming`</sub> |
| [Kubernetes CN](https://tgbox.cc/en/detail/kubernetes_cn/) | 1.6K | Kubernetes Chinese community Technical landing practices and communication platform Group… <sub>`Linux` `Open Source`</sub> |
| [\[CN\]Flutter Dev](https://tgbox.cc/en/detail/fluttercn/) | 1.6K | Official website of Flutter: https://gitter.im/flutter/flutter <sub>`Programming`</sub> |
| [DevOps CN](https://tgbox.cc/en/detail/devops_cn/) | 1.3K | 一个严肃的技术交流群 在本群，你甚至可以讨论编程🐶 支持各种开发语言 兼容各种平台（包括Windows Server） 实力甩锅，文明互喷… <sub>`Programming`</sub> |
| [\#archlinux-cn-appearance](https://tgbox.cc/en/detail/archlinuxcn_appearance/) | 937 | 欢迎来到 Arch Linux CN Appearance Sharing Group，欢迎分享讨论各种壁纸、桌面环境、窗口管理器美化配置～ ps: 过于 ot 的话题还请移步… <sub>`Design` `Linux`</sub> |
| [TONAPI Tech](https://tgbox.cc/en/detail/tonapitech/) | 833 | Ask anything related to TONAPI usage Links: tonapi.io • tonconsole.com •… <sub>`Programming` `TON`</sub> |
| [Haskell 中文交流](https://tgbox.cc/en/detail/haskellzh/) | 327 | Haskell is a standardized, universal, pure function programming language with non-limited… <sub>`Programming`</sub> |

</details>

[All development on TGbox →](https://tgbox.cc/en/group/tech/)

<a id="group-crypto"></a>

### Crypto & Web3

| Name | Members | About |
| --- | ---: | --- |
| [Binance English](https://tgbox.cc/en/detail/binanceexchange/) ✔️ | 338.3K | Binance announcements: @binance\_announcements Other Languages: Please type /communities… <sub>`Exchanges`</sub> |
| [币安官方中文群](https://tgbox.cc/en/detail/binancechinese/) ✔️ | 271.2K | Everything about yellow, black, gambling, gray, and politics will be kicked out Banned… <sub>`Exchanges`</sub> |
| [CoinMarketCap English](https://tgbox.cc/en/detail/coinmarketcap/) ✔️ | 72.6K | Official CMC Telegram Channel: Updates, News, and Events - Ann/News Channel:… |
| [Bitget海外华语社区](https://tgbox.cc/en/detail/bitget_cnofficial/) | 68.3K | Bitget 成立于2018年，是世界领先的加密货币交易平台和 Web 3 公司。Bitget为全球100多个国家和地区提供服务，通过领先的跟单服务等多种交易解决方案，帮助超250… <sub>`Exchanges` `Official`</sub> |
| [Toncoin Chat](https://tgbox.cc/en/detail/toncoin_chat/) | 44.4K | TON = $1.65 @Toncoin channel chat. A community dedicated to TON discussions. Official… <sub>`TON`</sub> |
| [Lookonchain](https://tgbox.cc/en/detail/lookonchain/) ✔️ | 36.9K | Please contact @lookonchainsupport if you have any questions. We will never DM you first!… <sub>`On-chain Data`</sub> |
| [💎GMGN Degen Group - Official](https://tgbox.cc/en/detail/gmgnai/) | 23.4K | Discover MEME Trading in seconds 🚀 Follow Smart Money to earn!💰 DM @AleXX\_094 to collab -… <sub>`Quant Trading` `Solana`</sub> |
| [CoinGecko](https://tgbox.cc/en/detail/coingecko/) ✔️ | 19.9K | Official CoinGecko Telegram Group |
| [币安API中文群](https://tgbox.cc/en/detail/binance_api_chinese/) | 19.6K | 本群仅讨论币安API及直接相关的程序设计话题。不处理账户问题，不接受投诉，更不接收它群被封申诉。否则一律ban。 如果您不明白“API”的含义，那么本群并不适合您。 API英文群… <sub>`Exchanges` `Official`</sub> |
| [BlockTempo 官方討論群 - 動區動趨，由社群而生的區塊鏈媒體](https://tgbox.cc/en/detail/blocktempo/) | 18.8K |  |
| [TRON China 03](https://tgbox.cc/en/detail/tronnetworkcn03/) | 15.9K | ❤️欢迎来到波场TRON官方中文社群 官网： tron.network trondao.org 官方中文X： https://x.com/trondaoCN 波场官方微信客服：… <sub>`Official`</sub> |
| [Gate.io的小伙伴們](https://tgbox.cc/en/detail/gate_io/) | 11.8K | Welcome! here is the official Chinese community for the opening of the gate.All about the… <sub>`Exchanges`</sub> |
| [區塊先生 Mrblock (DeFi Asia)](https://tgbox.cc/en/detail/mrblocktw/) | 10.7K | 圖書館：https://t.me/mrblock\_info YouTube: 區塊先生 or mrblocktw Discord:… |
| [CoinGlass.com](https://tgbox.cc/en/detail/coinglass/) | 10.6K | CoinGlass is a professional cryptocurrency derivative data analysis platform. <sub>`On-chain Data` `Quant Trading`</sub> |
| [BNB Chain 官方華語社群](https://tgbox.cc/en/detail/bnbchainzh/) | 10K | BNB Chain 官方網站： https://www.bnbchain.org BNB Chain 華語TG社群: https://t.me/BNBChainZH 開發者社區：… <sub>`Official`</sub> |

<details>
<summary>Show 14 more</summary>

| Name | Members | About |
| --- | ---: | --- |
| [PANews 加密货币冲锋队](https://tgbox.cc/en/detail/panewslab/) | 10K | Provide valuable information and in-depth insights in the field of cryptocurrencies and… <sub>`Finance`</sub> |
| [TRON China](https://tgbox.cc/en/detail/tronnetworkcn/) | 8.9K | ❤️ 欢迎有兴趣交流互动的用户加入波场官方Telegram 03 群： https://t.me/tronnetworkCN03 一起畅聊波场最新动态！… <sub>`Official`</sub> |
| [BlockBeats Chat](https://tgbox.cc/en/detail/blockbeats_app/) | 6.7K | 欢迎进入BlockBeats交流群 |
| [Solidity Development](https://tgbox.cc/en/detail/dev_solidity/) | 5.8K | This chat is for Solidity developers. Wiki - @soliditypedia Jobs - @ethereum\_jobs\_market <sub>`Programming`</sub> |
| [Biteye中文电报群](https://tgbox.cc/en/detail/biteyecn/) | 5.4K |  |
| [BITWU精选频道｜Bitwu Community](https://tgbox.cc/en/detail/bitewu/) | 5K | The group is a daily survey for BITWU, mainly for daily high-quality information push,… |
| [金色财经交流群](https://tgbox.cc/en/detail/jinse2022/) | 4.3K | Gold Finance is a one-stop blockchain industry service platform that gathers industry… |
| [币安\|欧易\| Bybit \| Bitget \|火币加密货币交流群](https://tgbox.cc/en/detail/jmhbgroup/) | 2.4K | Navigation station: www.0xnav.com Airborne channel:@tglukongtou Navigation… <sub>`Airdrops` `Exchanges`</sub> |
| [Lighter 中文社區](https://tgbox.cc/en/detail/lighter_asia/) | 2.2K | Lighter.xyz 是一個針對永續合約的去中心化交易協議，透過專屬的 zk‑Rollup 技術（稱為 zkLighter）提供可驗證的訂單撮合與清算引擎。整個系統運行在以太坊… |
| [MugglePay 麻瓜宝💰crypto only](https://tgbox.cc/en/detail/mugglepay/) | 2.1K | MugglePay EN: https://t.me/mugglepay\_en MuggleWallet: https://mugglewallet.com/ MugglePay… |
| [加密货币与区块链讨论群](https://tgbox.cc/en/detail/onblockchain/) | 2K | Cryptocurrency and Blockchain Channels: https://t.me/knowBlockchain |
| [Foresight News Official](https://tgbox.cc/en/detail/foresightnewstalk/) | 1.9K | 欢迎来到Foresight News Talk大本营\~ APP下载地址： foresightnews.pro/download… |
| [OctoBot 🐙](https://tgbox.cc/en/detail/octobot_trading/) | 1.4K | https://octobot.click/UBpmSv <sub>`Open Source` `Quant Trading`</sub> |
| [CCXT Chat](https://tgbox.cc/en/detail/ccxt_chat/) | 1.2K | CryptoCurrency eXchange Trading Library <sub>`Open Source` `Quant Trading`</sub> |

</details>

[All crypto & web3 on TGbox →](https://tgbox.cc/en/group/crypto/)

<a id="group-software"></a>

### Software

| Name | Members | About |
| --- | ---: | --- |
| [🤙 AyuGram MaxGround](https://tgbox.cc/en/detail/ayugramchat/) | 94.4K | 📈 Channel: @ayugram 🔬 FAQ: @AyuGramFAQ 💘 exteraGram: @exteraGram 😂 Memes: @ayugramfun 📚… |
| [Google Drive 无限容量](https://tgbox.cc/en/detail/google_drive/) | 21.5K | 本群提供无限容量 Google Drive 申请方法、大电影小电影电子书无损音乐等资源。 This group provides ways to get unlimited… <sub>`Cloud Drive`</sub> |
| [iBeta 尝鲜派 \| 官方群](https://tgbox.cc/en/detail/ibetame/) | 16.7K | iBeta is the official group of iBeta.me. Welcome to @ibeta\_channel <sub>`iOS`</sub> |
| [\[ZH/EN\] HyperCeiler \| Chat](https://tgbox.cc/en/detail/cemiuiler/) | 13.6K | The group must first look at the top of the group!! - Recommended use of… <sub>`Android` `Open Source`</sub> |
| [好软分享·交流群](https://tgbox.cc/en/detail/haoruanfenxianggroup/) | 12.7K | Good Soft Sharing · Main channel: @haoruanfenxiang Good Soft Sharing · Listing channel:… <sub>`Android` `Windows`</sub> |
| [Notion 中文社区](https://tgbox.cc/en/detail/notionso/) | 12.4K | 💡 With Notion primarily, around Notion efficiency tools, experience as a supplement. The… |
| [Appinn Talk 💧 小众软件](https://tgbox.cc/en/detail/appinn/) | 8.3K | Appinn.com This group link: https://t.me/appinn New news channel: https://t.me/appinnfeed… <sub>`Free`</sub> |
| [exteraGram Forum 🌐](https://tgbox.cc/en/detail/exteraforum/) | 7.7K | Channel: @exteraGram Website: exteraGram.app FAQ: @exteraFAQ APKs: @exteraReleases Beta:… <sub>`Android` `Open Source`</sub> |
| [Zapro · 2026](https://tgbox.cc/en/detail/zaprobest/) | 7.4K | Home / Blog / Blog / Blog / Blog / Blog / Blog / Blog / Blog / Blog / Blog / Blog / Blog… |
| [Adblock🎃 群组 🅥 🌟🌈🌝🐳🍗🏳️‍🌈](https://tgbox.cc/en/detail/adblock4limbo/) | 6.6K | ✅ Go to the web advertising tutorial reference https://t.me/Adblock4limbo/21 Newcomers… |
| [Legado Beta](https://tgbox.cc/en/detail/beta_legado/) | 6.1K | Beta Release Notification Channel: @Legado\_Beta Release Discussion Group: @Beta\_Legado <sub>`Android` `E-books` `Open Source`</sub> |
| [软件工具交流群](https://tgbox.cc/en/detail/widgettalk/) | 5.8K | Unblock @WidgetPlusBot Efficiency Tools, Software Review, Resource Sharing, Everything… |
| [小众 App 交流群](https://tgbox.cc/en/detail/appfoundgroup/) | 5.5K |  |
| [APPDO 数字交流群](https://tgbox.cc/en/detail/appdododo/) | 4.5K | Post/Promoting/Channel Unlock: @appdo\_bot 🏻 Blog https://japan.typlog.io/ 📷 Emerging… |
| [CoolApk World](https://tgbox.cc/en/detail/riocoolapk/) | 3.9K | 非酷安官方群组 群组日志 @CWLOG 群组规定 t.me/c/1247493529/475086 The English group rules… <sub>`Android`</sub> |

<details>
<summary>Show 12 more</summary>

| Name | Members | About |
| --- | ---: | --- |
| [AdGuard 【中文】](https://tgbox.cc/en/detail/adguard_chinese/) | 3.3K | WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB WEB… <sub>`Security`</sub> |
| [OctoGram \| Chat](https://tgbox.cc/en/detail/octogramchat/) | 3.3K | 🐙 Official channel: @OctoGramApp 📦 Stable releases: @OctoGramAPKs 🧪 Beta releases:… <sub>`Android` `Open Source`</sub> |
| [极客分享の交流群](https://tgbox.cc/en/detail/igeekshare/) | 3.1K | Here is the great share channel @geekshare @xgeekshare group chat The group’s only main… |
| [\[Chat\] Nekogram](https://tgbox.cc/en/detail/nekochat/) | 3.1K | The official discussion group of the Nekogram, English ONLY. Rules: Send !rules in chat.… <sub>`Android` `Open Source`</sub> |
| [macOS 交流组](https://tgbox.cc/en/detail/macoser/) | 2.7K | Setapp driving, second-hand software trading, exchange of experience using |
| [简悦 - SimpRead](https://tgbox.cc/en/detail/simpreadgroup/) | 2.6K | Hi, thank you for using it, and welcome to join the "big family" of it. 🙂 Please… |
| [Cherrygram Support](https://tgbox.cc/en/detail/cherrygramsupport/) | 2.5K | The official support group of @Cherrygram. Chatting in English or Russian ONLY. NO ads or… <sub>`Android` `Open Source`</sub> |
| [\[讨论\] Nekogram](https://tgbox.cc/en/detail/nekochatzh/) | 2.5K | Nekogram official discussion group, only in Chinese. If the following information is… <sub>`Android`</sub> |
| [谷歌、微软云盘讨论群](https://tgbox.cc/en/detail/google_win/) | 1.8K | 自助购物 https://888f.store 前缀修改 @pjassistantbot ★本群 🈲黄 🈲暴 🈲广告 🈲涉及政治话题… <sub>`Cloud Drive`</sub> |
| [PanoanDrive-免费离线下载资源至Google Drive](https://tgbox.cc/en/detail/panoandrivebasic/) | 1.7K | 频道：https://t.me/PanoanChannel /mirror 下载链接 /cancel 取消（回复下载的消息） /status 查看下载状态 <sub>`Cloud Drive`</sub> |
| [Telegram macOS](https://tgbox.cc/en/detail/macswift/) | 1.6K | Beta version - https://t.me/macswift/65264 Stable version - https://macos.telegram.org <sub>`Official`</sub> |
| [Talk About RSS](https://tgbox.cc/en/detail/allaboutrss/) | 1.1K | Chat group for channel t.me/aboutrss ALL about RSS list: https://github.com/AboutRSS/ALL-a… <sub>`Open Source`</sub> |

</details>

[All software on TGbox →](https://tgbox.cc/en/group/software/)

<a id="group-vpn"></a>

### VPN & Proxy

| Name | Members | About |
| --- | ---: | --- |
| [Shadowrocket](https://tgbox.cc/en/detail/shadowrocketapp/) | 126.4K | Shadowrocket only supports Apple platforms ⚠️ Check out familiar tutorials, friendly,… <sub>`iOS`</sub> |
| [Quantumult X](https://tgbox.cc/en/detail/quanxapp/) | 53.5K | Quantumult X Communications Group ⚠️ group rules are as follows, if there is a violation,… <sub>`iOS`</sub> |
| [Loon-0x00](https://tgbox.cc/en/detail/loon0x00/) | 41.9K | This is Loon's official group Official website https://nsloon.app/ Official channel… <sub>`iOS`</sub> |
| [Clash Verge Rev](https://tgbox.cc/en/detail/clash_verge_rev/) | 37K | Clash Verge Rev is an open source free software if you spend money buying software from… <sub>`Open Source`</sub> |
| [Official Geph Users \| 迷霧通官方用戶群](https://tgbox.cc/en/detail/gephusers/) | 36.9K | Announcements Notification channels https://t.me/gephannounce Group rules cluster rules… |
| [Stash](https://tgbox.cc/en/detail/stashfans/) | 26.2K | Stash 官方讨论群组。 Stash 是 Clash 规则在 iOS 平台的最佳选择。 Stash macOS 下载 https://stash.ws Stash… <sub>`iOS` `Official`</sub> |
| [v2rayN&G](https://tgbox.cc/en/detail/v2rayn/) | 25.3K | v2rayN&G discussion group, content-only tool for the use of related topics, prohibits any… <sub>`Open Source`</sub> |
| [Surge 交流](https://tgbox.cc/en/detail/loveapps/) | 24.7K | Surge uses communication sharing (limited version!) This group theme includes but is not… <sub>`iOS`</sub> |
| [Project X](https://tgbox.cc/en/detail/projectxray/) | 15.8K | Project X 的群聊 交流群可在底线之上随便水，不要撕逼，没有滥权 有问题尽管随便问，知道的尽量回答 禁键政，禁露点，禁人身攻击/阴阳怪气 广告（含 aff）可能会被删… <sub>`Open Source`</sub> |
| [科学上网技术研究会👨‍💻](https://tgbox.cc/en/detail/kxswjs/) | 13.5K | Harmony of civilization, integrity and friendship, freedom and equality, fairness and… |
| [Hiddify Discussion](https://tgbox.cc/en/detail/hiddify_board/) | 13K | Hiddify, Internet Freedom Solution Supporting: 🎯 Mieru 🎯 Naive 🎯 xHTTP 🎯 HTTPUpgrade 🎯… <sub>`Open Source`</sub> |
| [Project V （使用与反馈）](https://tgbox.cc/en/detail/projectv2ray/) | 12.5K | Project V official discussion group, content limited to the use of wall tools and banned… <sub>`Open Source`</sub> |
| [Clash Party讨论群(原 Mihomo Party)](https://tgbox.cc/en/detail/mihomo_party_group/) | 12K | Within group Ai anti-advertising, we are invited to contact the administrator or private… <sub>`Open Source`</sub> |
| [TGCN 机场联盟](https://tgbox.cc/en/detail/ssunion/) | 8.4K | TGCN 机场联盟 官方机场主交流中心 禁自动发送/多个小号/干扰群聊/NFSW/带AFF的广告 禁冒充官方人员 进不去群账号解封申诉等… |
| [Egern](https://tgbox.cc/en/detail/egern_app/) | 8.4K | Egern 已经在 App Store 上架，如果你喜欢 Egern 请支持一下作者并分享给身边的朋友。 App Store https://apps.apple.com/us/a… <sub>`iOS`</sub> |

<details>
<summary>Show 9 more</summary>

| Name | Members | About |
| --- | ---: | --- |
| [Surge Community](https://tgbox.cc/en/detail/surgecommunity/) | 8.3K | ❏ 本群是Surge第三方社区，限正版用户，本群不提供售后服务; ❏ 基础教学可触发Bot教程:关键词列表; ❏ 为避免纠纷，除管理人员外一律不允许合租开车、机场推广相关及“明文广… <sub>`iOS`</sub> |
| [ShadowsocksR - 技术\|讨论\|提问\|少水](https://tgbox.cc/en/detail/chatssr/) | 7K | SSR discussion group t.me/chatssr managing girlfriends banned political advertising 18+… |
| [\#v2fly](https://tgbox.cc/en/detail/v2fly_chat/) | 6.8K | Announcement channel:@v2fly Please read:https://t.me/v2fly\_chat/180673 Notice of… <sub>`Open Source`</sub> |
| [mini club](https://tgbox.cc/en/detail/orz_mini/) | 6.5K | mini频道：https://t.me/Orzmini |
| [CYLINK - 不再支持SSR协议，请更新订阅！！！](https://tgbox.cc/en/detail/cylink/) | 5.1K | 次元链接\|40Gbps带宽冗余\|IEPL专线\|线路齐全的网络传输服务商 官方域名：https://2cy.now 跳转域名：https://次元.vip… |
| [中信加速器/ZxfastVPN官方群 - IPLC专线/V2RAY引擎/4K极速…](https://tgbox.cc/en/detail/zxfast/) | 3.8K | 🚚 优质线路，IPLC&BGP全中转线路 🥳 全客户端，安卓/iOS/Win/Mac无缝切换 🌐 全球覆盖，无缝连接亚美欧澳四大洲 🇨🇳 版权限制，海外用户惬意使用回国节点 🔐… |
| [Netch 使用交流群组](https://tgbox.cc/en/detail/netch_discuss_group/) | 2.9K | Support Socks5, SSR, SSR, VMess, Trojan Proxy UDP NAT FullCone - Prohibited Advertising -… <sub>`Windows`</sub> |
| [SSPanel之國 章魚集團🐙](https://tgbox.cc/en/detail/woaizyg/) | 1.6K | 声明： 群文件仅供学习研究 如需售后和长久更新 请购买正版获得服务 本群所有文件虽免费 安装咨询服务仍收费 有能力你大可自己装 没能力请你虚心请教 章鱼哥的网盘：… <sub>`Open Source`</sub> |
| [DLK老群](https://tgbox.cc/en/detail/dlkvpn/) | 1.5K | 官网地址： dlk.xyz 软件更换问题，请自行参考教程解决 dlk.xyz/doc 电脑端： Netch t.me/Netch SSR t.me/dlkvpn/913416… |

</details>

[All vpn & proxy on TGbox →](https://tgbox.cc/en/group/vpn/)

<a id="group-interest"></a>

### Interests

| Name | Members | About |
| --- | ---: | --- |
| [貼圖群 - 進群閱置頂 Sticker Group Taiwan](https://tgbox.cc/en/detail/stickergroup/) | 39.6K | This group specifies telegra.ph/Sticker-Group-Rule-03-22 This group links… |
| [Google Voice 交流群](https://tgbox.cc/en/detail/googlevoice/) | 13.4K | Discuss the methods and techniques for using Google Voice numbers, as well as the… |
| [好棒 🌈 羊毛超级搜](https://tgbox.cc/en/detail/dajiajia/) | 9.3K | QQ WeChat Group https://jinshuju.net/f/tZwNdJ ⬇️ Shepherd Channel @yangwangpindao… <sub>`Freebies`</sub> |
| [全球Sim卡爱好者交流群](https://tgbox.cc/en/detail/simfans/) | 9K | Official channel: @kayouhome Buy Hong Kong Macau Japan Card Contact: @hkmovie666 If… |
| [期货与期权📈📉📊 Derivatives](https://tgbox.cc/en/detail/cnderivatives/) | 6.5K | Futures, options and speculations of various financial transactions Equity Equity Equity… <sub>`Finance`</sub> |
| [Tg云搜索](https://tgbox.cc/en/detail/tgsongs/) | 5.8K | 全球无限音乐资源 在线听歌找歌、在线资源搜索 在线问题解答、谷歌级电报云搜索 更多资源，请点击电报群组索引： https://t.me/TgTrillion <sub>`Music`</sub> |
| [ACG萌](https://tgbox.cc/en/detail/acg_moe/) | 5.6K | Discuss any ACG-related topics (animation, COS, secondary, etc.) Please observe the… <sub>`Anime`</sub> |
| [Google Voice 互拨交流群](https://tgbox.cc/en/detail/zh_gv/) | 5K | The group discusses GV-related topics, as well as interconnection. ⚠️ The group does not… |
| [HiFi音乐资源](https://tgbox.cc/en/detail/hifimusicresource/) | 4.1K | Please support the purchase of original music not for commercial use, download and delete… <sub>`Music`</sub> |
| [青桔ACG](https://tgbox.cc/en/detail/qingjuacg_chat/) | 4.1K | This group is an affiliate group of "Naranja ACG" channel: @QingjuACG Naranja network:… <sub>`Anime`</sub> |
| [羊毛党--TG支部 (薅死羊不偿命系列)🐑🐑🐑](https://tgbox.cc/en/detail/cn_coupon/) | 4K | The Dead Sheep Series. <sub>`Freebies`</sub> |
| [Amazon 海淘购物交流群](https://tgbox.cc/en/detail/firstamazon/) | 3.7K | Meria (American Amazon) purchases, packs, shipping, discount, Fire TV Stick purchases,… |
| [网易云音乐](https://tgbox.cc/en/detail/yuncun/) | 2.9K | No music, no life. Music makes people happy, and music is also a cure for sadness. <sub>`Music`</sub> |
| [体育爱好者\|足球\|篮球\|NBA\|CBA\|交流](https://tgbox.cc/en/detail/tiyu365/) | 2.7K | Sports enthusiasts, football, basketball, NBA, CBA, exchanges All advertising /… <sub>`Sports`</sub> |
| [爷青结动画分享交流群](https://tgbox.cc/en/detail/yeqingjie/) | 2.4K | Take a look at your childhood memories! (prohibited advertising and any form of… <sub>`Anime`</sub> |

<details>
<summary>Show 7 more</summary>

| Name | Members | About |
| --- | ---: | --- |
| [智能手机讨论组📱](https://tgbox.cc/en/detail/m_phone/) | 2.1K | The smartphone discussion group, mainly discussing various mobile phones, prohibits the… <sub>`Gadgets`</sub> |
| [中文独立博客](https://tgbox.cc/en/detail/indieblogs/) | 2K | Chinese independent blogs |
| [WallStreetBets华尔街中文官方交流群](https://tgbox.cc/en/detail/wsbetszh/) | 2K | WallStreetBets Twitter: https://twitter.com/wallstreetbets & https://www.twitter.com/WSBMa… <sub>`Finance`</sub> |
| [\[上半身不配衣冠南渡\] 摄影](https://tgbox.cc/en/detail/cnphotog/) | 1.9K | Here is the photo group of @Lychee\_Li Welcome to the discussion of all kinds of… <sub>`Photography`</sub> |
| [亨嘉之会 \| 二次元美图仓库 🅥](https://tgbox.cc/en/detail/hengjiazhihui/) | 1.2K | 二次元/动漫/插画作品互动讨论，闲聊吹水群 ⚠️进群需要进行人机验证，验证失败会被移出群组。 群规： t.me/hengjiazhihui/122 关联频道：… <sub>`Anime`</sub> |
| [全球音樂鑒賞🎶](https://tgbox.cc/en/detail/qqyyjs/) | 846 | 🎵伯牙子期于此相遇相知… 🎵高山流水曲高合者众众… ❓如需帮助请联系美女管理员 @shareocean @qiandengyin ✔️本群组链接… <sub>`Music`</sub> |
| [无损音乐交流群](https://tgbox.cc/en/detail/lossless_yinyue_chat/) | 416 | <sub>`Music`</sub> |

</details>

[All interests on TGbox →](https://tgbox.cc/en/group/interest/)

<a id="group-chat"></a>

### Chat

| Name | Members | About |
| --- | ---: | --- |
| [欧易OKX官方中文群](https://tgbox.cc/en/detail/okxgroup_cn/) ✔️ | 260.3K | OKX公告频道：https://t.me/OKXAnnouncements\_CN Discord中文社区： https://discord.gg/hkCGKbbbqf… <sub>`Exchanges`</sub> |
| [Telegram 中文社群 🅥](https://tgbox.cc/en/detail/tgcnx/) | 138.3K | Telegram Chinese/Chinese/Knowledge/Tutorial, Technology, Airports, Scientific Internet...… |
| [电报人:认识世界,认识社会,认识你自己](https://tgbox.cc/en/detail/three001/) | 51.5K | Group Purpose: Information Sharing, Knowing the World, Strengthening Yourself Group Rules… |
| [Telegram 中文社区 🅥](https://tgbox.cc/en/detail/tgzhcn/) | 38.4K | Telegram Chinese/Chinese/Knowledge/Tutorial, Technology, Airports, Scientific Internet...… |
| [Telegram Party](https://tgbox.cc/en/detail/publictestgroup/) | 26.8K | Welcome to the party! 😌 Be nice 🚫 Don't invite bots or post ads, NSFW or copyrighted… <sub>`Official`</sub> |
| [Bybit 华语交流群](https://tgbox.cc/en/detail/bybitzh/) ✔️ | 25.7K | 官方公告频道👉 https://t.me/bybit\_announcement\_ZH ❗️重要提醒：问题咨询请认准本群管理员标识！不要相信任何非管理员的私聊！… <sub>`Exchanges`</sub> |
| [逼逼和他的精神股东们](https://tgbox.cc/en/detail/imbbbbbbbbbbb/) | 22.9K | Welcome to join the group of “Pirates!” 100% automatically blocked, please think! first… |
| [𝒍𝒊𝒍𝒚的聊天室](https://tgbox.cc/en/detail/lilydeyaa/) | 18.3K | Welcome to lily's chat room (rejection automatic ban) Channel: @lily\_yaya To ensure the… |
| [𝑯𝒆𝒍𝒍𝒐 𝑾𝒐𝒓𝒍𝒅](https://tgbox.cc/en/detail/hello_world_1024/) | 16.4K | Please read the group rules in detail, all violations of the rules are prohibited!… <sub>`Programming`</sub> |
| [Telegram 新手帮助](https://tgbox.cc/en/detail/newbie_chat/) | 9.5K | The Telegram New Guide channel @Newbie\_Guide affiliate group is mainly used for channel… |
| [The Forum](https://tgbox.cc/en/detail/theforum/) | 9.2K | <sub>`Official`</sub> |
| [TG 简中交流（水）群](https://tgbox.cc/en/detail/cnpub/) | 7.6K | Telegram English \*\* Non-official\*\* Community Communication Group Related Projects:… |
| [在花小茶馆🍵](https://tgbox.cc/en/detail/zaihuachat/) | 6.7K | 科技圈投稿：@zaihuabot 【莫谈国事】 大家多多分享 • 禁止推广/黑产/刷屏/色情/ NSFW • 禁止黄赌毒/宗教/政治/键政 • 禁止撕逼/人身攻击/阴阳怪气… <sub>`Gadgets`</sub> |
| [逗比据地根](https://tgbox.cc/en/detail/doubi/) | 6.2K | Please follow the following group rules: - Banned screenshots - Banned rumors - Banned… |
| [Yummy's Small Talk](https://tgbox.cc/en/detail/godlygroup/) | 5.4K | Group Old Names: I'm So Lonely \* Prohibit the Spread of Rumors/Piracy \* Prohibit Screen… |

<details>
<summary>Show 4 more</summary>

| Name | Members | About |
| --- | ---: | --- |
| [PriceAI 交流群（禁广）](https://tgbox.cc/en/detail/priceaicc/) | 5.2K | PriceAI: AI subscriptions, model API, low-cost channels, commodity and functional issues.… <sub>`ChatGPT` `Open Source`</sub> |
| [科技&趣闻](https://tgbox.cc/en/detail/kejiquchat/) | 3.8K | Share valuable and interesting information channel @kejiqu Post @kejiqubot Contact… |
| [喵子科学站-吹水群](https://tgbox.cc/en/detail/mzkxzchat/) | 1.9K | Website link: www.mzkxz.net 禁️ excessive🇳 domestic politics, other aspects will \~… |
| [Snowball Fight - Streamers Den](https://tgbox.cc/en/detail/snowballfight/) | 642 | Welcome to the Snowball Fight! 😌 Be nice 🚫 Don't invite bots or post ads, NSFW or… <sub>`Memes` `Official`</sub> |

</details>

[All chat on TGbox →](https://tgbox.cc/en/group/chat/)

<a id="group-ios"></a>

### iOS

| Name | Members | About |
| --- | ---: | --- |
| [NobyDa Script](https://tgbox.cc/en/detail/nobyda_chat/) | 23.2K | The group is based on other similar applications such as QuantumultX, Surge, Loon and… |
| [🇬🇧 Nicegram Chat](https://tgbox.cc/en/detail/nicegramchat/) | 19K | Official Nicegram chat! Channel: @nicegramapp Navigation: @NicegramNavigation Download… <sub>`iOS`</sub> |
| [ios黑科技交流群](https://tgbox.cc/en/detail/ioshkj007/) | 11.8K | 各种实用软件与技巧。 京东羊毛： https://t.me/jdjd007 黑科技资源站： http://ioshkj.cn 苹果证书定制 https://s0h.cn/ios… <sub>`iOS`</sub> |
| [iOS 越狱](https://tgbox.cc/en/detail/ios_jailbreaking/) | 10.5K | The exchange of players. |
| [Miao Project](https://tgbox.cc/en/detail/miaoproject/) | 8.8K | Bilibili for tvOS update push channel:https://t.me/miaorelease The main discussion /… <sub>`Open Source`</sub> |
| [GBox官方交流群](https://tgbox.cc/en/detail/gboxtg/) | 6.1K | GBox is a free-to-escape tool for giving IPA signatures, with IPA plugins, IPA install… |
| [台灣蘋果同好交流群](https://tgbox.cc/en/detail/taiwanapplefans/) | 3.6K | ️ News & Mind ️ Hardware & Software ️ Nearby & Accessories ️ Joint & Promotion ️ Losing &… |
| [Pharos Pro For Apple](https://tgbox.cc/en/detail/shadow_x_user_support/) | 3.5K | 软件通知频道 https://t.me/Pharos\_x\_Announcements \* https://itunes.apple.com/app/pharos-pro/id145… <sub>`iOS`</sub> |
| [Price Tag Friends](https://tgbox.cc/en/detail/pricetagapp/) | 3.4K | This is a buy-and-buy group Official public number “PriceTag find the best app” Official… |
| [Telegram iOS Talk](https://tgbox.cc/en/detail/telegramiostalk/) | 3.1K | 🤠 Everyone is welcome to ask and answer 😌 Please be nice 🗣 Let's use English so everyone… <sub>`iOS`</sub> |
| [Pin apps](https://tgbox.cc/en/detail/pintg/) | 3K | Discussing applications like Pin and JSBox here 📌 Before asking questions, it’s best to… |
| [Swiftgram Chat](https://tgbox.cc/en/detail/swiftgramchat/) | 2.8K | Discuss Swiftgram in English News - @swiftgram Beta - @SGBetaSlots Source Code and More -… |
| [iOS](https://tgbox.cc/en/detail/iosdevotee/) | 2.5K | This is a China iOS user group for free talking This is a China iOS user group for free… |
| [Telegram iOS - Public beta testing](https://tgbox.cc/en/detail/tgiostests/) | 2.3K | This unofficial group is dedicated to testing and discussing Telegram beta app for iOS.… <sub>`iOS`</sub> |
| [捷径社区](https://tgbox.cc/en/detail/shortcuts_cn/) | 2.2K | Shortcuts Community Official Group Shortcuts Discuss Tech News News News Interesting… |

<details>
<summary>Show 2 more</summary>

| Name | Members | About |
| --- | ---: | --- |
| [ Apple 用户社群](https://tgbox.cc/en/detail/appleusergroup_tg/) | 1.8K | Explore Apple hardware, software products, including news, rumors, opinions and analysis,… |
| [ Mac 用户社群](https://tgbox.cc/en/detail/macusergroup/) | 932 | 苹果 macOS 操作系统用户社群。 ⭐️Mac 应用推荐：t.me/mac\_app\_store 💬Apple 社群：t.me/AppleUserGroup\_tg 🈲广告传销！… <sub>`iOS`</sub> |

</details>

[All ios on TGbox →](https://tgbox.cc/en/group/ios/)

<a id="group-ai"></a>

### AI

| Name | Members | About |
| --- | ---: | --- |
| [ChatGPT AI](https://tgbox.cc/en/detail/gpt_user/) | 9.9K | ChatGPT uses AI robots to communicate <sub>`ChatGPT`</sub> |
| [OpenClaw 中文社区](https://tgbox.cc/en/detail/openclaw_group/) | 6.9K | 本群为 OpenClaw 中文免费社区，用于技术学习交流； 禁止一切邀请、推广、拉人头链接，禁止向国内平台引流！ 置顶消息: https://t.me/OpenClaw\_Group… <sub>`AI Agents`</sub> |
| [CLIProxyAPI交流群](https://tgbox.cc/en/detail/cliproxyapi/) | 6.7K | <sub>`AI Coding` `API Relay` `Open Source`</sub> |
| [Cherry Studio 官方频道](https://tgbox.cc/en/detail/cherrystudioai/) | 5.7K | Desktop GPT client that supports multi-model services <sub>`LLMs` `Open Source`</sub> |
| [Packycode Chat](https://tgbox.cc/en/detail/packycode/) | 5.7K | PackyCode website: www.packycode.com Status monitoring: check.linux.do/group/Packy Use… <sub>`AI Coding` `API Relay`</sub> |
| [Kaggle](https://tgbox.cc/en/detail/kaggle/) | 4.9K | This group is for people that love Kaggle! We can discuss competitions, share ideas and… |
| [AI星球中文资源群](https://tgbox.cc/en/detail/chatgpt003/) | 3.1K | Ai Planet Chinese Channel 🇳 Artificial Intelligence Travel Guide ChatGPT Gmini Claude… <sub>`LLMs`</sub> |
| [ChatGPT中文研究所](https://tgbox.cc/en/detail/gpt345/) | 2.8K | 📢Channel：t.me/ai\_news\_cn <sub>`ChatGPT`</sub> |
| [Machine Learning CN](https://tgbox.cc/en/detail/ml_cn/) | 2.4K | 机器学习中文讨论群组，共同学习，一起炼丹。 No NSFW stuff here If you don't click a button, try again in 12… |
| [🔥AI 富豪俱乐部🔥](https://tgbox.cc/en/detail/cloudnativer/) | 2.1K | This group is for AI rich people, mainly used to discuss technology-related topics, can… <sub>`LLMs`</sub> |
| [IKunCode](https://tgbox.cc/en/detail/ikuncode/) | 2.1K | <sub>`AI Coding` `API Relay`</sub> |
| [xedu账号交流群](https://tgbox.cc/en/detail/xedume/) | 709 | Shop https://xedu.me openai to https://xeduapi.com chatgpt wholesale,jetbrains whole home… <sub>`API Relay` `ChatGPT`</sub> |
| [Claude Code中文社区](https://tgbox.cc/en/detail/claudecode_cn/) | 652 | <sub>`AI Coding` `LLMs`</sub> |
| [MN API用户群](https://tgbox.cc/en/detail/mn_api/) | 646 | <sub>`API Relay`</sub> |
| [AI 科技全球共赢交流群](https://tgbox.cc/en/detail/api_aabao/) | 476 | Welcome to share top news! <sub>`API Relay`</sub> |

<details>
<summary>Show 1 more</summary>

| Name | Members | About |
| --- | ---: | --- |
| [转转AI讨论群-uu6.top](https://tgbox.cc/en/detail/api_uu6_top/) | 424 | HTTPS://t.me/uu6\_top HTTPS://t.me/uu6\_top HTTPS://wzyp.cn/shop/zhuanzhuan… <sub>`API Relay`</sub> |

</details>

[All ai on TGbox →](https://tgbox.cc/en/group/ai/)

<a id="group-local"></a>

### Local & Regional

| Name | Members | About |
| --- | ---: | --- |
| [北京](https://tgbox.cc/en/detail/beijingz/) | 32.8K | Prohibit the spread of rumors/piracy Prohibit the stripping/blasphemy/personal assault… |
| [泉城济南电报总群 🅥](https://tgbox.cc/en/detail/jinan_tg/) | 27.1K | ❗️ The group banned ads, the advertisement must ban! banned all illegal crimes The group… |
| [深圳](https://tgbox.cc/en/detail/shenzhenz/) | 10.7K | 禁止传播谣言/盗版 禁止撕逼/谩骂/人身攻击 禁止广告/推广/黑产/灰产/暗网/刷屏/色情/开车/NSFW 搜索索引 @Googlezs 话题 @beijingv 上海… |
| [广州](https://tgbox.cc/en/detail/guangzhouz/) | 8K | 禁止传播谣言/盗版 禁止撕逼/谩骂/人身攻击 禁止广告/推广/黑产/灰产/暗网/刷屏/色情/开车/NSFW 搜索索引 @Googlezs 话题 @beijingv 上海… |
| [上海](https://tgbox.cc/en/detail/shanghaiz/) | 7.7K | 禁止传播谣言/盗版 禁止撕逼/谩骂/人身攻击 禁止广告/推广/黑产/灰产/暗网/刷屏/色情/开车/NSFW 搜索索引 @Googlezs 话题 @beijingv 上海… |
| [西安电报群](https://tgbox.cc/en/detail/xiancity/) | 7.7K | This group is used to discuss topics related to Xi'an's clothing and food housing, and… |
| [臺灣群＊](https://tgbox.cc/en/detail/formosataiwan/) | 5.3K | Free chat rooms for Taiwanese, welcome friends from Taiwan to participate. From food to… |
| [湖南电报群](https://tgbox.cc/en/detail/hunantg/) | 4.6K | Created in 2018, the group has been committed to building a high-quality, orderly Chinese… |
| [东京IT技术者交流群](https://tgbox.cc/en/detail/tokyoit/) | 3.6K | TokyoIT discussion group |
| [武汉](https://tgbox.cc/en/detail/wuhanz/) | 2.7K | 禁止传播谣言/盗版 禁止撕逼/谩骂/人身攻击 禁止广告/推广/黑产/灰产/暗网/刷屏/色情/开车/NSFW 搜索索引 @Googlezs 话题 @beijingv 上海… |
| [中国 China](https://tgbox.cc/en/detail/cnchinese/) | 2.5K | 禁止传播谣言/盗版 禁止撕逼/谩骂/人身攻击 禁止广告/推广/黑产/灰产/暗网/刷屏/色情/开车/NSFW 搜索索引 @Googlezs 话题 @beijingv 上海… |
| [杭州](https://tgbox.cc/en/detail/hangzhouz/) | 1.6K | 禁止传播谣言/盗版 禁止撕逼/谩骂/人身攻击 禁止广告/推广/黑产/灰产/暗网/刷屏/色情/开车/NSFW 搜索索引 @Googlezs 话题 @beijingv 上海… |
| [热炸了东京 Chat](https://tgbox.cc/en/detail/japanchats/) | 1K | 解封/商务/其他任何联系 ： @appdo\_bot 低质量讨论规避原则：本群不鼓励群成员进行低质量争论，讨论事情时请言之有物就事论事，避免人身攻击、扣帽子和无端推测。… |
| [重庆](https://tgbox.cc/en/detail/chongqingz/) | 944 | 禁止传播谣言/盗版 禁止撕逼/谩骂/人身攻击 禁止广告/推广/黑产/灰产/暗网/刷屏/色情/开车/NSFW 北京 @beijingz 备用 @beijingv 上海… |

[All local & regional on TGbox →](https://tgbox.cc/en/group/local/)

<a id="group-learning"></a>

### Learning & Exams

| Name | Members | About |
| --- | ---: | --- |
| [电子书/听书/期刊杂志/课程分享](https://tgbox.cc/en/detail/sharing_books4u/) | 48.2K | Business @msgmenow\_bot Search book @sharing\_books4u Navigation https://t.me/sharebooks4you… <sub>`E-books`</sub> |
| [wikipedia-zh](https://tgbox.cc/en/detail/wikipedia_zh_n/) | 22.7K | This group is a Chinese Wikipedia chat room, which mainly discusses Wikipedia. There are… |
| [小声读书 ☘️](https://tgbox.cc/en/detail/what_youread/) | 12.9K | Just AI and Read |
| [早起读书 网盘 资源共享](https://tgbox.cc/en/detail/ideahub_ml/) | 11K | \[RSS full subscription real-time update\] News @zhihuribao\_rss New York Times… <sub>`E-books`</sub> |
| [读舍 - 享受阅读时光](https://tgbox.cc/en/detail/shufm/) | 8.3K | Reading room: enjoy reading time, talk about reading Website: https://shu.fm Channel:… |
| [杂志｜考研｜考公｜互助群](https://tgbox.cc/en/detail/waikan2023/) | 7.6K | National journals, various exams, interest journals! |
| [读书分享](https://tgbox.cc/en/detail/dushufenxiang_chat/) | 5.1K | Group Rules: 1.Please communicate with friends, discuss as much focus as you can and… |
| [zread 读书会 📚](https://tgbox.cc/en/detail/zread/) | 4.2K | ❤️Welcome bookmates Maybe you are looking for that kindle of peace. Maybe you are… |
| [\[好读\] ReadFine交流总群](https://tgbox.cc/en/detail/readfinechat/) | 4.2K | Channel @Readfine Communications Group @ReadfineChat is good for reading and provides an… <sub>`E-books`</sub> |
| [中英语言学习 \| Learning Chinese & English](https://tgbox.cc/en/detail/linguisticacademy/) | 2.4K | 1. 本群旨在汉语和英语的学习 2. 你可以聊任何话题，但请不要让人感到不适 3. 请带着爱帮助别人，或者带着感恩求助于别人 1. The purport is to learn… |
| [wikipedia-zh-science&technology](https://tgbox.cc/en/detail/wikipedia_zh_science_and_tech/) | 2.1K | 維基百科科技與科學群，氣象、程式、航太......等等都可以在此討論。 物理分群： @wikipedia\_zh\_physics 生物及醫學分群：… <sub>`Science`</sub> |

[All learning & exams on TGbox →](https://tgbox.cc/en/group/learning/)

<a id="group-vps"></a>

### Hosting & VPS

| Name | Members | About |
| --- | ---: | --- |
| [搬瓦工 (BandwagonHost)](https://tgbox.cc/en/detail/bandwagonhostgroup/) | 10.1K | In addition to gambling, politics, reaction, dishonesty, advertising, the topic is free… <sub>`Hosting & VPS`</sub> |
| [ARM&X86&VPS&聊车玩家交流群，专业！](https://tgbox.cc/en/detail/pin1group/) | 7.9K | 通知频道： t.me/NewPiN1Channel 固件频道： t.me/+SlOHJbC7dHXba3Z6 三千問: github.com/real-pin1group/3000… <sub>`Hosting & VPS`</sub> |
| [Hostloc Group](https://tgbox.cc/en/detail/myhostloc/) | 7.7K | Here is the official Hostloc group, forbidding discussing political content, forbidding… <sub>`Hosting & VPS`</sub> |
| [VPS信号旗水群](https://tgbox.cc/en/detail/vps_xinhaoqi/) | 7.3K | Includes a paid VPS community, dynamic feedback on important messages in the host… <sub>`Hosting & VPS`</sub> |
| [全球主机交流中心](https://tgbox.cc/en/detail/vpschat/) | 6.3K | Forbidden politics, forbidden pornography, forbidden advertising, forbidden black… <sub>`Hosting & VPS`</sub> |
| [VPS信号旗信息技术组](https://tgbox.cc/en/detail/vpsxinhaoqi/) | 4.9K | Focus on the discussion of cloud services such as VPS, the group does not discuss current… <sub>`Hosting & VPS`</sub> |
| [Vultr 用户群](https://tgbox.cc/en/detail/vultr_group/) | 3.8K | Vultr user base, non-official Group rules: \* Prohibit the spread of rumors/piracy \*… <sub>`Hosting & VPS`</sub> |
| [🤣古博 - 靠谱VPS交流群（禁止名字打广告）](https://tgbox.cc/en/detail/guboorg/) | 3.5K | VPS measures, reviews, and recommends VPS supplement recommendation notification:… <sub>`Hosting & VPS`</sub> |
| [腾讯云VS阿里云](https://tgbox.cc/en/detail/tencentaliyun/) | 3.4K | Welcome to join Ali Yun☆Tencent Cloudtg group Channel Recommendation: Host information… <sub>`Hosting & VPS`</sub> |
| [Cloudflare 在中国](https://tgbox.cc/en/detail/cn_cloudflare/) | 3.1K | Cloudflare in China User (Chief) Communication Group We talked about: about the site /… <sub>`Hosting & VPS`</sub> |
| [NAS私有云技术交流](https://tgbox.cc/en/detail/nasteam/) | 1.8K | NAS私有云搭建技术交流 <sub>`Hosting & VPS`</sub> |

[All hosting & vps on TGbox →](https://tgbox.cc/en/group/vps/)

<a id="group-games"></a>

### Games

| Name | Members | About |
| --- | ---: | --- |
| [\[Official\] The Sandbox](https://tgbox.cc/en/detail/sandboxgame/) | 27.2K | ❌SANDBOX TEAM WILL NEVER PM YOU FIRST OR ASK FOR MONEY❌ Official Group for The Sandbox, a… <sub>`Official`</sub> |
| [NS Xbox PS PC游戏闲聊](https://tgbox.cc/en/detail/nintendoswitchcn/) | 3.9K | We come together with the game The group does not welcome any ads, focused on the NS… |
| [\#archlinux-cn-game](https://tgbox.cc/en/detail/archlinuxcn_game/) | 1.3K | 欢迎来到 ArchLinuxCN 社区游戏频道。 加入本群之前需要先加入 @archlinuxcn\_group 另有游戏机器人分群 \#archlinux-cn-play… <sub>`Linux`</sub> |
| [wikipedia-zh-game](https://tgbox.cc/en/detail/wikipedia_zh_game/) | 1.1K | 中文維基遊戲群組。群組規則「請見置頂消息」。已有遊戲：狼人殺、阿瓦隆、Uno、1A2B、偵探任務、屏風四子棋、Hangman、OX、踩雷兢速、词义高手、31點、wordle，如果這… |
| [聊与追尾游戏热点](https://tgbox.cc/en/detail/steamtg/) | 806 | 一切皆有可能！ 你能来聊游戏真是太好了！ 抱紧友善才出口聊天！ + 拒绝无关话题，浅尝辄止即可。 + 聊与分享折扣，追尾游戏热点。 上为介绍，下为勿言 +… |

[All games on TGbox →](https://tgbox.cc/en/group/games/)

<a id="group-video"></a>

### Video & Music

| Name | Members | About |
| --- | ---: | --- |
| [TVBox 野生接口群](https://tgbox.cc/en/detail/maoyingshi/) | 23.6K | Cat video wild interface, rational blankness. Send in the crowd "interface" to get… |
| [🎬 阿里云盘交流群 💾](https://tgbox.cc/en/detail/yppshare/) | 12K | The group supports the Alie Cloud related regulations. <sub>`Cloud Drive`</sub> |
| [美剧电影吧](https://tgbox.cc/en/detail/meijukingdom/) | 7.3K | The latest comedy drama comedy comedy theater movie exchange, 115 online technical… <sub>`TV Series`</sub> |
| [NF讨论群](https://tgbox.cc/en/detail/netflixchina/) | 3K | Group affiliation channel: Frontline @FLine\_cn \* Note: If blocked / banned, the private… |
| [电影爱好者交流](https://tgbox.cc/en/detail/moviemarket_group/) | 1.6K | 电影爱好者 https://t.me/MovieAnywhere <sub>`Movies`</sub> |

[All video & music on TGbox →](https://tgbox.cc/en/group/video/)

<a id="group-jobs"></a>

### Jobs & Careers

| Name | Members | About |
| --- | ---: | --- |
| [💻☕️ Jobs\_IT](https://tgbox.cc/en/detail/jobs_it/) | 10.9K | @jobs\_it :: International IT jobs portal. Please read the rules in the pinned message!… <sub>`Programming`</sub> |
| [DeJob求职交流群](https://tgbox.cc/en/detail/dejob_official_group/) | 4.6K | DeJob｜Web3 & AI 专业服务平台 62K+用户 · 1,890+ Web3企业 🌐 dejob.ai 📢 中文 @DeJob\_official… |
| [Ethereum Jobs Chat](https://tgbox.cc/en/detail/ethereum_jobs_market/) | 4K | Post job - @gptforce Vacancies and job search requests in the field of Blockchain,… |
| [DeJob Global—Web3 Jobs Chat](https://tgbox.cc/en/detail/dejob_global_group/) | 1.5K | DeJob｜Web3 & AI Professional Services 62K+ Users · 1,890+ Web3 Companies 🌐 dejob.ai 📢 CN… |

[All jobs & careers on TGbox →](https://tgbox.cc/en/group/jobs/)

<a id="group-other"></a>

### Other

| Name | Members | About |
| --- | ---: | --- |
| [Telegram 抽奖活动索引](https://tgbox.cc/en/detail/cnlottery_123/) | 7.8K | Easy and fair Telegram Group Lottery Tool. Telegram Lottery Assistant @cnLottery\_bot… |
| [Telegram 新手問答區](https://tgbox.cc/en/detail/tgqna/) | 4K | You are welcome to ask about any problems you encounter on Telegram, and also welcome to… |

[All other on TGbox →](https://tgbox.cc/en/group/other/)

<a id="bot"></a>

## Bots

<a id="bot-tools"></a>

### Utilities

| Name | Monthly users | About |
| --- | ---: | --- |
| [Spam Info Bot](https://tgbox.cc/en/detail/spambot/) ✔️ | 35.6M | The official Spam Info Bot by Telegram. Helps users with limited accounts regain the full… <sub>`Free` `Official` `Security`</sub> |
| [BotFather](https://tgbox.cc/en/detail/botfather/) ✔️ | 8.2M | BotFather is the one bot to rule them all. Use it to create new bot accounts and manage… <sub>`Free` `Official` `Programming`</sub> |
| [Premium Bot](https://tgbox.cc/en/detail/premiumbot/) ✔️ | 6.7M | This bot lets you subscribe to Telegram Premium or buy subscription gifts for loved ones… <sub>`Official`</sub> |
| [User Info • Get ID • IDbot](https://tgbox.cc/en/detail/userinfobot/) | 598.5K | 🔍 Shows the user’s basic information @dostoyno by g.media <sub>`Free`</sub> |
| [VoteBot](https://tgbox.cc/en/detail/vote/) | 146.2K | This bot will help you create polls and share them with friends. <sub>`Free` `Official`</sub> |
| [Fake Mail](https://tgbox.cc/en/detail/fakemailbot/) | 100.2K | Create hundreds of email address and receive unlimited mails everything just under… <sub>`Free`</sub> |
| [Previews](https://tgbox.cc/en/detail/previews/) ✔️ | 72K | Use this bot to leave feedback about webpage previews generated for Telegram's Instant… <sub>`Official`</sub> |
| [Telegraph](https://tgbox.cc/en/detail/telegraph/) ✔️ | 67.2K | This bot can help you log in on Telegra.ph, manage your articles, and get page view… <sub>`Free` `Official`</sub> |
| [IDBot](https://tgbox.cc/en/detail/username_to_id_bot/) | 53K | Bot to get user, group and channel ids <sub>`Free`</sub> |
| [Transparency Reports](https://tgbox.cc/en/detail/transparency/) ✔️ | 51.6K | Get Telegram transparency reports. <sub>`Official` `Security`</sub> |
| [IDBot](https://tgbox.cc/en/detail/myidbot/) | 47.8K | Replies with your or your group's Telegram ID. In case of any disruption of this service,… |
| [Transcriber Bot](https://tgbox.cc/en/detail/transcriber_bot/) | 42K | This bot transcribes voice messages and pictures into text. @Transcriber\_botNewsChannel &… |
| [DropMail.me](https://tgbox.cc/en/detail/dropmailbot/) | 37.9K | https://dropmail.me/ in Telegram <sub>`Free` `Security`</sub> |
| [Search Report Bot](https://tgbox.cc/en/detail/searchreport/) ✔️ | 37.7K | Report problematic search terms. <sub>`Official` `Security`</sub> |
| [Webpage Bot](https://tgbox.cc/en/detail/webpagebot/) | 35.7K | <sub>`Free`</sub> |

<details>
<summary>Show 62 more</summary>

| Name | Monthly users | About |
| --- | ---: | --- |
| [Creation Date](https://tgbox.cc/en/detail/creationdatebot/) | 35K | shows the approximate creation date for any account in telegram upd: @devctl <sub>`Free`</sub> |
| [GetIDs Bot](https://tgbox.cc/en/detail/getidsbot/) | 30.4K | This bot gives you telegram-internal information about messages 👤 by @wjclub <sub>`Free`</sub> |
| [Telegram Bot Raw](https://tgbox.cc/en/detail/rawdatabot/) | 25.3K | ℹ️ Made by @SeanChannel <sub>`Open Source`</sub> |
| [Texify - Image to Text (OCR) - Photo to…](https://tgbox.cc/en/detail/texifybot/) | 20K | 📷 Turn any photo, book or document into editable text with fast, accurate OCR — over 100… |
| [Dr.Web](https://tgbox.cc/en/detail/drwebbot/) | 17.1K | Add this bot to your group to check shared files and links for malware, or send them to… <sub>`Free` `Security`</sub> |
| [Username Bot](https://tgbox.cc/en/detail/username_bot/) ✔️ | 16.8K | Claim an occupied username for your Telegram account, public group or channel. <sub>`Free` `Official`</sub> |
| [Verify Bot](https://tgbox.cc/en/detail/verifybot/) ✔️ | 15.2K | This bot can help you verify a Telegram channel, public group or bot. <sub>`Official` `Security`</sub> |
| [Voicy 🇺🇦](https://tgbox.cc/en/detail/voicybot/) | 15.1K | 😎 Voicy converts any voice messages, audio files and video messages it gets to text.… <sub>`Free` `Open Source`</sub> |
| [Image To Text \[OCR\]](https://tgbox.cc/en/detail/imagetotext_bot/) | 14.9K | https://www.buymeacoffee.com/raingart <sub>`Free`</sub> |
| [toncenter.com bot](https://tgbox.cc/en/detail/toncenter/) | 14.2K | Bot for managing toncenter.com keys. Support: @toncenter\_help\_bot Terms of Use:… <sub>`Programming` `TON`</sub> |
| [Durger King](https://tgbox.cc/en/detail/durgerkingbot/) | 13.7K | Use this bot to order fictional fast food – the only fast food that is good for your… <sub>`Free` `Official` `Programming`</sub> |
| [Temp Mail](https://tgbox.cc/en/detail/tempmailbot/) | 10.4K | Disposable temporary emails — your inbox will be clean: no spam, ads and malwares By… |
| [Account Created Date](https://tgbox.cc/en/detail/accountcreatedbot/) | — | Get Telegram account information and estimated creation dates using various methods.… |
| [Anti-Tracking Bot](https://tgbox.cc/en/detail/antitrackingbot/) | — | Send any link, and the bot tries its best to remove all tracking from the link you sent.… <sub>`Security`</sub> |
| [Markdown Bot](https://tgbox.cc/en/detail/bold/) | — | <sub>`Free` `Official`</sub> |
| [Bugs and Suggestions](https://tgbox.cc/en/detail/bugs/) ✔️ | — | Report bugs and suggest new features for Telegram <sub>`Free` `Official`</sub> |
| [Color Code](https://tgbox.cc/en/detail/colorcodebot/) | — | Share code snippets as beautiful syntax-highlighted images. Bugs @… <sub>`Programming`</sub> |
| [InstantViewBot](https://tgbox.cc/en/detail/corsabot/) | — | Make Instant View from any article. News @instantViewBotUpdates Ads @FPeople348463838fh <sub>`Free`</sub> |
| [docHostBot](https://tgbox.cc/en/detail/dochost_bot/) | — | Turn text, forwarded messages, Markdown, and HTML into a clean public link instantly. No… <sub>`Free`</sub> |
| [中英翻譯(English Chinese(Mandarin)…](https://tgbox.cc/en/detail/en_to_tw_bot/) | — | 這個可以在超級群中用 -\> @en\_to\_tw2\_bot 自動翻譯 英\<-\>中 Automatic translation English \<-\>… <sub>`Free`</sub> |
| [Telegram Email](https://tgbox.cc/en/detail/etlgr_bot/) | — | <sub>`Free`</sub> |
| [EU Regulation Bot](https://tgbox.cc/en/detail/euregulation/) ✔️ | — | Telegram’s EU Regulation Bot <sub>`Free` `Official` `Security`</sub> |
| [Exchange Rates](https://tgbox.cc/en/detail/exchangeratesbot/) | — | Actual exchange rates for travel, work and daily life. <sub>`Finance`</sub> |
| [翻译](https://tgbox.cc/en/detail/fanyi_bot/) | — | Offer English translation for languages around the world. Support for private chat,… <sub>`Free`</sub> |
| [Gist Bot](https://tgbox.cc/en/detail/github_gist_bot/) | — | Create Gist link of your texts, codes or snippets. <sub>`Programming`</sub> |
| [GitHub](https://tgbox.cc/en/detail/githubbot/) | — | Get notifications about events in your public GitHub repositories and post comments… <sub>`Programming`</sub> |
| [iOS Jailbreak Bot](https://tgbox.cc/en/detail/iosjailbreakbot/) | — | iOS Escape Plugin Search Robot If there is a confiscated source, please go to the… <sub>`iOS`</sub> |
| [I Run Code](https://tgbox.cc/en/detail/iruncode_bot/) | — | I run code. Source code: https://github.com/tusharsadhwani/piston\_bot Admin:… <sub>`Open Source` `Programming`</sub> |
| [isPhishBot](https://tgbox.cc/en/detail/isphishbot/) | — | This bot is going to check if an URL has been reported as a phishing site. isPhishBot is… <sub>`Security`</sub> |
| [Jobs Bot](https://tgbox.cc/en/detail/jobs_bot/) ✔️ | — | This bot lists career opportunities at Telegram and accepts candidates' applications.… <sub>`Official`</sub> |
| [Json Dump Bot](https://tgbox.cc/en/detail/jsondumpbot/) | — | This bot is developed and sends you the JSON received from your client. <sub>`Free` `Programming`</sub> |
| [JSONiceBot \| View JSON Interactively](https://tgbox.cc/en/detail/jsonicebot/) | — | A Nice JSON Viewer! + View Telegram's message details in JSON. <sub>`Programming`</sub> |
| [KillerBgBot - Remove background](https://tgbox.cc/en/detail/killerbgbot/) | — | 🎨 This bot lets you quickly remove image backgrounds with unlimited subscription, no… <sub>`Design` `Photography`</sub> |
| [Language Translator](https://tgbox.cc/en/detail/lang_translate_bot/) | — | I'll translate any language to any language I can also work in groups. @od\_bots <sub>`Free`</sub> |
| [MathBot](https://tgbox.cc/en/detail/mathbot/) | — | <sub>`Science`</sub> |
| [MTProxy Admin Bot](https://tgbox.cc/en/detail/mtproxybot/) ✔️ | — | Set up and manage Telegram MTProxy servers. <sub>`Free` `Official` `Open Source`</sub> |
| [ID查询机器人](https://tgbox.cc/en/detail/nminfobot/) | — | 1. forward a chat message to me, quickly query the other person's ID 2. send any message… |
| [NodeRssBot](https://tgbox.cc/en/detail/noderssbot/) | — | author: @sinchang github: https://github.com/sinchang/rssbot <sub>`Free`</sub> |
| [OCR this](https://tgbox.cc/en/detail/ocrthis_bot/) | — | Donations: BTC: bc1qvezhs7mrwvg9j56ulxj2cplfcdps8w06p3qr47 paypal:… |
| [TOTP](https://tgbox.cc/en/detail/onetimepassbot/) | — | Yet, just another bot to generate totp <sub>`Security`</sub> |
| [Telegram Press Team](https://tgbox.cc/en/detail/pressbot/) ✔️ | — | This bot can be used to submit press requests to the Telegram team. <sub>`Official`</sub> |
| [PronunciationBot](https://tgbox.cc/en/detail/pronunciationbot/) | — | This is a bot that allows you to convert text into speech in 84 languages. <sub>`Free`</sub> |
| [QRwizard](https://tgbox.cc/en/detail/qr_wizardbot/) | — | Bot can magically decode QR images and photos to data and encode data to QR code image. <sub>`Free`</sub> |
| [QR bot](https://tgbox.cc/en/detail/qrcodebot/) | — | by ChatReactor |
| [QRcoder](https://tgbox.cc/en/detail/qrcodegen_bot/) | — | QR-code generator and decoder. Another bot for decoding: @QRHelper\_bot |
| [QR Code Bot](https://tgbox.cc/en/detail/qrcoderobot/) | — | Convert any QR Code, I will Get it decoded..👍 <sub>`Free`</sub> |
| [Resize Image](https://tgbox.cc/en/detail/resizertool_bot/) | — | Resizer image bot by bots8.com <sub>`Design` `Free`</sub> |
| [Rich Text Demo](https://tgbox.cc/en/detail/richtextdemobot/) | — | Get samples of Telegram's filthy rich text formatting options. <sub>`Free` `Official` `Programming`</sub> |
| [Jailbreak Bot](https://tgbox.cc/en/detail/rjailbreakbot/) | — | Tools for iOS and/or jailbreak. Save SHSH2, get device info, firmware etc Donate :… <sub>`iOS`</sub> |
| [Shop Bot](https://tgbox.cc/en/detail/shopbot/) | — | Try out the new Telegram Payments without actually paying anything. <sub>`Free` `Official` `Programming`</sub> |
| [Sticker Time Bot](https://tgbox.cc/en/detail/sticker_time_bot/) | — | https://github.com/imxieyi/sticker\_time\_bot <sub>`Free`</sub> |
| [TempMail](https://tgbox.cc/en/detail/temp_mail_bot/) | — | Bot is disabled Бот отключен <sub>`Free`</sub> |
| [Image to Text (OCR) bot](https://tgbox.cc/en/detail/the_ocr_bot/) | — | Extract text from images using this bot |
| [Translator - مترجم](https://tgbox.cc/en/detail/the_translator_bot/) | — | 🔻Translate Text to 69 Languages,either \#Directly or in \#Chats 🔻ترجمه متن به 69 زبان،… <sub>`Free`</sub> |
| [TIDA Bot](https://tgbox.cc/en/detail/tidabot/) ✔️ | — | TIDA or TAKE IT DOWN Act: Tools to Address Known Exploitation by Immobilizing Tech… <sub>`Free` `Official` `Security`</sub> |
| [Translate Father](https://tgbox.cc/en/detail/translatefather_bot/) | — | <sub>`Free`</sub> |
| [Text 2 Speech Bot](https://tgbox.cc/en/detail/txt2speechbot/) | — | Get audios in several languages from your text or even store and use your own audios.… |
| [Unicode Bot](https://tgbox.cc/en/detail/unicodeinfobot/) | — | Gives codepoints of the characters in your message. Try inline mode!… <sub>`Open Source`</sub> |
| [🔎 What's your User Id? 🪪](https://tgbox.cc/en/detail/userid_checkbot/) | — | Find your Telegram User ID, Chat ID or Channel ID instantly. Tap Start — done. Free… |
| [Voice Transcriber Bot](https://tgbox.cc/en/detail/voicetranscriberobot/) | — | Transcribe voice messages to text in a snap! |
| [Internet Archive: Wayback Machine](https://tgbox.cc/en/detail/wayback_machine_bot/) | — | Arhive your webpage to https://archive.org/web/ |
| [Whois\_Bot](https://tgbox.cc/en/detail/whois_bot/) | — | @Whois\_Bot is a simple domain whois lookup tool. The service is provided by Whois7.com |

</details>

[All utilities on TGbox →](https://tgbox.cc/en/bot/tools/)

<a id="bot-group-admin"></a>

### Group Admin

| Name | Monthly users | About |
| --- | ---: | --- |
| [Group Help](https://tgbox.cc/en/detail/grouphelpbot/) | 3M | 🤖 This is the most complete Bot to help you manage your groups easily and safely! ➡️ Add… <sub>`Free`</sub> |
| [Rose](https://tgbox.cc/en/detail/missrose_bot/) | 2.3M | Powerful telegram bot to help you manage your groups. Updates in @RoseNews. Get your own… <sub>`Free`</sub> |
| [阿福](https://tgbox.cc/en/detail/afoolgroupbot/) | 927K | Most convenient Groups & Channels Robots Everything Configurable Latest Features… <sub>`Free`</sub> |
| [Combot](https://tgbox.cc/en/detail/combot/) | 852.2K | Moderation, analytics, anti-spam, and content management for Telegram groups and… |
| [nmBot](https://tgbox.cc/en/detail/nmnmfunbot/) | 727.1K | nmBot boots chats by wonderful dramas. Send /lang to switch languages nmBot Channel… <sub>`Free`</sub> |
| [ChatKeeperBot](https://tgbox.cc/en/detail/chatkeeperbot/) | 527.3K | Just do my work ✅ More than 500,000 Admins trust me with their groups |
| [We Group Bot](https://tgbox.cc/en/detail/wegrouprobot/) | 369.7K | This is the best group management bot with hundreds of features and free to use. @WeGroup <sub>`Free`</sub> |
| [方丈机器人](https://tgbox.cc/en/detail/fangzhang_bot/) | 285.4K | The functions implemented by the robot are: new entry group verification, new entry group… |
| [Shieldy](https://tgbox.cc/en/detail/shieldy_bot/) | 195.7K | Shieldy fights Telegram spam. By using, you agree to Terms of Use bit.ly/4tpqkp4 &… <sub>`Free` `Open Source`</sub> |
| [MBot](https://tgbox.cc/en/detail/mbot/) | 165.8K | Make channel & group management smarter! Help channel: @MyBots Discussion group: @GroupTG… |
| [SangMata](https://tgbox.cc/en/detail/sangmata_bot/) | 159.3K | Bot that can track name and username changes and notify it in your group. Add me now to… |
| [KinhRoBot](https://tgbox.cc/en/detail/kinhrobot/) | 136.3K | Millisecond response speed (TG's fastest robot) 2 with ultra-rich cluster capabilities 4… |
| [Protectron](https://tgbox.cc/en/detail/protectronbot/) | 96K | AI-powered security, anti-spam and analytics. Join thousands of groups with millions of… <sub>`Security`</sub> |
| [OrgRobot](https://tgbox.cc/en/detail/orgrobot/) | 30.1K | Bot to challenge a user with questions when a user joins chat group. |
| [群管理机器人 @qunbot](https://tgbox.cc/en/detail/qunbot/) | 23.9K | @qunguanjia Updated announcement. ✅ The worldwide cluster robot features are all… |

<details>
<summary>Show 35 more</summary>

| Name | Monthly users | About |
| --- | ---: | --- |
| [The Join Captcha Bot](https://tgbox.cc/en/detail/join_captcha_bot/) | 17.3K | Bot to verify if incoming group users are real humans. Check Bot info channel:… <sub>`Free` `Open Source`</sub> |
| [PollBot](https://tgbox.cc/en/detail/pollbot/) | 16.6K | Add this bot to groups to create simple polls. |
| [广告杀手 🥷🏻](https://tgbox.cc/en/detail/guanggaoshashoubot/) | 11.7K | This robot professionally blocked Chinese advertising content as well as advertisement… |
| [joinhider\_bot](https://tgbox.cc/en/detail/joinhider_bot/) | 11K | Bot to remove system messages like when a user joined or left chat group. <sub>`Free`</sub> |
| [入群验证机器人](https://tgbox.cc/en/detail/p4captchabot/) | 10.3K | Source: https://github.com/fossifer/Telegram-CAPTCHA-bot <sub>`Free` `Open Source`</sub> |
| [TGCN-币&黄广告专杀](https://tgbox.cc/en/detail/adnamekillerbot/) | — | <sub>`Free`</sub> |
| [Alita ✨](https://tgbox.cc/en/detail/alita_robot/) | — | Hey I'm Alita, an fast group management bot with tons of features. I can manage your… <sub>`Open Source`</sub> |
| [AntiArabicScriptBot](https://tgbox.cc/en/detail/antiarabicscriptbot/) | — | Removes messages which contain Arabic script. More ad-free bots @GroupOwnerBots. <sub>`Free`</sub> |
| [反频道马甲 Bot](https://tgbox.cc/en/detail/antichannelspammersbot/) | — | Automatically remove messages sent by group members as a channel, along with a few useful… <sub>`Free`</sub> |
| [AntiHyperlinkBot](https://tgbox.cc/en/detail/antihyperlinkbot/) | — | Removes messages which contain hyperlinks (URLs, Text Links). More ad-free bots… <sub>`Free`</sub> |
| [AntiServiceMessageBot](https://tgbox.cc/en/detail/antiservicemessagebot/) | — | Removes join, add and leave messages (Service Messages). More ad-free bots… <sub>`Free`</sub> |
| [Cleaning Toolbox](https://tgbox.cc/en/detail/cleaningtoolboxbot/) | — | Be able to kick all your users or delete all messages from your channel/group easily! |
| [DaySandBox](https://tgbox.cc/en/detail/daysandbox_bot/) | — | Bot to manage chat groups. <sub>`Free`</sub> |
| [大掌门](https://tgbox.cc/en/detail/fengdoorbot/) | — | Welcome news, message self-destruction, cluster authentication - Fengko output This… |
| [群主の蜜 - 雪儿](https://tgbox.cc/en/detail/fengfaqbot/) | — | This robot needs administrator permissions. This robot should be the most customized… |
| [验证机器人](https://tgbox.cc/en/detail/group_confirmation_bot/) | — | https://t.me/docker tutorial <sub>`Free`</sub> |
| [Group Butler](https://tgbox.cc/en/detail/groupbutler_bot/) | — | This bot can help you in managing your group with rules, anti-flood, description, custom… <sub>`Open Source`</sub> |
| [hexlightning ⚡️](https://tgbox.cc/en/detail/hexlightning_bot/) | — | Taiwanese own blacklisted robots. More creativity to build a trusted service. Public… |
| [Captcha](https://tgbox.cc/en/detail/joincaptchabot/) | — | <sub>`Open Source`</sub> |
| [呆呆兽](https://tgbox.cc/en/detail/keduckbot/) | — | Group Discussion: @SparkDis Update Log: @slowStudy Questions Feedback: @intOrderBot… |
| [miniBot](https://tgbox.cc/en/detail/keyworddel_bot/) | — | This robot is based on artificial intelligence, machine learning, deep learning,… |
| [Approve By Poll](https://tgbox.cc/en/detail/kimmyxyc_approve_by_poll_bot/) | — | A Bot for voting to join the group. Open-source repository: https://github.com/KimmyXYC/Ap… <sub>`Open Source`</sub> |
| [mattata](https://tgbox.cc/en/detail/mattatabot/) | — | Powerful multi-purpose bot by @wrxck. Open source! Spread the ❤️ and check it out:… <sub>`Open Source`</sub> |
| [NoSticker](https://tgbox.cc/en/detail/nosticker_bot/) | — | Bot to manage chat groups. <sub>`Free`</sub> |
| [Policr Mini (控制台版)](https://tgbox.cc/en/detail/policr_mini_bot/) | — | An authentication robot, recently updated @policr\_changelog Feedback @policr\_community… <sub>`Free` `Open Source`</sub> |
| [Premium Join Bot](https://tgbox.cc/en/detail/premiumjoinbot/) ✔️ | — | Automatically approve join requests from Telegram Premium users. |
| [SpamMeNotBot](https://tgbox.cc/en/detail/spammenotbot/) | — | This bot provides a simple and fully automatic way to protect your group from spam/flood… |
| [Tag Everyone](https://tgbox.cc/en/detail/tageveryone_thebot/) | — | Mention all users in a group! Updates: @tageveryone\_news <sub>`Free` `Open Source`</sub> |
| [TGCN-CAPTCHA加群验证](https://tgbox.cc/en/detail/tgcnjoincaptchabot/) | — | <sub>`Free`</sub> |
| [天童AL-1S🤖 Powered by @TooruchanNews](https://tgbox.cc/en/detail/toorucaptchabot/) | — | A robot for cluster authentication project donation: https://afdian.net/@tooruchan… <sub>`Free` `Open Source`</sub> |
| [ubervotebot](https://tgbox.cc/en/detail/ubervotebot/) | — | I'm the best 🤖 to create polls with. Check out my source code and screenshots:… <sub>`Open Source`</sub> |
| [WatchDog Robot](https://tgbox.cc/en/detail/watchdog_robot/) | — | Bot to manage chat groups. <sub>`Free`</sub> |
| [Watch Door \[beta\]](https://tgbox.cc/en/detail/watchdoorbot/) | — | This bot can help you manage your group. Have problem about bot? pm @smailzhu <sub>`Free`</sub> |
| [干物妹！小霾](https://tgbox.cc/en/detail/woomaibot/) | — | Advertising channel @woomaibotchannel Use manual https://xiaomai.docs.wmlabs.net |
| [查特皮皮鸡](https://tgbox.cc/en/detail/zuoluotv_ai_bot/) | — | The Anti-Spam robot, based on OpenAI and Google Cloud AI Platform, can effectively filter… <sub>`Security`</sub> |

</details>

[All group admin on TGbox →](https://tgbox.cc/en/bot/group-admin/)

<a id="bot-media"></a>

### Media Download

| Name | Monthly users | About |
| --- | ---: | --- |
| [VK Music Bot](https://tgbox.cc/en/detail/vkmusic_bot/) | 2.3M | Find and get any music <sub>`Free` `Music`</sub> |
| [Vk Music Bot](https://tgbox.cc/en/detail/vkmbot_musikbot/) | 358K | Content can be removed at the request of the copyright holder News @vkmusicbotnews… <sub>`Free` `Music`</sub> |
| [Download IT \| YouTube, Instagram,…](https://tgbox.cc/en/detail/download_it_bot/) | 226.1K | 💾 Download IT - download almost any media to your mobile or computer. <sub>`Free`</sub> |
| [Twitter (𝕏) Media Downloader دانلود از…](https://tgbox.cc/en/detail/twittervid_bot/) | 139.1K | Twitter media downloader Support \| Ads \| پشتیبانی \| تبلیغات: @jbotssupport کانال \| info:… <sub>`Free`</sub> |
| [Classical Music](https://tgbox.cc/en/detail/music/) | 92.4K | This bot can help you find beautiful classical music. <sub>`Free` `Music` `Official`</sub> |
| [TeraBot Link To Video Bot \| Tera Box DL…](https://tgbox.cc/en/detail/terabestbot/) | 65.6K | Another TeraBox Bot: @TeraRealBot Any issue ? Dm: @CheemsSupportBot Bot Policy:… <sub>`Cloud Drive` `Free`</sub> |
| [InstaSave](https://tgbox.cc/en/detail/instasave_bot/) | 59.7K | I help you to download content from Instagram. Updates: @instasavestatus Contact:… |
| [🇺🇦YoutubeDL - YouTube Download Bot](https://tgbox.cc/en/detail/ytbdownbot/) | 50.7K | Download videos from YouTube and TT Chief: @alexmboss <sub>`Free`</sub> |
| [Douyin Downloader](https://tgbox.cc/en/detail/douyin_download_bot/) | 20.5K | Please notify @robinhust if you encounter any errors. <sub>`Free`</sub> |
| [Song🆔](https://tgbox.cc/en/detail/songidbot/) | 13.5K | Open-source bot that can identify music, similar to Shazam. Support: @SongIDsupportbot… <sub>`Music` `Open Source`</sub> |
| [YouTube Downloader](https://tgbox.cc/en/detail/safe_utubebot/) | 12.5K | I can download any videos with limit. Max limit 500 mb. If you want to download large… <sub>`Free`</sub> |
| [Reddit Downloader](https://tgbox.cc/en/detail/reddit_download_bot/) | 12K | A small bot to download the reddit posts, images and videos. Source:… <sub>`Open Source`</sub> |
| [聚合解析姬](https://tgbox.cc/en/detail/parsehubot/) | 12K | Multi-platform aggregation bot problem feedback: @MisakaSisters Update push: @mio\_room… <sub>`Open Source`</sub> |
| [CloudMusicDownloader](https://tgbox.cc/en/detail/music163bot/) | 10.2K | 一个下载网易云歌曲的bot 交流群 @Music163\_group Github: github.com/XiaoMengXinX/Music163bot-Go 爱发电:… <sub>`Free` `Music`</sub> |
| [AudD](https://tgbox.cc/en/detail/auddbot/) | — | Identifies music in voice messages, audio files, and videos Music Recognition API —… <sub>`Music`</sub> |

<details>
<summary>Show 25 more</summary>

| Name | Monthly users | About |
| --- | ---: | --- |
| [AI在线解析视频 (DC5)](https://tgbox.cc/en/detail/bilibiliparse_bot/) | — | Currently supported thumbnail analysis, shaking analysis, red book analysis, Twitter… <sub>`Free`</sub> |
| [Bilibili Feed Bot](https://tgbox.cc/en/detail/bilifeedbot/) | — | B site link content extractor with support for inline, group and channel text matching,… <sub>`Open Source`</sub> |
| [抖音Tiktok去水印bot](https://tgbox.cc/en/detail/douyintg_bot/) | — | Shake up tiktok X (Twitter) Red Book and other video image resolution Supports bulk… <sub>`Free`</sub> |
| [多平台解析下载机器人](https://tgbox.cc/en/detail/freedlvideo_bot/) | — | Support Twitter / X, Instagram, YouTube, Facebook, Threads, Bilibili, Tremor / TikTok,… <sub>`Free`</sub> |
| [GatMedia \> SaveMediaBot](https://tgbox.cc/en/detail/getmediabot/) | — | ⬇️ Get any media file from anywhere. جستجوی ویدئو و موزیک دانلود از اینستا و توئیتر |
| [🎧好音乐搜索](https://tgbox.cc/en/detail/haoyybot/) | — | Song music search robot, enter song name or singer name and turn TG into a music player!… <sub>`Free` `Music`</sub> |
| [全网视频下载](https://tgbox.cc/en/detail/icbcbot/) | — | Supported platforms: Trembling video, Trembling video, YouTube, YouTube Music, TikTok… <sub>`Free`</sub> |
| [Multi Saver \| Tiktok Facebook Instagram…](https://tgbox.cc/en/detail/multisaverxbot/) | — | For Support/Feedback message: @superuser\_bots Happy Usage! |
| [OdesliBot](https://tgbox.cc/en/detail/odesli_bot/) | — | Send a link to a song in a streaming service and get back a message with the links in… <sub>`Music`</sub> |
| [视频解析中心](https://tgbox.cc/en/detail/parsehubbot/) | — | Supported platforms: Trembling video, Trembling video, YouTube, YouTube Music, TikTok… <sub>`Free`</sub> |
| [Pixiv](https://tgbox.cc/en/detail/pixiv_bot/) | — | \[unoffoical\] wiki: https://pixiv-bot.pages.dev status: https://status.ugoira.com… <sub>`Anime`</sub> |
| [Podcastly](https://tgbox.cc/en/detail/podcastly_bot/) | — | Find new podcasts by search or recommendation system, deliver notification on new… |
| [Reddit Video Downloader](https://tgbox.cc/en/detail/redditdownloader_bot/) | — | Downloads Reddit and Facebook videos with sound. <sub>`Free`</sub> |
| [YoutubeDL](https://tgbox.cc/en/detail/remoteyoutubedl_bot/) | — | Youtube does not work anymore. Other sites may work. Good luck. See here for more… <sub>`Free`</sub> |
| [SauceNAO](https://tgbox.cc/en/detail/saucenaobot/) | — | Send me an image (image, GIF or a sticker) and I'll try to reverse search using the… <sub>`Anime`</sub> |
| [Spotify](https://tgbox.cc/en/detail/spotifybot/) | — | This bot lets you search Spotify and download or share a 30sec preview of your favorite… <sub>`Music`</sub> |
| [Spotify Save Bot \[SCAM\]](https://tgbox.cc/en/detail/spotifysavebot/) | — | Support/Advertising: @save\_support Channel: @allsaverschannel API: @fastsaver\_api <sub>`Music`</sub> |
| [Subtitles](https://tgbox.cc/en/detail/subtitle_dl_bot/) | — | I'll Provide Subtitles Of Movies @odbots <sub>`Movies`</sub> |
| [Video Downloader — TikTok, Instagram,…](https://tgbox.cc/en/detail/tiktapsavebot/) | — | Send a link, get the video back in seconds. <sub>`Free`</sub> |
| [TV Subtitles](https://tgbox.cc/en/detail/tv_subtitles_bot/) | — | I'll Provide English Subtitles Of TV Series @odbots <sub>`TV Series`</sub> |
| [utubebot](https://tgbox.cc/en/detail/utubebot/) | — | I am the Best Video Downloader with MP3 Conversion support. To search videos use… |
| [Загрузчик видео \| Ютуб \| ВкВидео \|…](https://tgbox.cc/en/detail/vid_download_robot/) | — | Download video from 1800+ platforms absolutely free <sub>`Free`</sub> |
| [VideoTubeBot](https://tgbox.cc/en/detail/videotubebot/) | — | Downloads audio/video from YouTube. Support: @MediaTube\_chat Channel: @MediaTube\_stream |
| [Web to Album](https://tgbox.cc/en/detail/web2album_bot/) | — | This bot will convert web url to album for you, in private message, in group, and in… <sub>`Free`</sub> |
| [WAIT: What Anime Is This](https://tgbox.cc/en/detail/whatanimebot/) | — | Send me any anime screenshot or GIF, and I'll tell you the exact anime, episode, and… <sub>`Anime` `Free` `Open Source`</sub> |

</details>

[All media download on TGbox →](https://tgbox.cc/en/bot/media/)

<a id="bot-search"></a>

### Search

| Name | Monthly users | About |
| --- | ---: | --- |
| [Foursquare](https://tgbox.cc/en/detail/foursquare/) | 21.4M | This bot can help you find restaurants and other places nearby and send their addresses… <sub>`Free` `Official`</sub> |
| [极搜🔍资源搜索@JISOU](https://tgbox.cc/en/detail/jisou/) | 5.7M | Telegram is the essential search engine for JISOU to help you accurately find the groups,… <sub>`Free`</sub> |
| [Yandex Image Search](https://tgbox.cc/en/detail/pic/) | 4.4M | <sub>`Free` `Official`</sub> |
| [快搜🔍资源搜索@kuai](https://tgbox.cc/en/detail/kuai/) | 2.8M | Find cool stuff all in one bot! Robot: @kuai @kuaiaa Channel: @kuaisoupd Group: @kuaisouqz <sub>`Free`</sub> |
| [新必搜 xbso 🔍 中文搜索](https://tgbox.cc/en/detail/xbso1/) | 1.3M | Search @xbso is an intelligent search robot. Send me keywords to help you find… |
| [SOSO搜搜](https://tgbox.cc/en/detail/soso/) | 1M | Telegram Chinese Search and Growth Platform: Search groups, channels and content, manage… <sub>`Free`</sub> |
| [🔍超级索引 @CJSY](https://tgbox.cc/en/detail/cjsy/) | 918.3K | Super index @CJSY Super powerful search engine, fast search, source search, lead the… <sub>`Free`</sub> |
| [Super 🔍](https://tgbox.cc/en/detail/super/) | 676.8K | TG must "search for artifacts"@Super and find the group, channel, movie, music or robot… <sub>`Free`</sub> |
| [🐴 神马搜索 🔍](https://tgbox.cc/en/detail/smss/) | 617.4K | Official channel: @o123456 (not missing attention) Communication groups: @smbot0… <sub>`Free`</sub> |
| [极搜🔍中文搜索@JISO](https://tgbox.cc/en/detail/jiso/) | 391.8K | Extreme Search JISO helps you find interesting groups, channels, videos, music, movies,… <sub>`Free`</sub> |
| [Searchee Bot](https://tgbox.cc/en/detail/searcheebot/) | 271.8K | Your guide in the world of Telegram channels. Find any channel in Telegram. ads:… <sub>`Free`</sub> |
| [Video Search for YouTube](https://tgbox.cc/en/detail/vid/) | 219.6K | This bot can help you find and share YouTube videos. By using this Bot you agree with… <sub>`Free` `Official`</sub> |
| [GIF Search](https://tgbox.cc/en/detail/gif/) | 134.9K | <sub>`Free` `Official`</sub> |
| [Bing Image Search](https://tgbox.cc/en/detail/bing/) | 72.4K | <sub>`Free` `Official`</sub> |
| [一起搜电影](https://tgbox.cc/en/detail/sotv/) | 68K | TG must be a video search engine to find your favorite movies and TV shows! welcome to… <sub>`Movies` `TV Series`</sub> |

<details>
<summary>Show 24 more</summary>

| Name | Monthly users | About |
| --- | ---: | --- |
| [搜啦🔍资源搜索机器人 @soula](https://tgbox.cc/en/detail/soula/) | 28.9K | Search @SouLa English and easily search for Telegram groups, channels, videos, music and… |
| [MotherSearch - Telegram search](https://tgbox.cc/en/detail/mothersearchbot/) | 21.3K | Search 8M+ Telegram channels and 10B+ posts. Find channels, track topics, and discover… |
| [aiso中文搜索🔍](https://tgbox.cc/en/detail/aiso/) | 14.7K | English search Required bots Send keywords Find groups, channels, movies, music or bots.… <sub>`Free`</sub> |
| [Wikipedia Search](https://tgbox.cc/en/detail/wiki/) | 11.9K | <sub>`Free` `Official`</sub> |
| [阿里云盘资源 Bot](https://tgbox.cc/en/detail/aliyunpanbot/) | — | Gather, search, collect, watch, share, and export to become the all-powerful Ali Cloud… <sub>`Cloud Drive` `Movies`</sub> |
| [阿里云盘搜索机器人🤖️](https://tgbox.cc/en/detail/aliyunshare_bot/) | — | About me I am the Ali Cloud Resource Search Robot Gathering the most full resources on… <sub>`Cloud Drive` `Movies`</sub> |
| [每日 AWESOME 搜索](https://tgbox.cc/en/detail/awesomeopensource_search_bot/) | — | Here is the search bot for @awesomeopensource, sending directly what you want to search… <sub>`Open Source`</sub> |
| [BotList Bot 🤖](https://tgbox.cc/en/detail/botlistbot/) | — | 🇬🇧Official @BotList bot 🇪🇸Bot oficial del @BotList 🤖 <sub>`Free`</sub> |
| [达摩索引](https://tgbox.cc/en/detail/damososobot/) | — | Indexing machine @SUOBOT @SYJQ Professional indexing tool that integrates a huge amount… <sub>`Free`</sub> |
| [TG群组/频道导航](https://tgbox.cc/en/detail/daohangbot/) | — |  |
| [Telegram Explorer](https://tgbox.cc/en/detail/exploretelegrambot/) | — | Explore the world of Telegram, find bots, channels, groups, stickers, masks AND secret… |
| [Hitagi](https://tgbox.cc/en/detail/hitagibot/) | — | Hitagi is qt anime bot capable of searching for TV shows, manga, characters, and fan art. <sub>`Anime`</sub> |
| [IMDb](https://tgbox.cc/en/detail/imdb/) | — | This bot automatically works in all your chats and groups, no need to add it anywhere.… <sub>`Free` `Movies` `Official`</sub> |
| [IQDB bot](https://tgbox.cc/en/detail/iqdb_org_bot/) | — | Contact @Nakagawa\_Kanon for more information. <sub>`Anime`</sub> |
| [消息检索](https://tgbox.cc/en/detail/msg_index_bot/) | — | 搜索频道 - "/search\_channel 读书", "/sc 读书" 搜索消息 - 直接输入"文章", 或 "/search 文章", "/s 文章" \#搜索 \#检索… |
| [北极星搜索](https://tgbox.cc/en/detail/polarisseekbot/) | — | Hi, I can navigate the Telegram Groups/Channels. You can click category search or enter… |
| [群组大全\_Bot](https://tgbox.cc/en/detail/qunzudaquan_bot/) | — | This bot is currently suspended, onlyins existing links, invalid links are automatically… <sub>`Free`</sub> |
| [Image Reverse Search](https://tgbox.cc/en/detail/reverse_image_search_bot/) | — | Reverse image search bot for various engines like SauceNAO, Google, Yandex and more. |
| [SmartSearch](https://tgbox.cc/en/detail/smartsearchbot/) | — |  |
| [Telegram Store Bot](https://tgbox.cc/en/detail/storebot/) | — | @Storebot helps you discover the best bots 👾 on Telegram. Explore charts, rate bots and… |
| [Telegram Channels Bot](https://tgbox.cc/en/detail/tchannelsbot/) | — | Discover the best channels 📢 available on Telegram. Explore charts, rate ⭐️ and enjoy… |
| [tg\_searcher](https://tgbox.cc/en/detail/tgsearcherbot/) | — | <sub>`Open Source`</sub> |
| [TGCN-群组频道狐🦊(不是猫)](https://tgbox.cc/en/detail/zh_groups_bot/) | — | TGCN-Group Indexing Plan Robots Update Channel: @zh\_groups Community Help:… <sub>`Free`</sub> |
| [SE-索引秘书](https://tgbox.cc/en/detail/zh_secretary_bot/) | — | User Experience Up to the Top Open Source and Non-Profit SE-Index Community: @se\_talks <sub>`Free`</sub> |

</details>

[All search on TGbox →](https://tgbox.cc/en/bot/search/)

<a id="bot-productivity"></a>

### Productivity

| Name | Monthly users | About |
| --- | ---: | --- |
| [Gmail Bot](https://tgbox.cc/en/detail/gmailbot/) ✔️ | 40.1K | This bot is an email client that works right inside Telegram. Connect your Gmail account… <sub>`Free` `Official`</sub> |
| [打卡机器人 免费版](https://tgbox.cc/en/detail/ttjfree_bot/) | 21.7K | This robot is a free-shared version of the Card Robot for experience use. Features: Card,… <sub>`Free`</sub> |
| [Skeddy](https://tgbox.cc/en/detail/skeddybot/) | 14.3K | Skeddy is a simple yet powerful reminder tool that can help you create and manage your… <sub>`Free`</sub> |
| [Alert Bot](https://tgbox.cc/en/detail/alertbot/) | — | Use this bot to create simple alerts. <sub>`Free`</sub> |
| [ExpenseBot](https://tgbox.cc/en/detail/expensebot/) | — | https://github.com/muety/telegram-expense-bot <sub>`Finance` `Open Source`</sub> |
| [Gitlab](https://tgbox.cc/en/detail/gitlab_bot/) | — | Receive notifications from Gitlab(cloud and hosted version) and response to them quickly! <sub>`Programming`</sub> |
| [IFTTT](https://tgbox.cc/en/detail/ifttt/) ✔️ | — | Link your Telegram groups or channels to more than 360 other services. |
| [keyword alert bot](https://tgbox.cc/en/detail/keyword_alert_bot/) | — | Keyword bots that can help you subscribe to messages 可以帮你订阅消息的关键字机器人 BUG反馈:… <sub>`Open Source`</sub> |
| [King打卡](https://tgbox.cc/en/detail/kingdk_bot/) | — | Playing card surveillance robots, go to work, go out of work, eat, go to the toilet,… |
| [Master Tag Alert](https://tgbox.cc/en/detail/mastertagalertbot/) | — | Unique Notification Center on Telegram, for mentions, replies, hashtags and much more! |
| [Moreform](https://tgbox.cc/en/detail/moreformbot/) | — | Forms and Surveys builder for Telegram Support - @Samandar\_Yusupov |
| [Save to Notion Bot](https://tgbox.cc/en/detail/notion_echo_bot/) | — | Telegram support group: https://t.me/+BOWbMpNPh6IzNTY0 Official website:… |
| [ONLYOFFICE](https://tgbox.cc/en/detail/onlyoffice_bot/) | — | Easily create, convert, edit and collaborate on office files using the ONLYOFFICE bot. |
| [Pomodoro Bot](https://tgbox.cc/en/detail/pomodoro_timer_bot/) | — | Pomodoro timer. Feedback: @CaddisFly. Also, try @platon\_platon\_bot - text & video… <sub>`Free`</sub> |
| [Github Releases](https://tgbox.cc/en/detail/prgitrelbot/) | — | Will send you updates for the repos you tell me to watch <sub>`Programming`</sub> |

<details>
<summary>Show 12 more</summary>

| Name | Monthly users | About |
| --- | ---: | --- |
| [Receipt AI Split](https://tgbox.cc/en/detail/receiptaisplit_bot/) | — | Split the bill without math: snap the receipt → AI reads it → who owes whom. Telegram,… |
| [RMB](https://tgbox.cc/en/detail/remindmegabot/) | — | Бот работает на платформе HostiMan.ru 💬 Support 🇬🇧🇪🇸🇷🇺 → @RMBSupport ♾ Сервис учета… |
| [Save to Notion](https://tgbox.cc/en/detail/savetonotionbot/) | — | Save content to your Notion Workspace! |
| [Email to self bot](https://tgbox.cc/en/detail/selfmailbot/) | — | Send me a message and find it right in your inbox. For GTD and email geeks. Author —… <sub>`Open Source`</sub> |
| [SplitTabs · Split Bills](https://tgbox.cc/en/detail/splittabsbot/) | — | Group bill splitter: /add 20 dinner @anna, /balance, /settle. Free 20 expenses per group. <sub>`Finance`</sub> |
| [SSL & Uptime monitoring](https://tgbox.cc/en/detail/sslmonitoringbot/) | — | Domain certificate and uptime monitoring system |
| [TasksListsBot](https://tgbox.cc/en/detail/taskslistsbot/) | — | A multiple tasks lists manager for groups and private chats. - by @Pablo\_Davila |
| [To do list manager](https://tgbox.cc/en/detail/todobot/) | — | <sub>`Free`</sub> |
| [Todorant](https://tgbox.cc/en/detail/todorant_bot/) | — | Todorant.com smart assistant. Умный ассистент todorant.com. |
| [push.tg](https://tgbox.cc/en/detail/trafficrobot/) | — | Emails and API to your Telegram |
| [Tyzenhaus](https://tgbox.cc/en/detail/tyzenhausbot/) | — | Tyzenhaus is a shared expenses tracking bot. 📃: https://github.com/madhead/tyzenhaus ✍️:… <sub>`Open Source`</sub> |
| [Telegram for Zoom](https://tgbox.cc/en/detail/zoombot/) | — | Start Zoom Meetings from any Telegram chat or direct message using a command or inline… |

</details>

[All productivity on TGbox →](https://tgbox.cc/en/bot/productivity/)

<a id="bot-channel-tools"></a>

### Channel Tools

| Name | Monthly users | About |
| --- | ---: | --- |
| [Post Bot](https://tgbox.cc/en/detail/postbot/) | 3.3M | ⚡️ Ultimate post builder. Templates, inline buttons, premium emojis, one-click sending to… <sub>`Free`</sub> |
| [Channel Help](https://tgbox.cc/en/detail/chelpbot/) | 2.8M | Most complete channel bot: post with buttons and PREMIUM EMOJI, welcome message,… |
| [Tribute](https://tgbox.cc/en/detail/tribute/) ✔️ | 1.8M | Monetize content through donations and subscriptions. News: @TributeNewsEN TRBT Limited… <sub>`Finance`</sub> |
| [Manybot](https://tgbox.cc/en/detail/manybot/) | 917.2K | 🔥Manybot lets you create your own bots. Send messages, create custom commands and menus.… |
| [TelepostBot](https://tgbox.cc/en/detail/telepostbot/) | 616.4K | Панель управления: app.telepost.me/login Официальный блог - @telepost\_blog Техподдержка… |
| [LikeBot](https://tgbox.cc/en/detail/like/) | 595K | A cool bot to create posts with emoji-based like buttons. |
| [Controller Bot](https://tgbox.cc/en/detail/controllerbot/) | 257.8K | Awesome bot for channel owners that helps you to create rich posts, view stats and more. |
| [Channel Actions Bot - Join Approver…](https://tgbox.cc/en/detail/channelactionsbot/) | 63.6K | I can auto approve as well as disapprove new channel join requests!… |
| [DiscussBot](https://tgbox.cc/en/detail/discussbot/) ✔️ | 34.3K | Make this bot an admin in your channel to add comment buttons to all posts. <sub>`Free` `Official`</sub> |
| [Comments Bot](https://tgbox.cc/en/detail/commentsbot/) | 26.2K | I can help you to create posts with comments, share them to your channels or groups. <sub>`Free`</sub> |
| [React Bot](https://tgbox.cc/en/detail/reactbot/) | 22.5K | I can help you to create rich posts with reactions and comments in your channels. |
| [Junction Bot](https://tgbox.cc/en/detail/junction_bot/) | 22.1K | 🤖 Automates your Telegram routine Message forwarding, digests and more 🌐 junctionbot.io… |
| [ComenBot](https://tgbox.cc/en/detail/comenbot/) | — | The first comment platform in Telegram. |
| [Donate](https://tgbox.cc/en/detail/donate/) ✔️ | — | This bot helps content creators receive financial support from their followers directly… <sub>`Official`</sub> |
| [InviteMember](https://tgbox.cc/en/detail/invitememberbot/) | — | InviteMember — membership bot platform for paid Telegram channels and groups. <sub>`Finance`</sub> |

<details>
<summary>Show 2 more</summary>

| Name | Monthly users | About |
| --- | ---: | --- |
| [Like and Comment](https://tgbox.cc/en/detail/likecombot/) | — | The bot helps automatically add Like and Comment buttons under every post in the channel… |
| [Markdown](https://tgbox.cc/en/detail/markdownbot/) | — | Translates text to markdown style. It has a function for the post to channels. Channel… |

</details>

[All channel tools on TGbox →](https://tgbox.cc/en/bot/channel-tools/)

<a id="bot-files"></a>

### Files & Storage

| Name | Monthly users | About |
| --- | ---: | --- |
| [File Converter](https://tgbox.cc/en/detail/newfileconverterbot/) | 53.5K | This bot allows to convert files from one format to another easily. It works with images,… <sub>`Cloud Drive` `Free`</sub> |
| [Send to Kindle 📚](https://tgbox.cc/en/detail/send2kindlebot/) | 20.6K | Send to Kindle Bot @GabrielRF http://grf.xyz <sub>`E-books`</sub> |
| [URL Uploader](https://tgbox.cc/en/detail/uploadbot/) | 19.2K | Quick way to upload files to telegram by url. Maximum file size allowed 500MB Daily… <sub>`Cloud Drive` `Free`</sub> |
| [PDF Bot](https://tgbox.cc/en/detail/pdfbot/) | 14.9K | A bot that can do a lot of things related to PDF files |
| [Get Public Link Bot](https://tgbox.cc/en/detail/getpubliclinkbot/) | 13.8K | Just Send Me any Document File of any SIZE i will Generate a External Link. Any Issues… <sub>`Cloud Drive`</sub> |
| [eBook Converter Bot](https://tgbox.cc/en/detail/ebook_converter_bot/) | — | Convert E-Books to various formats for free. Since 2022. Supports 57 input formats and 31… <sub>`E-books`</sub> |
| [File Converter](https://tgbox.cc/en/detail/fileconverterbot/) | — |  |
| [I Love PDF](https://tgbox.cc/en/detail/ilovepdfbot/) | — |  |
| [iSave](https://tgbox.cc/en/detail/isavebot/) | — | This bot will help you store and share files. |
| [Linkgrambot ™](https://tgbox.cc/en/detail/linkgrambot/) | — | • تماس با ما: @RelinkSupp کانال ربات: @RegramChannel —---------— Contact us: @RelinkSupp… <sub>`Cloud Drive` `Free`</sub> |
| [PDF Tools Bot](https://tgbox.cc/en/detail/pdftoolsbot/) | — |  |
| [File Renamer](https://tgbox.cc/en/detail/renamerbot/) | — | FIle Renamer, Send your File Here, and now you can change your file Name 😉 |
| [Safe Bot](https://tgbox.cc/en/detail/thesafebot/) | — | Use this bot to save all the files you want in an UNLIMITED and FREE Cloud <sub>`Cloud Drive` `Free`</sub> |
| [To PDF](https://tgbox.cc/en/detail/topdf_bot/) | — | Send me .doc, .docx, .odt, .txt or .jpg files and I'll convert them into .pdf. <sub>`Free`</sub> |
| [Unarchive files by @EDM115 (unzip,…](https://tgbox.cc/en/detail/unzip_edm115bot/) | — | Unarchiver bot, supports ≈ all archives types including splitted ones, allows thumb… |

<details>
<summary>Show 1 more</summary>

| Name | Monthly users | About |
| --- | ---: | --- |
| [Video Compressor Bot](https://tgbox.cc/en/detail/vidcom_robot/) | — | Video Compressor Bot \|\| Check Bot's Live Status: @VideoCompressLogs \|\| By… |

</details>

[All files & storage on TGbox →](https://tgbox.cc/en/bot/files/)

<a id="bot-stickers"></a>

### Stickers & Emoji

| Name | Monthly users | About |
| --- | ---: | --- |
| [QuotLy](https://tgbox.cc/en/detail/quotlybot/) | 662.5K | I'll help create a quote from the message. To do this, write in response to the message… <sub>`Memes`</sub> |
| [Stickers Bot](https://tgbox.cc/en/detail/stickers/) ✔️ | 650.5K | Create Telegram Stickers and get usage stats for your stickers with this bot. <sub>`Free` `Memes` `Official`</sub> |
| [fStik — Sticker & Emoji Bot \| Create…](https://tgbox.cc/en/detail/fstikbot/) | 509.2K | Create stickers and emoji from photos, videos, GIFs. Sticker catalog and search. 🇺🇦 |
| [Sticker Downloader](https://tgbox.cc/en/detail/stickerdownloadbot/) | 37.1K | Download any telegram sticker in jpg, png & Webp formats, or whole sticker pack in zip… <sub>`Free`</sub> |
| [StickerBot](https://tgbox.cc/en/detail/sticker/) | 12.8K | This bot will help you find new relevant stickers for your favorite emoji. <sub>`Free` `Official`</sub> |
| [Sticker Downloader](https://tgbox.cc/en/detail/downloadstickersbot/) | 10.5K | Download your favourite stickers in jpg,png,webp formats or whole pack as zip file. Join… <sub>`Free`</sub> |
| [Favorite Stickers Bot](https://tgbox.cc/en/detail/favorite_stickers_bot/) | — | Create sticker pack made easy! Video, copying, collaborative editing, and more! Feedback:… <sub>`Free`</sub> |
| [MyPack](https://tgbox.cc/en/detail/mypackbot/) | — | I can create your personal pack with any media files. Without limits and installing. In… |
| [Sticker Optimizer](https://tgbox.cc/en/detail/newstickeroptimizerbot/) | — | Written by Luca Patera (@Lukasss93). |
| [RateStickerBot](https://tgbox.cc/en/detail/ratestickerbot/) | — | This bot allows you to rate random stickers and discover new sticker sets. |
| [Sticker Convert Bot](https://tgbox.cc/en/detail/sticker_convert_bot/) | — | Converts images and URLs into the required format for Telegram Stickers, so you can… |
| [Sticker To GIF](https://tgbox.cc/en/detail/sticker2gifbot/) | — | Convert stickers to gif, use in other apps. Contact @rankun203 for any issues, thanks <sub>`Free`</sub> |
| [AI Sticker Maker 🎨](https://tgbox.cc/en/detail/stickeraimakerbot/) | — | AI sticker maker \| Selfie to custom packs \| 9 Stars \| Invite friends free <sub>`AI Art`</sub> |
| [Stickerify](https://tgbox.cc/en/detail/stickerifyimagebot/) | — | Convert images and videos into the format required to be used as Telegram stickers. <sub>`Open Source`</sub> |
| [Stickerizer](https://tgbox.cc/en/detail/stickerizerbot/) | — | This bot let's you create stickers on the fly, via Telegram Mini App Developed by… |

<details>
<summary>Show 1 more</summary>

| Name | Monthly users | About |
| --- | ---: | --- |
| [StickerSetBot](https://tgbox.cc/en/detail/stickerset2packbot/) | — | Export sticker to PNG, or multiple stickers/sticker sets to a zip of PNG/JPG. <sub>`Free`</sub> |

</details>

[All stickers & emoji on TGbox →](https://tgbox.cc/en/bot/stickers/)

<a id="bot-rss"></a>

### RSS & Feeds

| Name | Monthly users | About |
| --- | ---: | --- |
| [El Monitorro - Feed Reader](https://tgbox.cc/en/detail/el_monitorro_bot/) | — | El Monitorro is Feed Reader. Type /help to see available commands. Join… <sub>`Free` `Open Source`</sub> |
| [EventsAggregator](https://tgbox.cc/en/detail/eventsaggregatorbot/) | — | Forward your favorite Twitter, Instagram, RSS, Reddit, YouTube, Twitch accounts in… <sub>`Free`</sub> |
| [Feed Man Bot](https://tgbox.cc/en/detail/feedmanbot/) | — | @FeedManBot Your perfect companion to keep up to date with all of your RSS feeds <sub>`Free`</sub> |
| [NodeRSSBot](https://tgbox.cc/en/detail/noderss_bot/) | — | A RSSBot in Node.js https://github.com/fengkx/NodeRSSBot. Feel free to post issue or pull… <sub>`Free` `Open Source`</sub> |
| [一个勤勤恳恳的RSS订阅器](https://tgbox.cc/en/detail/pdorss_bot/) | — | <sub>`Free`</sub> |
| [RSS bot](https://tgbox.cc/en/detail/rss2tg_bot/) | — | A fast notification bot for RSS and Atom feeds. Web: https://rss2tg.duck.consulting/… <sub>`Free`</sub> |
| [flowerss](https://tgbox.cc/en/detail/rssflowbot/) | — | flowerss is an in-app readable RSS robot with the source code address:… <sub>`Free` `Open Source`</sub> |
| [RSS-ify](https://tgbox.cc/en/detail/rssifyxbot/) | — | Made by @Burhanverse https://burhanverse.eu.org/blog/rssify |
| [RSS to Telegram Bot](https://tgbox.cc/en/detail/rsstt_bot/) | — | Channel: @RSStT\_Channel Group: @RSStT\_Group GitHub: https://git.io/RSStT <sub>`Free` `Open Source`</sub> |
| [RSS屋](https://tgbox.cc/en/detail/rsswbot/) | — | More great https://t.me/aheatwave <sub>`Free`</sub> |
| [RSSBot](https://tgbox.cc/en/detail/rustrssbot/) | — | Chinese Telegram RSS bot Project address: https://github.com/iovxw/rssbot If you have any… <sub>`Free` `Open Source`</sub> |
| [telefeedbot - web/rss reader](https://tgbox.cc/en/detail/telefeedbot/) | — | Read rss feeds and social networks right in telegram. My other bot: @BlogFatherBot… |
| [Feed Reader Bot](https://tgbox.cc/en/detail/thefeedreaderbot/) | — | The Feed Reader Bot monitors RSS feeds, X/Youtube accounts, sending you notifications… |
| [YouTube Notifier](https://tgbox.cc/en/detail/youtubenotifbot/) | — | https://github.com/xditya/YouTubeFeeds <sub>`Open Source`</sub> |

[All rss & feeds on TGbox →](https://tgbox.cc/en/bot/rss/)

<a id="bot-crypto"></a>

### Crypto & Wallets

| Name | Monthly users | About |
| --- | ---: | --- |
| [Crypto Bot](https://tgbox.cc/en/detail/cryptobot/) | 1.5M | Use @CryptoBot to buy, sell, store, @send and pay with cryptocurrency right in Telegram. <sub>`Finance`</sub> |
| [xRocket](https://tgbox.cc/en/detail/tonrocketbot/) ✔️ | 534.5K | \#1 CEX & SocialFi platform on Telegram. @xrocketnews — news channel @TonRocketSupportBot… <sub>`TON`</sub> |
| [xRocket](https://tgbox.cc/en/detail/xrocket/) ✔️ | 534.5K | \#1 CEX & SocialFi platform on Telegram. @xrocketnews — news channel @TonRocketSupportBot… <sub>`TON`</sub> |
| [Okpay💰](https://tgbox.cc/en/detail/okpay/) | 137.8K | Easy crypto transactions, secure storage, mobile top-ups, & digital red envelopes.… <sub>`Finance`</sub> |
| [kkpay💰](https://tgbox.cc/en/detail/kkpay/) | 83.3K | kkpay is a Chinese market dedicated wallet created by @okay Group, more in line with… <sub>`Finance`</sub> |
| [Bitget Wallet Lite](https://tgbox.cc/en/detail/bitgetwallet_tgbot/) ✔️ | 35.4K | Join 10M+ users to securely earn and transfer crypto on the TON blockchain Learn more… <sub>`TON`</sub> |
| [Tonkeeper](https://tgbox.cc/en/detail/tonkeeper/) ✔️ | 35.2K | Tonkeeper bot is a wallet and a direct line to our support team. Ask us any question… <sub>`TON`</sub> |
| [极搜UT钱包](https://tgbox.cc/en/detail/utbot/) | 15.1K | Secure, convenient and anonymous storage or exchange of cryptocurrencies Official channel… <sub>`Finance`</sub> |
| [ChainCrawler](https://tgbox.cc/en/detail/crawlerhunterbot/) | — | AML wallet check tool to avoid scam, rug pulls, honeypots and those under sanctions. <sub>`Security`</sub> |
| [Crypto Explorer (Crex)](https://tgbox.cc/en/detail/crexp_bot/) | — | Get information about cryptocurrency transactions and wallets. Site: https://bots8.com <sub>`On-chain Data`</sub> |
| [区块链助手 - 链上信息查询](https://tgbox.cc/en/detail/querytokenbot/) | — | Chain information query assistant energy flash rental trx exchange trx address trading… <sub>`On-chain Data`</sub> |
| [Wallet](https://tgbox.cc/en/detail/wallet/) | — | 💎 Buy and send USDT, Gold, Bitcoin, GRAM and other crypto. 🌏 Trusted by 150M users 🎧 24/7… <sub>`Finance`</sub> |

[All crypto & wallets on TGbox →](https://tgbox.cc/en/bot/crypto/)

<a id="bot-games"></a>

### Games & Fun

| Name | Monthly users | About |
| --- | ---: | --- |
| [Quiz Bot](https://tgbox.cc/en/detail/quizbot/) ✔️ | 1.4M | Create a quiz with several multiple choice questions and test your friends. <sub>`Free` `Official`</sub> |
| [Gamee](https://tgbox.cc/en/detail/gamee/) ✔️ | 273K | 🎮 The biggest gaming platform on Telegram! 🔥 Join the community 👉 @gameechannel Perks,… <sub>`Free`</sub> |
| [GameBot](https://tgbox.cc/en/detail/gamebot/) ✔️ | 204.8K | I'm a demo bot for the Telegram Gaming Platform. I can get you a few fun sample games to… <sub>`Free` `Official`</sub> |
| [Werewolf Moderator \[☮\]](https://tgbox.cc/en/detail/werewolfbot/) | 81.9K | Werewolf Moderator bot - http://www.tgwerewolf.com for information Source:… <sub>`Open Source`</sub> |
| [UNO Bot](https://tgbox.cc/en/detail/unobot/) | 42K | Play the UNO card game via inline commands <sub>`Free` `Open Source`</sub> |
| [Inline Games 🎲](https://tgbox.cc/en/detail/inlinegamesbot/) | 12.7K | I'm just a simple bot that uses inline keyboards to provide some entertainment. <sub>`Free`</sub> |
| [Avalon](https://tgbox.cc/en/detail/avalonbg_bot/) | — | Avalon Game Bot. News channel: http://telegram.me/avalonbgbot. Support group:… |
| [BlackJack Bot](https://tgbox.cc/en/detail/blackjackbot/) | — | This is a simple Bot to play the game BlackJack. You can also add it to a group. |
| [Minesweeper](https://tgbox.cc/en/detail/mine_sweeper_bot/) | — | https://github.com/hczhcz/telegram-kuso-bots <sub>`Free` `Open Source`</sub> |
| [Telegram Chess Bot](https://tgbox.cc/en/detail/tgchessbot/) | — | A bot that allows you to play chess with friends! |
| [Secret Hitler Moderator](https://tgbox.cc/en/detail/thesecrethitlerbot/) | — | This bot acts as a moderator for the card game Secret Hitler by Max Temkin |
| [TriviaBot](https://tgbox.cc/en/detail/triviabot/) | — | This bot allows you to test your trivia knowledge. Add it to groups if you'd like to… |

[All games & fun on TGbox →](https://tgbox.cc/en/bot/games/)

<a id="bot-giveaway"></a>

### Giveaways

| Name | Monthly users | About |
| --- | ---: | --- |
| [LuckyStar \| 抽奖机器人🎁](https://tgbox.cc/en/detail/myluckystar8_bot/) | 13.1K | 🌟 Unlock your luck! The easiest way to host fair giveaways and pick winners.… |
| [Telegram 抽奖助手](https://tgbox.cc/en/detail/cnlottery_bot/) | — | Easy and fair Telegram Group Lottery Tools. Lottery campaign navigation @Lottery\_home… <sub>`Free` `Freebies`</sub> |
| [Telegram 抽奖助手](https://tgbox.cc/en/detail/cnlottery123_bot/) | — | 方便好用、公平公正的 Telegram 群组抽奖工具。 抽奖活动导航 https://t.me/Lottery\_home 抽奖活动群组… |
| [抽奖秘书](https://tgbox.cc/en/detail/fengdrawbot/) | — | This robot is developed by FengSharegroup for the promotion of the lottery on the channel… <sub>`Free` `Freebies`</sub> |
| [GiveawaysBot 🎰](https://tgbox.cc/en/detail/giveaways_bot/) | — | Hi, i'm GiveawaysBot and i'm here to create and manage giveaways on Telegram. |
| [抽奖助手](https://tgbox.cc/en/detail/lotteryhelperbot/) | — | TG Bot's Father's Special Lottery Robot <sub>`Free` `Freebies`</sub> |
| [Cnlottery V2【公测中】](https://tgbox.cc/en/detail/newcnlotterybot/) | — | Easy and fair Telegram Group Lottery Tools. Lottery campaign navigation @Lottery\_home… |
| [Lottery Bot V2](https://tgbox.cc/en/detail/tglotterybot/) | — | Telegram 中文公平抽奖机器人 <sub>`Free` `Freebies`</sub> |
| [抽奖助手](https://tgbox.cc/en/detail/tglotteryv2bot/) | — | Easy and fair Telegram Group Lottery Tools. Telegram Lottery Campaign Navigation… <sub>`Freebies`</sub> |
| [抽奖机器人](https://tgbox.cc/en/detail/xiaocailotterybot/) | — | A bot that supports users, anonymous channels, and anonymous group administrators with… <sub>`Free` `Freebies`</sub> |

[All giveaways on TGbox →](https://tgbox.cc/en/bot/giveaway/)

<a id="bot-analytics"></a>

### Analytics

| Name | Monthly users | About |
| --- | ---: | --- |
| [Telemetrio](https://tgbox.cc/en/detail/telemetr_io_bot/) | 164.8K | The bot creates a New Year's greeting card with Telegram channel stats for 2023 and… |
| [TGStat Bot](https://tgbox.cc/en/detail/tgstat_bot/) | 65.7K | Пришлю статистику любого Telegram-канала, чата, публикации. |
| [Channel Stats](https://tgbox.cc/en/detail/channelstatsbot/) | — |  |
| [Crosser Bot](https://tgbox.cc/en/detail/crosser_bot/) | — | Удаляю и защищаю от ботов, поднимаю ER, анализирую каналы. Чат: @crosser\_chat… |
| [统计秘书](https://tgbox.cc/en/detail/fengstatsbot/) | — | The Secretary makes various statistics in the group, the number of speeches, etc. <sub>`Free`</sub> |
| [词云机器人](https://tgbox.cc/en/detail/iwordcloudbot/) | — | By API restriction, after admin has been granted, the robot can receive chat text, and… <sub>`Free`</sub> |
| [Telegram Statistic Information](https://tgbox.cc/en/detail/tgstatbot/) | — |  |
| [The Stats](https://tgbox.cc/en/detail/thestats_bot/) | — | Daily stats for your channels. Created by @thedevs Github: https://github.com/poeti8/thest… <sub>`Open Source`</sub> |
| [Word Frequency](https://tgbox.cc/en/detail/wordfreqbot/) | — | Frequency in the statistical group. <sub>`Free`</sub> |

[All analytics on TGbox →](https://tgbox.cc/en/bot/analytics/)

<a id="bot-translate"></a>

### Translation

| Name | Monthly users | About |
| --- | ---: | --- |
| [Yandex Translate](https://tgbox.cc/en/detail/ytranslatebot/) | 38.3K | I can translate your messeges to any language. You can talk to new people from all around… <sub>`Free`</sub> |
| [Translator](https://tgbox.cc/en/detail/tgtranslatorbot/) | 33.7K | @tgtrns \| Bot that translates messages in groups. For support: @Dubzer 🦈 <sub>`Free`</sub> |
| [Google Translate](https://tgbox.cc/en/detail/translatebot/) | 10.5K | I'm Language Translator Bot. <sub>`Free`</sub> |
| [Babelgram](https://tgbox.cc/en/detail/babelgrambot/) | — | This bot can autotranslate your messages in any group chat or use it inline to translate… |
| [BingDict - AI Translator](https://tgbox.cc/en/detail/bingdict_bot/) | — | ❤️ Automatic Chinese to English AI translation robot (started in 2018) attention channel:… <sub>`Free`</sub> |
| [Lingvanex Translator](https://tgbox.cc/en/detail/lingvanexbot/) | — | This bot can translate any text or words, convert text to speech, give definitions for… |
| [TransChat - AI Translate](https://tgbox.cc/en/detail/transchat_bot/) | — | Group chat bot based on AI that translates dozens of languages! Telegram中最好的翻译机器人！ 💁… <sub>`Free` `LLMs`</sub> |
| [Translate It!](https://tgbox.cc/en/detail/translate_it_bot/) | — |  |
| [Voice translator](https://tgbox.cc/en/detail/voice_translator_bot/) | — | Bot is not working because of author's inability to pay for infrastructure. Author:… |

[All translation on TGbox →](https://tgbox.cc/en/bot/translate/)

<a id="bot-ai"></a>

### AI Assistants

| Name | Monthly users | About |
| --- | ---: | --- |
| [ChatGPT 6 \| Seedance \| Nano Banana](https://tgbox.cc/en/detail/gpt4telegrambot/) | 2.5M | The all-in-one AI bot: ChatGPT, Claude, Gemini, DeepSeek, Midjourney, etc. Join us:… <sub>`AI Art` `ChatGPT` `LLMs`</sub> |
| [Mira](https://tgbox.cc/en/detail/mira_ibot/) ✔️ | 551.7K | Personal AI agent that turns conversations into actions. Channel @miramedia\_en Support… <sub>`AI Agents` `LLMs`</sub> |
| [Syntx AI](https://tgbox.cc/en/detail/syntxaibot/) | 372.8K | The most powerful AI on TG: Seedance 2.0, Kling 3.0, VEO 3.1, Claude, GPT, Nano banana… <sub>`AI Art` `AI Video` `LLMs`</sub> |
| [Perplexity](https://tgbox.cc/en/detail/askplexbot/) ✔️ | 18.8K | Ask Anything! <sub>`LLMs`</sub> |
| [Chat Norris 🤠](https://tgbox.cc/en/detail/chatnorrisbot/) | 14.6K | 🎙 AI Саммари Бот для чатов Канал: @chatnorris Поддержка & реклама: @norrischat <sub>`LLMs`</sub> |
| [Microsoft Copilot](https://tgbox.cc/en/detail/copilotofficialbot/) ✔️ | 12.1K | Your everyday AI companion, here to make life easier and talk about anything. Always here… <sub>`LLMs` `Official`</sub> |
| [SaveDay AI Bot](https://tgbox.cc/en/detail/savedaybot/) | — | SaveDay Bot - Capture multimedia. Search by how you think. Get important notes from any… <sub>`LLMs`</sub> |

[All ai assistants on TGbox →](https://tgbox.cc/en/bot/ai/)

<a id="bot-trading"></a>

### Trading Bots

| Name | Monthly users | About |
| --- | ---: | --- |
| [Maestro](https://tgbox.cc/en/detail/maestrosniperbot/) | 49.9K | The official Maestro 🤖 trading bot! Need help? docs.maestrobots.com @MaestroSupport… <sub>`Quant Trading`</sub> |
| [Trojan on Solana](https://tgbox.cc/en/detail/solana_trojanbot/) ✔️ | 48.1K | The Fastest and Most Advanced On-Chain Trading Bot built to put you in full control. <sub>`Quant Trading` `Solana`</sub> |
| [Banana Gun Sniper Bot](https://tgbox.cc/en/detail/bananagunsniper_bot/) | 36K | https://bananagun.io https://docs.bananagun.io https://bananagun.io/telegram… <sub>`Quant Trading`</sub> |
| [pepeboost\_sol\_bot](https://tgbox.cc/en/detail/pepeboost_sol_bot/) | 33.6K | 💰 Cashback up to 30%, with copy trading, auto sell, and limit orders support Follow us:… <sub>`Quant Trading` `Solana`</sub> |
| [BONKbot](https://tgbox.cc/en/detail/bonkbot_bot/) | 23.3K | BONKbot is the fastest and simplest way to trade on Solana. No compromises. <sub>`Quant Trading` `Solana`</sub> |
| [GMGN Sniper Bot - Solana](https://tgbox.cc/en/detail/gmgn_sol_bot/) | 14K | Backup Bots备用Bot: @US\_GMGNBOT \| @GMGN\_sol02\_bot \| @GMGN\_sol03\_bot \|@GMGN\_sol04\_bot \|… <sub>`Quant Trading` `Solana`</sub> |
| [Unibot V2 - Elite Dex Trading Bot](https://tgbox.cc/en/detail/unibotsniper_bot/) | — | Elevate your trading with Unibot. <sub>`Quant Trading`</sub> |

[All trading bots on TGbox →](https://tgbox.cc/en/bot/trading/)

<a id="bot-messaging"></a>

### Messaging

| Name | Monthly users | About |
| --- | ---: | --- |
| [Livegram Bot](https://tgbox.cc/en/detail/livegrambot/) | 1M | Livegram Bot is a builder of feedback bots for Telegram. <sub>`Free`</sub> |
| [Anonymous Telegram Bot](https://tgbox.cc/en/detail/anonymous_telegram_bot/) | — |  |
| [NodeForwardBot](https://tgbox.cc/en/detail/nodeforwardbot/) | — | Double-directional private chat forwarding robot, no ads, no number limit, permanent… <sub>`Open Source`</sub> |
| [WegramBot](https://tgbox.cc/en/detail/wegram_bot/) | — | Create your own private chat robot that doesn’t have any advertising. <sub>`Open Source`</sub> |

[All messaging on TGbox →](https://tgbox.cc/en/bot/messaging/)

<a id="how"></a>

## How this list works

- Entries come from [TGbox](https://tgbox.cc/en): submitted by owners and users through the [bot](https://t.me/tgboxccbot), reviewed by admins, then re-checked by a crawler every few hours.
- A GitHub Action rebuilds this README every day from [`/data/entries.json`](https://tgbox.cc/data/entries.json) — a copy is kept in [`data/entries.json`](data/entries.json).
- Only public entries are listed. Adult, gambling, scam and other illegal content is not accepted.

<a id="contribute"></a>

## Add or report an entry

- **Fastest:** send the link to [@tgboxccbot](https://t.me/tgboxccbot?start=submit) — it is reviewed and appears here the next day.
- Or [open an issue](https://github.com/TGrpg/awesome-telegram/issues/new/choose). Please don't edit the README by hand: it is generated and your change would be overwritten.
- Found a dead, wrong or harmful entry? [Report it](https://github.com/TGrpg/awesome-telegram/issues/new?template=report.yml).

## Related

- [TGbox](https://github.com/TGrpg/tgbox) — the open-source directory behind this list (Astro + grammY on the Cloudflare free plan). Self-host your own.

<a id="license"></a>

## License

The list is available under [CC BY 4.0](LICENSE) — attribution: “[TGbox](https://tgbox.cc/en)”. The generator script is MIT.

<div align="center"><sub>If this list helped you, a ⭐ helps others find it.</sub></div>
