// Regenerates README.md, README.zh-CN.md and data/entries.json from TGbox's open data.
// Zero dependencies: Node 22+ (global fetch). Usage:
//   node scripts/generate.mjs                  # fetch https://tgbox.cc/data/entries.json
//   node scripts/generate.mjs --input file.json
...
const text = {
  en: {
    ...
    nav: "[🌐 Website](SITE) · [📮 Submit yours](BOT?start=submit) · [🤝 Exchange links](BOT?start=links) · **English** · [简体中文](README.zh-CN.md)",
    intro:
      "Every name below opens its page on **TGbox**, a free, open-source Telegram directory: subscriber counts and growth, recent posts, creation date, activity and similar channels, in English and Chinese. The list is regenerated every day from the site's [open data](SITE/data/entries.json), so dead or banned entries drop out on their own.",
  ...
  zh: {
    ...
    nav: "[🌐 访问网站](SITE) · [📮 提交收录](BOT?start=submit) · [🤝 交换友链](BOT?start=links) · [English](README.md) · **简体中文**",
    intro:
      "下面每个名称都链接到 **TGbox** 上的详情页——一个免费、开源的 Telegram 导航站：订阅人数与增长、最近消息、创建时间、活跃度和相似频道，中英双语。列表每天根据网站的[开放数据](SITE/data/entries.json)重新生成，失效或被封的条目会自动消失。",
...
export function describe(entry, locale) {
  const translated = locale === "zh" ? entry.descriptionZh : entry.descriptionEn;
...
export function render(data, locale) {
  ...
  const fill = (s) =>
    s
      .replaceAll("SITE", site + (locale === "en" ? "/en" : ""))
      .replaceAll("BOT", BOT)
      .replaceAll("MAIN", MAIN_REPO)
      .replaceAll("REPO", REPO);
  // Links into the site's data file must not get the /en prefix.
  const fixData = (s) => s.replaceAll(`${site}/en/data/`, `${site}/data/`);
  const f = (s) => fixData(fill(s));

  const catName = (c) => (locale === "zh" ? c.nameZh : c.nameEn);
  const tagName = new Map(data.tags.map((tag) => [tag.slug, locale === "zh" ? tag.nameZh : tag.nameEn]));
...
async function main() {
  ...
  const raw =
    inputIndex > 0
      ...
      : await fetch(SOURCE, { headers: { "user-agent": `awesome-telegram (+${REPO})` } }).then((res) => {
